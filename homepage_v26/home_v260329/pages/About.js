  ...
  template: /* html */ `
    <div class="p-6 max-w-5xl mx-auto">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
        ...
        <div class="space-y-4">
          <div v-for="v in values" :key="v.id" class="card p-4 flex items-start gap-4">
            <div class="text-2xl">{{ v.icon }}</div>
            <div><div class="font-bold text-sm mb-1" style="color:var(--text-primary)">{{ v.title }}</div><div class="text-xs" style="color:var(--text-secondary)">{{ v.desc }}</div></div>
          </div>
        </div>
      </div>
      <!-- Team -->
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div v-for="m in team" :key="m.id" class="card p-5 text-center">
