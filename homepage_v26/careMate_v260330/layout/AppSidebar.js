  ...
  <div id="sidebar" :class="[sidebarOpen?'':'collapsed', mobileOpen?'open':'']">
    <div style="padding:16px 10px;overflow-y:auto;height:100%;display:flex;flex-direction:column;gap:6px;">
      <template v-for="section in config.sidebarMenu" :key="section.section">
        ...
        <button v-for="item in section.items" :key="item.menuId" @click="navigate(item.menuId, { replace: true });$emit('close-mobile')"
      ...
      <div v-if="sidebarOpen" style="margin:8px 0;padding:14px 12px;border-radius:14px;background:linear-gradient(135deg,var(--blue-dim),var(--teal-dim));border:1px solid rgba(26,115,232,0.2);">
        ...
        <button @click="navigate('booking', { replace: true });$emit('close-mobile')" class="btn-blue btn-sm" style="width:100%;padding:7px;">지금 예약하기</button>
      </div>
      <button @click="$emit('toggle-sidebar')" style="display:flex;align-items:center;justify-content:center;gap:8px;padding:8px;border-radius:8px;background:none;border:1px solid var(--border);color:var(--text-muted);cursor:pointer;font-size:0.75rem;" class="hidden-sm">
