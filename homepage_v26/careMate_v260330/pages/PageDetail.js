...
<div class="page-wrap">
  ...
  <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:24px;">
    <button v-for="p in products" :key="p.id" @click="selectedProduct=p"
      ...
      :style="selectedProduct&&selectedProduct.id===p.id?'background:var(--blue);color:#fff;border:1.5px solid var(--blue);':'background:var(--bg-card);color:var(--text-secondary);border:1.5px solid var(--border);'">
      {{ p.emoji }} {{ p.name }}
    </button>
  </div>
  <div class="detail-grid" style="display:grid;grid-template-columns:1fr 1fr;gap:28px;" v-if="selectedProduct">
    <div style="display:flex;flex-direction:column;gap:20px;">
      <div class="card" style="padding:32px;">
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;flex-wrap:wrap;">
          ...
          <div>
            <div style="font-size:0.78rem;color:var(--text-muted);margin-bottom:4px;">{{ selectedProduct.categoryLabel }}</div>
        ...
        <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
          <button class="btn-blue" @click="selectedProduct.category==='hospital'?navigate('booking'):navigate('order')">📋 신청하기</button>
    ...
    <div style="display:flex;flex-direction:column;gap:16px;">
      <div class="card" style="padding:24px;" v-if="selectedProduct.category==='hospital'">
      ...
      <div class="card" style="padding:24px;">
        ...
        <div style="display:flex;flex-direction:column;gap:8px;">
          <div v-for="p in products.filter(x=>x.id!==selectedProduct.id&&x.category===selectedProduct.category).slice(0,3)" :key="p.id"
            class="card" style="padding:12px;cursor:pointer;margin-bottom:0;" @click="selectedProduct=p">
            <div style="display:flex;align-items:center;gap:10px;">
              <span style="font-size:1.4rem;">{{ p.emoji }}</span>
              <div><div style="font-size:0.85rem;font-weight:700;color:var(--text-primary);">{{ p.name }}</div><div style="font-size:0.75rem;color:var(--blue);">{{ p.price }}</div></div>
            </div>
          </div>
        </div>
      </div>
