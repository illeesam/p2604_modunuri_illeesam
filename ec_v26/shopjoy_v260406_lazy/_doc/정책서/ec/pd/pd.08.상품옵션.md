...
### 기준상품(대표 SKU) 정책 ⭐ (2026-08-23)

**옵션상품(OPTION)의 첫 번째 옵션조합(옵션1·옵션2 각각 정렬순서 1번 값의 조합)을 "기준상품"으로 하고,
기준상품의 `add_price` 는 항상 0원으로 고정한다.**

- **이유**: 상품목록·홈 화면은 `pd_prod.sale_price`(추가금 미포함)를 대표가로 노출한다. 옵션상품인데
  모든 SKU의 `add_price` 가 0보다 크면, 목록에 보이는 대표가로는 실제로 구매 가능한 조합이 하나도
  없는 모순이 생긴다. 기준상품 `add_price=0`을 강제하면 대표가가 항상 최소 1개 조합과 정확히 일치한다.
- **판정 기준**: 옵션1 값 목록 중 정렬순서 1번째 값 × 옵션2 값 목록 중 정렬순서 1번째 값(2단 옵션이 없으면
  옵션1만) 으로 만들어지는 SKU. 관리자 화면(`PdProdDtl.js`)의 옵션값 드래그 정렬이 곧 이 판정 기준이므로,
  "1번째로 보이는 옵션값 조합"이 곧 기준상품이다.
- **적용 위치**: `PdProdDtl.js` 옵션(가격/재고) 탭 — SKU 생성/재정렬/기존 데이터 로드 시마다
  `fnEnforceBaseSku()` 가 첫 번째 SKU의 `add_price` 를 0으로 재설정하고, 해당 행에는 "기준상품" 배지가
  표시되며 추가금액 입력은 잠김(readonly) 처리된다.
- **제약**: 옵션값 순서를 바꾸면(드래그 정렬) 기준상품도 함께 바뀐다 — 새로 1번이 된 조합의 `add_price`
  가 0으로, 기존 기준상품은 다시 편집 가능한 일반 SKU로 풀린다.
...
## 변경이력
- 2026-08-23: 기준상품(대표 SKU) 정책 신설 — 옵션상품 첫 번째 옵션조합의 `add_price=0` 고정.
  `PdProdDtl.js`에 `fnEnforceBaseSku()` 적용(생성/재정렬/로드 시마다 재검증) + "기준상품" 배지 + 입력 잠금.
