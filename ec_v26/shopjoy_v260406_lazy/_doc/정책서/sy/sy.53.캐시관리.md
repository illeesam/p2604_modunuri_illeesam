...
| 메서드 | 경로 | 설명 |
|---|---|---|
| GET | `/api/cache/redis/status` | 캐시 활성 여부 + 도메인별 지원 방식 확인 |
| GET | `/api/cache/redis/info` | 도메인별 캐시 상태 (키 존재, 갱신 시각, 남은 TTL, 키 수) |
| GET | `/api/cache/redis/data/{domain}` | 도메인 전체 캐시값 조회 |
| GET | `/api/cache/redis/data/{domain}/{key}` | 서브키 단건 캐시값 조회 |
| POST | `/api/cache/redis/reloadAll` | 전체 도메인 evict + DB 재조회 |
| POST | `/api/cache/redis/reload/{domains}` | 특정 도메인 evict + DB 재조회 |
| DELETE | `/api/cache/redis/{domains}` | 특정 도메인 evict only (lazy 재적재) |
...
```
# 단일
POST /api/cache/redis/reload/sy-code
DELETE /api/cache/redis/ec-pd-prod

# 멀티
POST /api/cache/redis/reload/sy-code^sy-menu^sy-role
DELETE /api/cache/redis/ec-pd-prod^ec-pm-prom^ec-dp-disp
```
...
| 운영 중 강제 갱신 | 캐시 관리 API `POST /api/cache/redis/reload/{domain}` 호출 |
| 장애·불일치 발생 | `DELETE /api/cache/redis/{domain}` 으로 evict → lazy 재적재 |
