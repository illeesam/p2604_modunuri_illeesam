...
CREATE TABLE shopjoy_2604.odh_pay_status_hist (
    pay_status_hist_id   VARCHAR(21)  NOT NULL CONSTRAINT odh_pay_status_hist_pk_pay_status_hist_id PRIMARY KEY,
    ...
    upd_by               VARCHAR(30) ,
    upd_date             TIMESTAMP   ,
    CONSTRAINT odh_pay_status_hist_fk_pay_id FOREIGN KEY (pay_id) REFERENCES shopjoy_2604.od_pay (pay_id)
);
...
COMMENT ON COLUMN shopjoy_2604.odh_pay_status_hist.reg_site_id IS '사이트ID (sy_site.site_id)';
...
CREATE INDEX odh_pay_status_hist_ix01_chg_date ON shopjoy_2604.odh_pay_status_hist USING btree (chg_date);
CREATE INDEX odh_pay_status_hist_ix02_order_id ON shopjoy_2604.odh_pay_status_hist USING btree (order_id);
CREATE INDEX odh_pay_status_hist_ix03_pay_id ON shopjoy_2604.odh_pay_status_hist USING btree (pay_id);
