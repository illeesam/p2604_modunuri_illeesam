/* ShopJoy - Sample01: 회원 관리 CRUD Grid  (API: GET|POST|PUT|DELETE /api/fo/xs/sample1, cdGrp='S01_MEMBER')
 * ZzSample1 필드 매핑:
...
export default {
  ...
  setup() {
    ...
    const handleSearchList = async (searchType = 'DEFAULT') => {
      try {
        const res = await foApi.get('/fo/xs/sample1', { params: { cdGrp: CD_GRP }, ...coUtil.cofApiHdr('샘플-회원관리', '조회') });
    ...
    const handleSave = async () => {
      ...
      try {
        for (const r of dRows) { await foApi.delete(`/fo/xs/sample1/${r.memberId}`, coUtil.cofApiHdr('샘플-회원관리', '삭제')); }
        for (const r of uRows) { await foApi.put(`/fo/xs/sample1/${r.memberId}`, toPayload(r), coUtil.cofApiHdr('샘플-회원관리', '저장')); }
        for (const r of iRows) { await foApi.post('/fo/xs/sample1', toPayload(r), coUtil.cofApiHdr('샘플-회원관리', '저장')); }
