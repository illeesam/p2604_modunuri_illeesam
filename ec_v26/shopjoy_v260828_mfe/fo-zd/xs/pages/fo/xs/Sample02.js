/* ShopJoy - Sample02: 상품 관리 CRUD Grid (Infinity Scroll)  (API: GET|POST|PUT|DELETE /api/fo/xs/sample1, cdGrp='S02_PRODUCT')
 * ZzSample1 필드 매핑:
...
export default {
  ...
  setup() {
    ...
    const uiState = reactive({                     // UI 상태
      loading: false, error: null, dragSrc: null, focusedIdx: null, visibleCount: 10, dragMoved: false, checkAll: false,
    });
    ...
    const handleSearchList = async (searchType = 'DEFAULT') => {
      try {
        const res = await foApi.get('/fo/xs/sample1', { params: { cdGrp: CD_GRP }, ...coUtil.cofApiHdr('샘플-상품관리', '조회') });
    ...
    const handleSave = async () => {
      ...
      try {
        for (const r of dRows) { await foApi.delete(`/fo/xs/sample1/${r.productId}`, coUtil.cofApiHdr('샘플-상품관리', '삭제')); }
        for (const r of uRows) { await foApi.put(`/fo/xs/sample1/${r.productId}`, toPayload(r), coUtil.cofApiHdr('샘플-상품관리', '저장')); }
        for (const r of iRows) { await foApi.post('/fo/xs/sample1', toPayload(r), coUtil.cofApiHdr('샘플-상품관리', '저장')); }
