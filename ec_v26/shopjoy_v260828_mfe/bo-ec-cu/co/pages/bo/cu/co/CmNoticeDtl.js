/* ShopJoy Admin - 공지사항관리 상세/등록 (bo-ec-cu-co 전용 사본)
 * 원본: bo-ec-cu-ba/pages/bo/cu/ba/BoCuBaCmNoticeDtl.js — 동일 파일명/동일 window 전역명이 서로 다른
 * 도메인 폴더에 있을 때의 충돌 시나리오를 점검하려고 일부러 복사해왔다가(2026-08-28),
 * 실제로 bo-ec-cu-co(registerComponents, 태그 'CmNoticeDtl')와 bo-sy-ba(register() 메뉴
 * 항목, comp.name 폴백)가 서로 다른 경로로 같은 app.component() 이름을 두고 충돌하는
 * 걸 확인한 뒤(2026-08-29), window 전역명·컴포넌트 name 을 도메인별로 분리하고 파일도
 * `pages/bo/cu/co/`(장차 통합 시스템으로 합칠 때 pages/ 트리가 안 겹치도록 도메인
 * 경로를 미리 새겨둠) 밑으로 옮겼다 — 이제 bo-ec-cu-ba의 원본과는 이름이 달라 어떤 순서로
 * 로드되어도 충돌하지 않는다. `<cm-notice-dtl>` 태그 자체는 manifest.js 의
 * registerComponents 가 명시적으로 지정하므로(comp.name 폴백이 아니라 tag 를 직접 씀)
 * CmNoticeMng.js 템플릿은 고칠 필요가 없다.
...
export default {
  ...
  setup(props) {
    ...
    const fnLoadCodes = async () => {
      ...
      await s.saLoadCodes(['NOTICE_TYPE_CD', 'NOTICE_STATUS'], {compNm: 'CmNoticeDtl'});
