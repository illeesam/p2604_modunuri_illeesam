...
export default {
  ...
  setup(props) {
    ...
    const codes = reactive({ bbm_type: [], use_yn: [] });
    ...
    const handleBtnAction = (cmd, param = {}) => {
      ...
      // 검색조건 초기화 + 재조회 (표시경로 트리도 전체로)
        ...
        resetDetailToNew();
        return handleSearchList('DEFAULT');
      // 게시판 신규 등록 (인라인 패널 / Ctrl·휠클릭 시 새창)
      } else if (cmd === 'bbms-add') {
        if (param && (param.ctrlKey || param.metaKey || param.button === 1)) { return props.openNewWindow('syBbmDtl', null, 'new'); }
        return openNew();
      // 상세 인라인 패널 닫기
    ...
    const handleGridCellAction = (cmd, colKey, row, e = {}) => {
      ...
      if (cmd === 'bbms-cellClick') {
        ...
        if (colKey === 'btn_row_edit') {
          if (e && (e.ctrlKey || e.metaKey || e.button === 1)) { return props.openNewWindow('syBbmDtl', row.bbmId, 'edit'); }
          return handleLoadDetail(row.bbmId);
        }
        if (colKey === 'btn_row_delete') { return handleDelete(row); }
        // 보기모드 트리거 컬럼: 제목(link) 셀 + 행번호(__no__) + VIEW_COLS 명시 헤더명
        const VIEW_COLS = ['__no__'];
        if ((e.col && e.col.link) || VIEW_COLS.includes(colKey)) {
          if (e.ctrlKey || e.metaKey || e.button === 1) { return props.openNewWindow('syBbmDtl', row.bbmId); }
