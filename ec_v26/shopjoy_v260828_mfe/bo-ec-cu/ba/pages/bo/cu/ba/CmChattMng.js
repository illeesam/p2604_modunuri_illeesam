...
export default {
  ...
  setup(props) {
    ...
    const handleBtnAction = (cmd, param = {}) => {
      ...
      // 채팅 신규 등록 (인라인 패널 / Ctrl·휠클릭 시 새창)
      } else if (cmd === 'chatts-add') {
        if (param && (param.ctrlKey || param.metaKey || param.button === 1)) { return props.openNewWindow('cmChattDtl', null, 'new'); }
        return openNew();
      // 상세 인라인 패널 닫기
    ...
    const handleGridCellAction = (cmd, colKey, row, e = {}) => {
      ...
      if (cmd === 'chatts-cellClick') {
        ...
        if (colKey === 'btn_row_edit')   { if (e && (e.ctrlKey || e.metaKey || e.button === 1)) { return props.openNewWindow('cmChattDtl', row.chattRoomId, 'edit'); } return handleLoadDetail(row.chattRoomId); }
        if (colKey === 'btn_row_delete') { return handleDelete(row); }
        // 보기모드 트리거 컬럼: 제목(link) 셀 + 행번호(__no__) + VIEW_COLS 명시 헤더명
        const VIEW_COLS = ['__no__'];
        if ((e.col && e.col.link) || VIEW_COLS.includes(colKey)) {
          if (e.ctrlKey || e.metaKey || e.button === 1) { return props.openNewWindow('cmChattDtl', row.chattRoomId); }
    ...
    const searchParam = reactive({ searchType: '', searchValue: '', dateRangeType: '', dateRange: '', dateRangeStart: '', dateRangeEnd: '', chattStatusCd: '' });
