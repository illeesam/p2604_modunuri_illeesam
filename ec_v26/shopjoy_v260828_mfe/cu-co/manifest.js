 ...
 * _domainReady)은 ab-home/manifest.js 주석 참조. cu-ba(고객) 와 같은 대메뉴(cu) 아래
 * 다른 소그룹(group)으로 기여하는 별도 레포다(2026-08-28) — 요청대로 cu-ba 와 같은
 * 화면(공지사항관리/FAQ관리)을 이 레포도 독립적으로 갖고 있다(각자 다른 물리 파일로
 * 복사돼 있음 — 같은 소스, 다른 레포). id 는 cu-ba 쪽과 겹치지 않게 접미어(_co)를
...
(function () {
  ...
  Promise.all(scripts).then(function () {
    ...
    console.error('[cu-co manifest] 로드 실패:', err);
  });
})();
