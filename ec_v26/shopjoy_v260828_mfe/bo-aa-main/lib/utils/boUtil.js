...
(function (global) {
  ...
  /* bofExportPdf — coUtil.cofExportPdf 의 BO 전용 래퍼(2026-08-23 FO 도 필요해져 coUtil 로 이동).
     현재 로그인한 BO 관리자 정보를 각주에 채워 넘기는 것 외에는 순수 위임. */
  boUtil.bofExportPdf = function (el, filename, showToast) {
    const curUser = (window.sfGetBoAuthStore ? window.sfGetBoAuthStore().sgCurrentUser : null) || {};
    return coUtil.cofExportPdf(el, filename, showToast, curUser);
  };
