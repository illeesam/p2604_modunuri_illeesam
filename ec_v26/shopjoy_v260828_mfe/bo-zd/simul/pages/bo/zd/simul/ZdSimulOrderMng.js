...
import ZdSimulBase from './ZdSimulBase.js';
  const { reactive, ref, computed, onMounted } = Vue;
  const { useSimulSetup, makeLogCols, makeBaseCfgColumns, makeRangeCol, makeRangeHandlers, rangeSlotTemplate } = ZdSimulBase;
  ...
  export default {
    ...
    setup(props) {
      ...
      const onSelectMember = (row) => {
        ...
        domCfg.fixedMemberNm = ZdSimulBase?._sanitize ? ZdSimulBase._sanitize(nm) : nm;
      ...
      /* 회원 지정 기본값 — 같은 브라우저에 FO 로그인된 회원이 있으면 자동 지정.
         내가 로그인한 회원으로 주문 데이터를 만들면 FO 마이페이지에서 바로 확인할 수 있다.
         (기본값일 뿐 ✕ 로 해제 가능) */
      onMounted(() => { ZdSimulBase.fnSeedFixedMember(domCfg); });
