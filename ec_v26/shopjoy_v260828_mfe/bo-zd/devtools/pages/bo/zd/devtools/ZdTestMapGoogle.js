 ...
 */
export default {
  name: 'zd-devtools-zdTestMapGoogle',
  props: {
  ...
  setup(props) {
    ...
    const checkSdk = () => {
      ...
      result.sdkUrl     = 'https://maps.googleapis.com/maps/api/js';
      result.sdkStatus  = ok ? '✅ Google Maps SDK 로드됨' : '❌ Google Maps SDK 없음 — API 키 설정 후 [SDK 로드] 클릭';
      result.initDetail = ok ? ('API 키: ' + (cfg.apiKey || '(미설정)')) : '';
    };
    ...
    const cfGeocodeRows = () => {
      ...
      return [
        { _label: '포맷 주소', _value: g.formattedAddress },
        { _label: '위도',      _value: g.lat },
        { _label: '경도',      _value: g.lng },
        { _label: 'Place ID',  _value: g.placeId },
      ];
    };
