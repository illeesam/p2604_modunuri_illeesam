...
window.BoCuCoCmFaqDtl = {
  name: 'bo-cu-co-cmFaqDtl',
  props: {
  ...
  setup(props) {
    ...
    const handleBtnAction = (cmd, param = {}) => {
      console.log(' ■■ BoCuCoCmFaqDtl.js : handleBtnAction -> ', cmd, param);
      if (cmd === 'form-save') {
        return handleSave();
    ...
    const handleSelectAction = (cmd, param = {}) => {
      console.log(' ■■ BoCuCoCmFaqDtl.js : handleSelectAction -> ', cmd, param);
    ...
    const fnCallbackModal = (popCmd, param, result) => {
      console.log(' ■■ BoCuCoCmFaqDtl : fnCallbackModal -> ', popCmd, param, result);
    ...
    const fnLoadCodes = async () => {
      try {
        ...
        await codeStore.saLoadCodes(['USE_YN'], {compNm: 'bo-cu-co-cmFaqDtl'});
    ...
    /* fnShareUrl — 이 FAQ 상세를 가리키는 독립 새창 딥링크 URL 생성 */
    const fnShareUrl = () => {
      const qs = new URLSearchParams();
      qs.set('page', 'cmFaqDtl');
      qs.set('id', form.faqId);
      qs.set('embed', '1');
    ...
    const handleShareKakao = () => {
      try {
        window.coExtSdk.shareKakao({
          title: `FAQ ${form.faqId} - ShopJoy BO`,
          description: form.faqQuestion || '',
    ...
    /* pdfAreaRef — FAQ 상세 카드 캡처 대상. handleExportPdf — PDF 다운로드(상세보기 모드 전용) */
    const pdfAreaRef = ref(null);
    const pdfExporting = ref(false);
    const handleExportPdf = async () => {
      pdfExporting.value = true;
      try {
        const filename = coUtil.cofBuildExportFilename(`FAQ상세_${form.faqId}.pdf`);
