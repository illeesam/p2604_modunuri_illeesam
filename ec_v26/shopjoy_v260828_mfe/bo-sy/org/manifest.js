 ...
 * _domainReady)은 bo-ap-home/manifest.js 주석 참조. bo-sy/ba(기준정보) 와 같은 대메뉴(sy)
 ...
 *
 * ES 모듈 전면 전환(2026-08-29) — bo-ap-home/manifest.js 주석 참조. window.ComponentName
 * 대신 export default + R.loadModule(). id/name: 도 같은 날 구 규칙(bo-sy/org-원래이름)
 * 에서 실제 프로젝트 복제 도메인 공통 규칙(bo-원래이름)으로 갱신했다. */
(function () {
  ...
  Promise.all(scripts).then(function (results) {
    const screens = [
      { id: 'sy-org-syUserMng', label: '사용자관리', group: '조직', comp: results[1].default },
      { id: 'sy-org-syDeptMng', label: '부서관리', group: '조직', comp: results[2].default },
    ];
    const innerComps = [
