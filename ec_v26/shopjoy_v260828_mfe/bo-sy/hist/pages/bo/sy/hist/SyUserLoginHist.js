...
export default {
  ...
  setup(props) {
    ...
    const fnLoadCodes = async () => {
      ...
      await cs.saLoadCodes(['LOGIN_RESULT', 'ACTION_CD', 'DATE_RANGE_OPT'], {compNm: 'SyUserLoginHist'});
      codes.login_results   = cs?.sgGetGrpCodes('LOGIN_RESULT')   || [];
      codes.token_actions   = cs?.sgGetGrpCodes('ACTION_CD')   || [];
    ...
    const EXCEL_MAP = {
      'log': { domain: 'userLoginLog', areaNm: '사용자 로그인 로그' },
      'token': { domain: 'userTokenLog', areaNm: '사용자 토큰 이력' }
    };
    ...
    columns.logGrid = [
      ...
      { key: '_user',    label: '사용자', excelKeys: [{key:'userNm',label:'사용자명'},{key:'userId',label:'사용자ID'}],
      ...
      { key: '_browser', excelKeys: [{key:'browser',label:'브라우저'},{key:'os',label:'OS'},{key:'device',label:'기기'}], label: 'OS/브라우저', cellStyle: 'font-size:11px;color:#666;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap', cellTitle: (v, row) => (row.browser || '') + (row.os ? ' / ' + row.os : ''), fmt: (v, row) => row.browser || row.device || '-' },
...
<bo-page title="사용자로그인이력" :share-query="searchParam"
  ...
  <bo-excel-down-modal :show="excelModal.show" :domain="cfExcelDomain"
    :area-nm="cfExcelAreaNm" :columns="cfExcelColumns" ui-nm="사용자로그인이력" :params="buildExcelParams()"
