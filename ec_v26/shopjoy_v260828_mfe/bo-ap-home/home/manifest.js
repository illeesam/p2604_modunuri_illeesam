 ...
 * 그 안에 화면이 몇 개 있는지, 파일명이 뭔지는 전혀 몰라도 된다. `bo-ap-home/`
 * 자체가 별도 git 레포(shopjoy-mfe-domain-home)다 — 셸 폴더 밖의 형제(sibling)
 * 폴더로 존재한다. 이 manifest.js는 그 레포 루트가 아니라 소그룹 하위 디렉터리
 * `bo-ap-home/home/` 안에 있다(다른 도메인들과 일관되게 "형상관리 단위(레포
 * 루트)"와 "지연로드 단위(소그룹 폴더)"를 분리한 것 — 이 도메인은 소그룹이
 * 지금 1개뿐이라 폴더 이름도 대메뉴와 같은 "home"을 그대로 썼다).
 *
...
(function () {
  ...
  Promise.all(scripts).then(function (results) {
    const screens = [
      { id: 'bo-ap-home-dashboardBoEc01', label: 'EC 대시보드 1', comp: results[0].default },
      { id: 'bo-ap-home-dashboardBoEc02', label: 'EC 대시보드 2', comp: results[1].default },
    ];
    R.register('bo-home', screens);
    R._domainReady(base);
  }).catch(function (err) {
    console.error('[bo-ap-home manifest] 로드 실패:', err);
  });
})();
