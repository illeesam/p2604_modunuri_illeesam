...
const uiState = reactive({ error: null, dateRange: '이번달', dateRangeStart: '', dateRangeEnd: ''});
    ...
    const buildListParams = () => ({ reconTypeCd: 'PAY', ...coUtil.cofOmitEmpty(searchParam) });
...
<bo-page title="결제-정산 대사" :share-query="searchParam" desc-summary="결제 승인·취소 데이터와 정산 수집원장 간 금액 불일치를 검출하고 대사 처리합니다." :desc-detail="['• PG사 결제금액(pg_amt) vs 정산 수집금액(settle_amt) 차이를 자동 비교합니다.','• 결제수단: 무통장/가상계좌/토스/카카오/네이버/핸드폰','• 차이 발생 시 PG사 정산 리포트와 대조 후 조정 처리합니다.'].join(String.fromCharCode(10))">
  ...
  <bo-excel-down-modal :show="excelModal.show" domain="stRecon" area-nm="결제-정산 대사"
    ui-nm="결제-정산 대사" :columns="columns.baseGrid" :params="buildExcelParams()"
