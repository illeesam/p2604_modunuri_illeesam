/* manifest.js — "정산 > 정산현황" 마이크로 도메인의 유일한 진입점.
 ...
 * (기준정보)/bo-ec-st/raw(수집원장)/bo-ec-st/adj(정산작업)/bo-ec-st/recon(대사관리)/
 ...
 * 원본은 전혀 수정하지 않음). 실제 좌측 메뉴 구조(LEFT_MENUS.settle)의 '정산현황'
 * 그룹(화면 1개, stStatusMng)만 이 폴더가 담당한다. */
(function () {
  const R = window.MFE_REGISTRY;
  const base = document.currentScript.src.replace(/manifest\.js(\?.*)?$/, '');

  const scripts = [
    R.loadModule(base + 'pages/bo/st/status/StStatusMng.js'),
  ];

  Promise.all(scripts).then(function (results) {
    const screens = [
      { id: 'st-status-stStatusMng', label: '정산현황', group: '정산현황', comp: results[0].default },
    ];
    R.register('bo-ec-st', screens);
    R._domainReady(base);
  }).catch(function (err) {
    console.error('[bo-ec-st/status manifest] 로드 실패:', err);
  });
})();
