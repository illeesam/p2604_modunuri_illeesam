...
export default {
  ...
  setup(props) {
    ...
    const handleBtnAction = (cmd, param = {}) => {
      ...
      // 캐시 신규 등록 (인라인 패널 / Ctrl·휠클릭 시 새창)
      } else if (cmd === 'caches-add') {
        if (param && (param.ctrlKey || param.metaKey || param.button === 1)) { return props.openNewWindow('pmCacheDtl', null, 'new'); }
        return openNew();
      // 캐시 목록 엑셀 다운로드 모달 열기
    ...
    const handleGridCellAction = (cmd, colKey, row, e = {}) => {
      ...
      if (cmd === 'caches-cellClick') {
        ...
        if (colKey === 'btn_row_edit')   { if (e && (e.ctrlKey || e.metaKey || e.button === 1)) { return props.openNewWindow('pmCacheDtl', row.cacheId, 'edit'); } return handleLoadDetail(row.cacheId); }
        if (colKey === 'btn_row_delete') { return handleDelete(row); }
        // 보기모드 트리거 컬럼: 제목(link) 셀 + 행번호(__no__) + VIEW_COLS 명시 헤더명
        const VIEW_COLS = ['__no__'];
        if ((e.col && e.col.link) || VIEW_COLS.includes(colKey)) {
          if (e.ctrlKey || e.metaKey || e.button === 1) { return props.openNewWindow('pmCacheDtl', row.cacheId); }
    ...
    const fnLoadCodes = async () => {
      ...
      await codeStore.saLoadCodes(['CACHE_TRANS_TYPE', 'DATE_RANGE_OPT'], {compNm: 'PmCacheMng'});
      try {
