...
export default {
  ...
  setup(props) {
    ...
    const uiState = reactive({ loading: false, showVendorModal: false, showTargetPicker: false, error: null, tab: window._pmGiftDtlState.tab || 'info', tabMode2: window._pmGiftDtlState.tabMode || 'tab'});
    const tab = Vue.toRef(uiState, 'tab');
    const tabMode2 = Vue.toRef(uiState, 'tabMode2');
    const codes = reactive({ gift_cond_types: [], gift_statuses: [], pm_prod_targets: [] });
    ...
    const handleBtnAction = (cmd, param = {}) => {
      ...
      if (['info-form-save', 'target-form-save', 'visibility-form-save'].includes(cmd)) {
        return handleSave();
      } else if (['info-form-delete', 'target-form-delete', 'visibility-form-delete'].includes(cmd)) {
        return handleDelete();
    ...
    const handleDelete = async () => {
      if (cfIsNew.value || !form.giftId) { return; }
      const ok = await showConfirm('삭제', `[${form.giftNm}] 사은품을 삭제하시겠습니까?`);
      if (!ok) { return; }
      try {
        await boApiSvc.pmGift.remove(form.giftId, '사은품관리', '삭제');
        showToast('삭제되었습니다.', 'success');
        props.navigate('pmGiftMng', { reload: true });
      } catch (err) {
        console.error('[catch-info]', err);
        const errMsg = (err.response?.data?.message) || err.message || '오류가 발생했습니다.';
        if (showToast) { showToast(errMsg, 'error', 0); }
    ...
    /* fnShareUrl — 이 사은품 상세를 가리키는 독립 새창 딥링크 URL 생성 */
    const fnShareUrl = () => {
      const qs = new URLSearchParams();
      qs.set('page', 'pmGiftDtl');
      qs.set('id', form.giftId);
      qs.set('embed', '1');
    ...
    const handleShareKakao = () => {
      try {
        window.coExtSdk.shareKakao({
          title: `사은품 ${form.giftId} - ShopJoy BO`,
          description: form.giftDesc || form.giftNm || '',
    ...
    const handleExportPdf = async () => {
      ...
      try {
        const filename = coUtil.cofBuildExportFilename(`사은품상세_${form.giftId}.pdf`);
    ...
    return {
      ...
      cfIsNew, cfSaveDisabled, cfIsView, cfVisibilityOptions, cfSelectedVendorNm, cfIssueTargetsColumns,                         // computed
...
<bo-container :title="!active ? '사은품 상세' : (cfIsNew ? '사은품 등록' : (cfIsView ? '사은품 상세' : '사은품 수정'))"
  ...
  <template #toolbar-actions>
    <button v-if="active ? (cfIsView ? !cfIsNew : false) : false" class="btn btn_link" title="링크 공유(URL만)" @click="handleCopyLink">🔗</button>
    <button v-if="active ? (cfIsView ? !cfIsNew : false) : false" class="btn btn_kakao" title="카카오톡 공유" @click="handleShareKakao">💬</button>
