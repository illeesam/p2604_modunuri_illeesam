...
function gnParseDdl(ddl, dbType /* 'oracle' | 'postgresql' */) {
    ...
    // PRIMARY KEY 추출 — 3가지 표기를 모두 지원한다(ShopJoy 이식분 보강, 2026-08-24).
    //  (1) 테이블 레벨 + 제약명 : CONSTRAINT pk_nm PRIMARY KEY (a, b)
    //  (2) 테이블 레벨 제약명 없음: PRIMARY KEY (a, b)
    //  (3) 컬럼 레벨 인라인      : col VARCHAR(21) NOT NULL CONSTRAINT pk_nm PRIMARY KEY
    // 원본(p2605_sourcegen)은 (1)만 인식해 (3) 스타일 DDL 에서 PK 0개 → Entity 생성 시 크래시했다.
    // ShopJoy 자체 DDL(_doc/ddl_pgsql/**)이 전부 (3) 스타일이라 반드시 필요.
    const pkCols = [];
    const pkTblMatch = body.match(/CONSTRAINT\s+\w+\s+PRIMARY\s+KEY\s*\(([^)]+)\)/i)
                    || body.match(/(?:^|,)\s*PRIMARY\s+KEY\s*\(([^)]+)\)/i);
    if (pkTblMatch) {
        pkTblMatch[1].split(',').forEach(s => pkCols.push(s.trim().toLowerCase()));
    }
    ...
    body.split(/,\s*\n/).forEach(line => {
        ...
        if (/^primary\s+key/i.test(line)) return;   // 테이블 레벨 PK 행은 컬럼이 아님
        ...
        // (3) 컬럼 레벨 인라인 PRIMARY KEY — 이 컬럼 행 자체에 PRIMARY KEY 가 붙어 있는 경우
        if (/\bPRIMARY\s+KEY\b/i.test(line) && !pkCols.includes(name)) pkCols.push(name);
        cols.push({
            name, sqlType, size, nullable,
            javaType: fmJavaType(sqlType),
            javaName: fmSnakeToCamel(name),
            isPk: false,   // pkCols 확정 후 아래에서 일괄 재계산 (인라인 PK 가 뒤늦게 추가될 수 있어서)
            isAudit: ['reg_id', 'reg_dt', 'upd_id', 'upd_dt'].includes(name)
        });
    });
    // pkCols 가 최종 확정된 뒤 isPk 를 다시 계산한다(테이블 레벨 PK 행이 컬럼 뒤에 오는 DDL 대응)
    cols.forEach(c => { c.isPk = pkCols.includes(c.name); });

    if (!cols.some(c => c.isPk)) {
        throw new Error('PRIMARY KEY 를 찾을 수 없습니다 — DDL 에 PK 정의가 있는지 확인해주세요');
    }
