/* manifest.js — "전시관리 > 전시리소스" 마이크로 도메인의 유일한 진입점.
...
(function () {
  ...
  const P = base + 'pages/bo/dp/lib/';

  const scripts = [
    R.loadModule(P + 'DpDispWidgetLibMng.js'),
    R.loadModule(P + 'DpDispWidgetLibDtl.js'),
  ];

  Promise.all(scripts).then(function (results) {
    const screens = [
      { id: 'dp-lib-dpDispWidgetLibMng', label: '전시위젯Lib', group: '전시리소스', comp: results[0].default },
    ];
    const innerComps = [
      { tag: 'DpDispWidgetLibDtl', comp: results[1].default },
    ];
    R.register('bo-ec-dp', screens);
    R.registerComponents(innerComps);
    R._domainReady(base);
  }).catch(function (err) {
    console.error('[bo-ec-dp/lib manifest] 로드 실패:', err);
  });
})();
