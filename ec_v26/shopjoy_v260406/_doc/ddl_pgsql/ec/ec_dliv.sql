...
CREATE TABLE ec_dliv (
    ...
    claim_id        VARCHAR(16),                            -- ec_claim.claim_id (클레임 배송일 때만)
...
COMMENT ON TABLE  ec_dliv                      IS '배송 (1주문 N배송 가능 — 정상출고/반품반입/교환배송)';
...
COMMENT ON COLUMN ec_dliv.claim_id           IS '클레임ID (ec_claim.claim_id, 클레임 배송일 때만)';
...
COMMENT ON COLUMN ec_dliv.dliv_div_cd        IS '입출고구분 (코드: DLIV_DIV — OUTBOUND/INBOUND)';
COMMENT ON COLUMN ec_dliv.dliv_type_cd       IS '배송유형 (코드: DLIV_TYPE — NORMAL/RETURN/EXCHANGE/EXCHANGE_OUT)';
COMMENT ON COLUMN ec_dliv.outbound_courier_cd IS '출고(발송) 택배사 (코드: COURIER)';
COMMENT ON COLUMN ec_dliv.outbound_tracking_no IS '출고(발송) 송장번호';
COMMENT ON COLUMN ec_dliv.inbound_courier_cd IS '반입 택배사 (반품일 때만, 코드: COURIER)';
COMMENT ON COLUMN ec_dliv.inbound_tracking_no IS '반입 송장번호';
COMMENT ON COLUMN ec_dliv.dliv_status_cd     IS '배송상태 (코드: DLIV_STATUS)';
COMMENT ON COLUMN ec_dliv.dliv_status_cd_before IS '변경 전 배송상태 (코드: DLIV_STATUS)';
...
COMMENT ON COLUMN ec_dliv.shipping_fee       IS '배송료';
COMMENT ON COLUMN ec_dliv.shipping_fee_type_cd IS '배송료 구분 (코드: SHIPPING_FEE_TYPE — OUTBOUND/RETURN/INBOUND/EXCHANGE)';
COMMENT ON COLUMN ec_dliv.parent_dliv_id     IS '부모 배송ID (교환 시 원본 배송 참조)';
...
COMMENT ON COLUMN ec_dliv.appr_status_cd_before IS '변경 전 결재상태 (코드: APPROVAL_STATUS)';
