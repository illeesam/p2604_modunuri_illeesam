...
CREATE TABLE shopjoy_2604.pm_event_item (
    event_item_id  VARCHAR(21) NOT NULL CONSTRAINT pm_event_item_pk_event_item_id PRIMARY KEY,
...
COMMENT ON COLUMN shopjoy_2604.pm_event_item.reg_site_id IS '사이트ID (sy_site.site_id)';
...
CREATE INDEX pm_event_item_ix_event ON shopjoy_2604.pm_event_item USING btree (event_id);
CREATE INDEX pm_event_item_ix01_target_type_cd_target_id_x2 ON shopjoy_2604.pm_event_item USING btree (target_type_cd, target_id);
CREATE UNIQUE INDEX pm_event_item_uk_event_id_target_type_cd_x3 ON shopjoy_2604.pm_event_item USING btree (event_id, target_type_cd, target_id);
