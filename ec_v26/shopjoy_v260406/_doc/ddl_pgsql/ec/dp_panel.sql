...
COMMENT ON COLUMN dp_panel.area_cd        IS '영역코드 (코드: DISP_AREA)';
COMMENT ON COLUMN dp_panel.panel_nm       IS '패널명';
COMMENT ON COLUMN dp_panel.widget_type_cd IS '위젯유형 (코드: WIDGET_TYPE)';
COMMENT ON COLUMN dp_panel.panel_type_cd   IS '표시유형 (코드: DISP_TYPE)';
COMMENT ON COLUMN dp_panel.click_action   IS '클릭동작 (LINK/MODAL/NONE)';
COMMENT ON COLUMN dp_panel.click_target   IS '클릭대상 (URL 또는 ID)';
COMMENT ON COLUMN dp_panel.auth_required  IS '[DEPRECATED] 로그인필요 Y/N — visibility_targets의 VERIFIED로 대체';
COMMENT ON COLUMN dp_panel.auth_grade_cd  IS '[DEPRECATED] 노출등급 — visibility_targets의 VIP/PREMIUM으로 대체';
COMMENT ON COLUMN dp_panel.visibility_targets IS '공개대상 (코드: VISIBILITY_TARGET, ^CODE^CODE^ 형식, 하나라도 해당하면 노출)';
COMMENT ON COLUMN dp_panel.sort_ord       IS '정렬순서';
COMMENT ON COLUMN dp_panel.condition_type_cd IS '[DEPRECATED] 노출조건 (코드: DISP_CONDITION_TYPE — ALL/GRADE/LOGIN) — visibility_targets로 대체';
...
COMMENT ON COLUMN dp_panel.content_json   IS '확장데이터 (JSON)';
