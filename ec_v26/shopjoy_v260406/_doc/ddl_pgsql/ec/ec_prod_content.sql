...
CREATE TABLE ec_prod_content (
    prod_content_id VARCHAR(16)     NOT NULL,
    site_id         VARCHAR(16),                            -- sy_site.site_id
    prod_id         VARCHAR(16)     NOT NULL,              -- FK: ec_prod.prod_id
    content_type    VARCHAR(50)     NOT NULL,              -- 상세설명, 사용설명, 배송정보, AS정보, 반품정책 등
...
COMMENT ON TABLE  ec_prod_content              IS '상품 상세 컨텐츠 (HTML 에디터)';
COMMENT ON COLUMN ec_prod_content.prod_content_id IS '상품컨텐츠ID';
COMMENT ON COLUMN ec_prod_content.site_id      IS '사이트ID (sy_site.site_id)';
COMMENT ON COLUMN ec_prod_content.prod_id      IS '상품ID (ec_prod.prod_id)';
COMMENT ON COLUMN ec_prod_content.content_type IS '컨텐츠유형 (상세설명, 사용설명, 배송정보, AS정보, 반품정책 등)';
