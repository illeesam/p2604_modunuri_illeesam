...
COMMENT ON TABLE pd_prod_opt IS '상품 옵션 값 (구조: 옵션그룹→옵션순서→옵션값)';
COMMENT ON COLUMN pd_prod_opt.opt_id         IS '옵션값ID';
COMMENT ON COLUMN pd_prod_opt.site_id        IS '사이트ID (sy_site.site_id)';
COMMENT ON COLUMN pd_prod_opt.opt_grp_id     IS '옵션그룹ID';
COMMENT ON COLUMN pd_prod_opt.prod_id        IS '상품ID';
COMMENT ON COLUMN pd_prod_opt.opt_seq        IS '옵션 순서 (1=색상, 2=사이즈 등)';
COMMENT ON COLUMN pd_prod_opt.opt_type_nm    IS '옵션 종류명 (예: 색상, 사이즈, 무게)';
COMMENT ON COLUMN pd_prod_opt.opt_level      IS '옵션 레벨 (같은 opt_seq 내에서의 계층)';
COMMENT ON COLUMN pd_prod_opt.opt_nm         IS '옵션값명 (예: 빨강, M, XX)';
COMMENT ON COLUMN pd_prod_opt.opt_code       IS '옵션값코드 (예: RED, SIZE_M, SIZE_XX)';
COMMENT ON COLUMN pd_prod_opt.sort_ord       IS '정렬순서';
COMMENT ON COLUMN pd_prod_opt.use_yn         IS '사용여부 Y/N';
COMMENT ON COLUMN pd_prod_opt.reg_by         IS '등록자 (sy_user.user_id, mb_mem.member_id)';
COMMENT ON COLUMN pd_prod_opt.reg_date       IS '등록일';
COMMENT ON COLUMN pd_prod_opt.upd_by         IS '수정자 (sy_user.user_id, mb_mem.member_id)';
