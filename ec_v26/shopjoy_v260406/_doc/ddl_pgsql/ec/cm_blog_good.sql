-- cm_blog_good 테이블 DDL
-- 블로그 좋아요

CREATE TABLE shopjoy_2604.cm_blog_good (
    blog_good_id  VARCHAR(21) NOT NULL CONSTRAINT cm_blog_good_pk_blog_good_id PRIMARY KEY,
    blog_id  VARCHAR(21) NOT NULL,
    user_id  VARCHAR(21) NOT NULL,
    reg_date TIMESTAMP   DEFAULT CURRENT_TIMESTAMP,
    reg_by   VARCHAR(30),
    upd_by   VARCHAR(30),
    upd_date TIMESTAMP  ,
    reg_site_id  VARCHAR(21) NOT NULL,
    CONSTRAINT cm_blog_good_uk_blog_id_user_id_x2 UNIQUE (blog_id, user_id)
);

COMMENT ON TABLE  shopjoy_2604.cm_blog_good IS '블로그 좋아요';
COMMENT ON COLUMN shopjoy_2604.cm_blog_good.blog_good_id IS '좋아요ID';
COMMENT ON COLUMN shopjoy_2604.cm_blog_good.blog_id IS '블로그ID (cm_bltn.)';
COMMENT ON COLUMN shopjoy_2604.cm_blog_good.user_id IS '사용자ID (sy_member.user_id)';
COMMENT ON COLUMN shopjoy_2604.cm_blog_good.reg_date IS '등록일';
CREATE INDEX cm_blog_good_ix01_user_id ON shopjoy_2604.cm_blog_good USING btree (user_id);
