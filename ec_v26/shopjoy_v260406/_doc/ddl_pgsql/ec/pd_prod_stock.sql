...
CREATE TABLE shopjoy_2604.pd_prod_stock (
    ...
    upd_by        VARCHAR(30),
    upd_date      TIMESTAMP,
    CONSTRAINT pd_prod_stock_pk_prod_stock_id PRIMARY KEY (prod_stock_id),
    CONSTRAINT pd_prod_stock_uk_stock_code UNIQUE (stock_code)
);
...
COMMENT ON COLUMN shopjoy_2604.pd_prod_stock.reg_site_id       IS '사이트ID (sy_site.site_id)';
...
CREATE INDEX pd_prod_stock_ix01_prod_id ON shopjoy_2604.pd_prod_stock USING btree (prod_id);
