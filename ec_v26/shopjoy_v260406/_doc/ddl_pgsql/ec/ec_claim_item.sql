...
COMMENT ON COLUMN ec_claim_item.unit_price    IS '판매가 (단가)';
COMMENT ON COLUMN ec_claim_item.claim_qty     IS '클레임 수량';
COMMENT ON COLUMN ec_claim_item.item_amt      IS '클레임금액 (unit_price × claim_qty)';
COMMENT ON COLUMN ec_claim_item.refund_amt    IS '환불금액';
COMMENT ON COLUMN ec_claim_item.claim_item_status_cd IS '항목상태 (코드: CLAIM_ITEM_STATUS)';
COMMENT ON COLUMN ec_claim_item.claim_item_status_cd_before IS '변경 전 클레임상태 (코드: CLAIM_ITEM_STATUS)';
