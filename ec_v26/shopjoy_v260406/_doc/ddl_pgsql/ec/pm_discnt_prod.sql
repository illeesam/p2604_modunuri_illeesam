-- pm_discnt_prod 테이블 DDL
-- 할인 적용 상품 전개 (배치 생성, pm_discnt_item 기반)

CREATE TABLE shopjoy_2604.pm_discnt_prod (
    discnt_prod_id VARCHAR(21)  NOT NULL,
    discnt_id  VARCHAR(21) NOT NULL,
    prod_id    VARCHAR(21) NOT NULL,
    reg_site_id    VARCHAR(21) NOT NULL,
    reg_date   TIMESTAMP   DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT pm_discnt_prod_pk_discnt_prod_id PRIMARY KEY (discnt_prod_id),
    CONSTRAINT pm_discnt_prod_uk_discnt_id_prod_id_x2 UNIQUE (discnt_id, prod_id)
);

COMMENT ON TABLE  shopjoy_2604.pm_discnt_prod IS '할인 적용 상품 전개 (배치 생성)';
COMMENT ON COLUMN shopjoy_2604.pm_discnt_prod.discnt_prod_id IS '할인상품ID (PK)';
COMMENT ON COLUMN shopjoy_2604.pm_discnt_prod.discnt_id IS '할인ID (pm_discnt.discnt_id)';
COMMENT ON COLUMN shopjoy_2604.pm_discnt_prod.prod_id   IS '상품ID (pd_prod.prod_id)';
COMMENT ON COLUMN shopjoy_2604.pm_discnt_prod.reg_site_id   IS '사이트ID (sy_site.site_id)';
COMMENT ON COLUMN shopjoy_2604.pm_discnt_prod.reg_date  IS '배치 생성일시';

CREATE INDEX pm_discnt_prod_ix01_prod_id ON shopjoy_2604.pm_discnt_prod USING btree (prod_id);
CREATE INDEX pm_discnt_prod_ix_discnt ON shopjoy_2604.pm_discnt_prod USING btree (discnt_id);
