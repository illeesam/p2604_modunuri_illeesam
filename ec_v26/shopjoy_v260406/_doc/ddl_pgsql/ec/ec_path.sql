...
-- ============================================================
CREATE TABLE ec_path (
    path_id         BIGSERIAL       NOT NULL,
    biz_cd        VARCHAR(50)     NOT NULL,               -- 업무코드 (테이블명)
    parent_path_id  BIGINT,                                 -- 부모 경로ID (ec_path.path_id, 루트는 NULL)
...
COMMENT ON TABLE  ec_path                IS '경로 (업무별 트리)';
COMMENT ON COLUMN ec_path.path_id        IS '경로ID (PK, auto)';
COMMENT ON COLUMN ec_path.biz_cd       IS '업무코드 (참조 테이블명, 예: sy_brand / sy_code_grp / ec_prop)';
COMMENT ON COLUMN ec_path.parent_path_id IS '부모 경로ID (ec_path.path_id, 루트는 NULL)';
COMMENT ON COLUMN ec_path.path_label     IS '경로 라벨 (한글 표시명)';
COMMENT ON COLUMN ec_path.sort_ord       IS '동일 부모 내 정렬순서';
COMMENT ON COLUMN ec_path.use_yn         IS '사용여부 Y/N';
COMMENT ON COLUMN ec_path.path_remark    IS '비고';
...
CREATE INDEX idx_ec_path_parent ON ec_path (parent_path_id);
