...
CREATE TABLE pd_prod_img (
    ...
    opt_id_1        VARCHAR(16),                            -- 옵션1 값ID (pd_prod_opt., 예: 색상-블랙)
    opt_id_2        VARCHAR(16),                            -- 옵션2 값ID (pd_prod_opt., 예: 사이즈-M)
...
COMMENT ON COLUMN pd_prod_img.opt_id_1    IS '옵션1 값ID (색상 등, NULL이면 공통 이미지)';
COMMENT ON COLUMN pd_prod_img.opt_id_2    IS '옵션2 값ID (사이즈 등, NULL이면 색상 공통)';
