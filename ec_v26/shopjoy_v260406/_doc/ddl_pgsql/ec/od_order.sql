...
COMMENT ON COLUMN od_order.total_amt        IS '상품합계금액';
COMMENT ON COLUMN od_order.discount_amt     IS '할인금액';
COMMENT ON COLUMN od_order.coupon_discount_amt IS '쿠폰할인금액';
COMMENT ON COLUMN od_order.cache_use_amt    IS '적립금사용금액';
COMMENT ON COLUMN od_order.pay_amt          IS '실결제금액';
COMMENT ON COLUMN od_order.pay_method_cd    IS '결제수단 (코드: PAY_METHOD_CD)';
...
COMMENT ON COLUMN od_order.outbound_shipping_fee IS '출고배송료';
...
COMMENT ON COLUMN od_order.dliv_status_cd   IS '배송상태 (코드: DLIV_STATUS, 최신값)';
...
COMMENT ON COLUMN od_order.reg_by           IS '등록자 (sy_user.user_id, mb_mem.member_id)';
COMMENT ON COLUMN od_order.reg_date         IS '등록일';
COMMENT ON COLUMN od_order.upd_by           IS '수정자 (sy_user.user_id, mb_mem.member_id)';
