...
COMMENT ON COLUMN ec_prod.prod_status_cd_before IS '변경 전 상품상태 (코드: PRODUCT_STATUS)';
...
COMMENT ON COLUMN ec_prod.reg_by        IS '등록자 (sy_user.user_id, ec_member.member_id)';
COMMENT ON COLUMN ec_prod.reg_date      IS '등록일';
COMMENT ON COLUMN ec_prod.upd_by        IS '수정자 (sy_user.user_id, ec_member.member_id)';
