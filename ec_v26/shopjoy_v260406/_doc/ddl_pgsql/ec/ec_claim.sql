...
CREATE TABLE ec_claim (
    ...
    inbound_dliv_id      VARCHAR(16),                       -- 반입 배송ID (ec_dliv.dliv_id)
    -- ── 교환 배송정보 ──
    ...
    outbound_dliv_id      VARCHAR(16),                      -- 교환상품 발송 배송ID (ec_dliv.dliv_id)
    -- ── 배송료 정산 ──
...
COMMENT ON COLUMN ec_claim.claim_status_cd IS '클레임상태 (코드: CLAIM_STATUS)';
COMMENT ON COLUMN ec_claim.claim_status_cd_before IS '변경 전 클레임상태 (코드: CLAIM_STATUS)';
...
COMMENT ON COLUMN ec_claim.return_shipping_fee   IS '수거배송료';
COMMENT ON COLUMN ec_claim.return_courier_cd     IS '수거 택배사 (코드: COURIER)';
COMMENT ON COLUMN ec_claim.return_tracking_no    IS '수거 송장번호';
COMMENT ON COLUMN ec_claim.return_status_cd      IS '수거 상태 (코드: DLIV_STATUS)';
COMMENT ON COLUMN ec_claim.return_status_cd_before IS '변경 전 수거상태 (코드: DLIV_STATUS)';
COMMENT ON COLUMN ec_claim.inbound_shipping_fee  IS '반입배송료';
COMMENT ON COLUMN ec_claim.inbound_courier_cd    IS '반입 택배사 (코드: COURIER)';
COMMENT ON COLUMN ec_claim.inbound_tracking_no   IS '반입 송장번호';
COMMENT ON COLUMN ec_claim.inbound_dliv_id       IS '반입 배송ID (ec_dliv.dliv_id)';
COMMENT ON COLUMN ec_claim.exchange_shipping_fee IS '교환상품 발송배송료';
COMMENT ON COLUMN ec_claim.exchange_courier_cd   IS '교환상품 발송 택배사 (코드: COURIER)';
COMMENT ON COLUMN ec_claim.exchange_tracking_no  IS '교환상품 발송 송장번호';
COMMENT ON COLUMN ec_claim.outbound_dliv_id      IS '교환상품 발송 배송ID (ec_dliv.dliv_id)';
COMMENT ON COLUMN ec_claim.total_shipping_fee    IS '총 배송료 (수거+반입+발송)';
COMMENT ON COLUMN ec_claim.shipping_fee_paid_yn  IS '배송료 정산 완료 여부 (Y/N)';
COMMENT ON COLUMN ec_claim.shipping_fee_paid_date IS '배송료 정산일시';
COMMENT ON COLUMN ec_claim.shipping_fee_memo     IS '배송료 비고';
...
COMMENT ON COLUMN ec_claim.appr_status_cd IS '결재상태 (코드: APPROVAL_STATUS)';
COMMENT ON COLUMN ec_claim.appr_status_cd_before IS '변경 전 결재상태 (코드: APPROVAL_STATUS)';
COMMENT ON COLUMN ec_claim.appr_amt       IS '결재 요청금액';
COMMENT ON COLUMN ec_claim.appr_target_cd IS '결재대상 구분 (코드: APPROVAL_TARGET)';
COMMENT ON COLUMN ec_claim.appr_target_nm IS '결재 대상명';
COMMENT ON COLUMN ec_claim.appr_reason    IS '사유/메모';
COMMENT ON COLUMN ec_claim.appr_req_user_id    IS '결재 요청자 (sy_user.user_id)';
COMMENT ON COLUMN ec_claim.appr_req_date  IS '결재 요청일시';
COMMENT ON COLUMN ec_claim.appr_aprv_user_id   IS '결재자 (sy_user.user_id)';
COMMENT ON COLUMN ec_claim.appr_aprv_date IS '결재일시';
