...
COMMENT ON COLUMN sy_vendor.addr           IS '주소';
COMMENT ON COLUMN sy_vendor.addr_detail    IS '상세주소';
COMMENT ON COLUMN sy_vendor.phone          IS '대표 전화';
COMMENT ON COLUMN sy_vendor.fax            IS '팩스';
COMMENT ON COLUMN sy_vendor.email          IS '대표 이메일';
COMMENT ON COLUMN sy_vendor.homepage       IS '홈페이지';
COMMENT ON COLUMN sy_vendor.bank_nm        IS '은행명';
...
CREATE INDEX idx_sy_vendor_status ON sy_vendor (status_cd);
