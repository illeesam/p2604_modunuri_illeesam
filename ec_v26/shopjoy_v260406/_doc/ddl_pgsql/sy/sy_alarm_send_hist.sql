...
COMMENT ON COLUMN sy_alarm_send_hist.channel        IS '발송채널';
COMMENT ON COLUMN sy_alarm_send_hist.send_to        IS '수신처 (이메일/전화/토큰)';
COMMENT ON COLUMN sy_alarm_send_hist.send_date      IS '발송일시';
COMMENT ON COLUMN sy_alarm_send_hist.send_hist_status_cd IS '발송결과 (SENT/FAILED)';
COMMENT ON COLUMN sy_alarm_send_hist.error_msg      IS '오류메시지';
COMMENT ON COLUMN sy_alarm_send_hist.reg_by         IS '등록자 (sy_user.user_id, ec_member.member_id)';
COMMENT ON COLUMN sy_alarm_send_hist.reg_date       IS '등록일';
COMMENT ON COLUMN sy_alarm_send_hist.upd_by         IS '수정자 (sy_user.user_id, ec_member.member_id)';
