...
CREATE TABLE od_pay (
    ...
    upd_date            TIMESTAMP,
    PRIMARY KEY (pay_id),
    CONSTRAINT fk_ec_pay_order FOREIGN KEY (order_id) REFERENCES od_order (order_id)
);
...
COMMENT ON COLUMN od_pay.pay_channel_cd      IS '결제채널 (코드: PAY_CHANNEL — TOSS만 해당: CARD/ACCOUNT/KAKAO/NAVER)';
...
COMMENT ON COLUMN od_pay.vbank_deposit_nm    IS '가상계좌 입금자명 (실제 입금 시 입력)';
