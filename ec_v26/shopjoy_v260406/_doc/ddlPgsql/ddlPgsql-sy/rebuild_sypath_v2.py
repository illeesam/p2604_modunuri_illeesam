"""
sy_path 전체 재구성 v2
- 전시관리(ec_disp_*) 5개: DB의 실제 데이터 기반 재정리
...
def ins(biz_cd, parent_id, label, sort):
    cur.execute(
        "INSERT INTO sy_path (biz_cd, parent_path_id, path_label, sort_ord, use_yn, reg_by, reg_date) "
        "VALUES (%s, %s, %s, %s, 'Y', 'admin', NOW()) RETURNING path_id",
...
try:
    ...
    mob = ins('sy_site', r, 'Mobile', 3)
    ins('sy_site', n, 'PC', 1)
    ins('sy_site', n, 'App', 2)
    ins('sy_site', h, 'English', 1)
    ins('sy_site', mob, 'iOS', 1)
    ins('sy_site', mob, 'Android', 2)
    ...
    print("\n=== 루트(1레벨) — biz_cd == path_label 확인 ===")
    for row in cur.fetchall():
        ok = "OK" if row[1] == row[2] else "MISMATCH"
        print(f"  id={row[0]} biz_cd={row[1]} label={row[2]} [{ok}]")
