...
COMMENT ON COLUMN mbh_member_token_log.login_log_id         IS '최초 로그인 로그ID (mbh_member_login_log.log_id)';
...
COMMENT ON COLUMN mbh_member_token_log.access_token         IS '액세스 토큰 (SHA-256 해시 저장 권장)';
COMMENT ON COLUMN mbh_member_token_log.token_exp            IS '액세스 토큰 만료일시';
COMMENT ON COLUMN mbh_member_token_log.prev_token           IS '갱신 전 액세스 토큰 해시 (REFRESH 액션 시)';
COMMENT ON COLUMN mbh_member_token_log.refresh_token        IS '리프레시 토큰 (SHA-256 해시 저장 권장, 서버 DB에만 보관)';
COMMENT ON COLUMN mbh_member_token_log.ip                   IS 'IP주소';
COMMENT ON COLUMN mbh_member_token_log.device_info          IS 'User-Agent';
COMMENT ON COLUMN mbh_member_token_log.revoke_reason        IS '폐기 사유 (LOGOUT/FORCE/EXPIRED 등)';
COMMENT ON COLUMN mbh_member_token_log.access_token_exp     IS '액세스 토큰 만료일시';
COMMENT ON COLUMN mbh_member_token_log.ui_nm                IS '화면명 (X-UI-Nm 헤더)';
COMMENT ON COLUMN mbh_member_token_log.cmd_nm               IS '기능명 (X-Cmd-Nm 헤더)';
COMMENT ON COLUMN mbh_member_token_log.reg_by               IS '등록자 (mb_member.member_id)';
COMMENT ON COLUMN mbh_member_token_log.reg_date             IS '등록일';
COMMENT ON COLUMN mbh_member_token_log.upd_by               IS '수정자 (mb_member.member_id)';
...
-- 코드값 참조
-- ============================================================
-- [CODES] mbh_member_token_log.action_cd    (토큰 액션) : TOKEN_ACTION { ISSUE:발급, REFRESH:갱신, EXPIRE:만료, REVOKE:폐기, LOGOUT:로그아웃 }
-- [CODES] mbh_member_token_log.token_type_cd (토큰 유형) : TOKEN_TYPE { ACCESS:액세스, REFRESH:리프레시 }
