...
| 코드값 | 라벨 | 설명 |
|--------|------|------|
| ACTIVE | 활성 | 승인 완료, 상품 판매 가능 |
| SUSPENDED | 정지 | 정책 위반·계약 이슈로 일시 정지 |
| TERMINATED | 계약종료 | 계약 종료, 상품 판매 중지 |
...
#### 1-3. 상태별 처리
- **ACTIVE**: 상품 등록·판매 가능, 정산 대상 포함
- **SUSPENDED**: 상품 판매 중단, 기존 주문·배송은 계속 처리, 정산 보류
- **TERMINATED**: 상품 판매 전면 중단, 미정산액 최종 정산 후 계정 비활성화
...
- `TERMINATED` 전환 전 미정산액 처리 완료 확인 필수
