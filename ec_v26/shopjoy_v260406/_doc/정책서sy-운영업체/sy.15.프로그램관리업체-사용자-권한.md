...
| 코드값 | 라벨 | 설명 |
|--------|------|------|
| ACTIVE | 활성 | 계약 유효, 시스템 접근 가능 |
| SUSPENDED | 정지 | 계약 이슈로 일시 접근 차단 |
| TERMINATED | 계약종료 | 계약 종료, 전 시스템 접근 차단 |
...
#### 1-4. 상태별 처리
- **ACTIVE**: 계약 범위 내 시스템 접근 및 작업 가능
- **SUSPENDED**: 전 시스템 접근 즉시 차단, VPN 계정 비활성화
- **TERMINATED**: 전면 차단, 모든 담당자 user_id 연결 해제, 발급된 인증서·토큰 폐기
...
- **계약 종료**: 업체 `TERMINATED` 전환 시 전 담당자 user_id + VPN + API키 일괄 폐기
...
- `TERMINATED` 전환 즉시 VPN·API키·인증서 폐기 필수
