...
| 코드값 | 라벨 | 설명 |
|--------|------|------|
| ACTIVE | 활성 | 승인 완료, CS 업무 수행 가능 |
| SUSPENDED | 정지 | 계약 이슈·서비스 불량으로 일시 정지 |
| TERMINATED | 계약종료 | 계약 종료, 신규 업무 배정 중단 |
...
#### 1-4. 상태별 처리
- **ACTIVE**: CS 업무 수행 가능, 상담사 계정 활성화 상태 유지
- **SUSPENDED**: 신규 업무 배정 중단, 진행 중 상담은 완료까지 처리 또는 이관
- **TERMINATED**: 전면 중단, 모든 상담사 계정 접근 차단, 미처리 문의 이관
...
- `TERMINATED` 전환 전 미처리 문의·클레임 이관 완료 필수
