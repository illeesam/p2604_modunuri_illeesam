...
| 코드값 | 라벨 | 설명 |
|--------|------|------|
| ACTIVE | 활성 | 승인 완료, 사이트 운영 가능 |
| SUSPENDED | 정지 | 계약 이슈로 일시 정지 |
| TERMINATED | 계약종료 | 계약 종료, 운영 권한 회수 |
...
#### 1-4. 상태별 처리
- **ACTIVE**: 사이트 운영 가능, 담당자 계정 활성화 상태 유지
- **SUSPENDED**: 사이트 운영 일시 중단, 플랫폼 관리자가 임시 운영 대행
- **TERMINATED**: 운영 권한 전면 회수, 전 담당자 계정 접근 차단
...
- **계약 종료**: 업체 `TERMINATED` 전환 시 전 담당자 user_id 연결 일괄 해제
...
- `TERMINATED` 전환 전 미처리 주문·클레임 플랫폼 이관 완료 필수
