...
| 이력 기록 | `pd_prod_chg_hist` — 변경 전/후 값, 변경사유, 처리자 |
| SKU 가격 이력 | `pd_prod_opt_sku_chg_hist` — chg_type_cd='PRICE' |
...
### 옵션 있는 상품
```
pd_prod_opt_sku.prod_opt_stock  ← SKU별 재고 단위
...
### 재고 변경 이력
`pd_prod_opt_sku_chg_hist` — chg_type_cd='STOCK':
```
before_val = '50'
after_val  = '30'
chg_reason = '판매'
...
[SKU - 2단 옵션: 향 × 용량]
SKU-001  향=라벤더  용량=50ml   add_price=0      stock=20
SKU-002  향=라벤더  용량=100ml  add_price=5,000  stock=15
SKU-003  향=로즈    용량=50ml   add_price=1,000  stock=8
SKU-004  향=로즈    용량=100ml  add_price=6,000  stock=0  ← 품절
```
...
```
선택: 라벤더 / 100ml
조회: SKU-002 (add_price=5,000, stock=15)
...
SKU-002.prod_opt_stock: 15 → 13 (2개 차감)
pd_prod.prod_stock: 합산 갱신

od_order_item
  sku_id     : SKU-002
  opt_nm     : 라벤더 / 100ml  (스냅샷)
...
### 가격 변경 시나리오
```
관리자: sale_price 25,000 → 22,000 변경
...
신규 주문부터: 22,000 + 5,000 = 27,000원 적용
```
...
## 관련 코드
- `PROD_CHG_TYPE`: PRICE / STOCK / STATUS / CONTENT
- `STOCK_CHG_REASON`: SALE / RETURN / EXCHANGE / ADJUST / INBOUND

## 관련 테이블
- `pd_prod` — 상품 기본 가격·재고 (`sale_price`, `prod_stock`, `sold_out_yn`)
- `pd_prod_opt_sku` — SKU별 추가금액·재고 (`add_price`, `prod_opt_stock`)
- `pd_prod_chg_hist` — 상품 변경 이력 (가격 변경 포함)
- `pd_prod_opt_sku_chg_hist` — SKU 변경 이력 (가격·재고)
- `od_order_item` — 주문 시 unit_price 스냅샷
