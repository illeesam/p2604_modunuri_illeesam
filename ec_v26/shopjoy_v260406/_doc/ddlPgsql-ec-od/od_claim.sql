...
-- [CODES] od_claim.reason_cd (사유코드) : CANCEL_REASON(CANCEL_REASON) { 코드값 미정의 }
-- [CODES] od_claim.refund_method_cd (환불수단) : 환불수단 { CARD_CANCEL:카드취소, BANK_REFUND:계좌환불, CACHE_REFUND:캐쉬환불 }
-- [CODES] od_claim.refund_bank_cd (환불 은행코드) : BANK_CODE(BANK_CODE) { 코드값 미정의 }
...
-- [CODES] od_claim.appr_status_cd (결재상태) : APPROVAL_STATUS(APPROVAL_STATUS) { 코드값 미정의 }
-- [CODES] od_claim.appr_target_cd (결재대상 구분) : APPROVAL_TARGET(APPROVAL_TARGET) { 코드값 미정의 }
