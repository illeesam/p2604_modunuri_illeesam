...
COMMENT ON COLUMN odh_order_status_hist.chg_user_id            IS '변경 담당자 (sy_user.user_id, mb_mem.member_id)';
COMMENT ON COLUMN odh_order_status_hist.chg_date               IS '변경 일시';
COMMENT ON COLUMN odh_order_status_hist.memo                   IS '메모';
COMMENT ON COLUMN odh_order_status_hist.reg_by                 IS '등록자 (sy_user.user_id, mb_mem.member_id)';
COMMENT ON COLUMN odh_order_status_hist.reg_date               IS '등록일';
COMMENT ON COLUMN odh_order_status_hist.upd_by                 IS '수정자 (sy_user.user_id, mb_mem.member_id)';
