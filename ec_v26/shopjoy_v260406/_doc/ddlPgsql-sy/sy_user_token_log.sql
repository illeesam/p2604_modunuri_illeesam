...
COMMENT ON TABLE  sy_user_token_log                 IS '관리자 사용자 토큰 이력';
COMMENT ON COLUMN sy_user_token_log.log_id          IS '로그ID (YYMMDDhhmmss+rand4)';
COMMENT ON COLUMN sy_user_token_log.site_id         IS '사이트ID (sy_site.site_id)';
COMMENT ON COLUMN sy_user_token_log.user_id         IS '사용자ID (sy_user.user_id)';
COMMENT ON COLUMN sy_user_token_log.login_log_id    IS '최초 로그인 로그ID (sy_user_login_log.log_id)';
COMMENT ON COLUMN sy_user_token_log.action_cd       IS '토큰 액션 (코드: TOKEN_ACTION — ISSUE/REFRESH/REVOKE/EXPIRE)';
COMMENT ON COLUMN sy_user_token_log.token_type_cd   IS '토큰 유형 (코드: TOKEN_TYPE — ACCESS/REFRESH)';
COMMENT ON COLUMN sy_user_token_log.token           IS '토큰값 (SHA-256 해시 저장 권장)';
COMMENT ON COLUMN sy_user_token_log.token_exp       IS '토큰 만료일시';
COMMENT ON COLUMN sy_user_token_log.prev_token      IS '갱신 전 토큰 해시 (REFRESH 액션 시)';
...
CREATE INDEX idx_sy_user_token_log_action    ON sy_user_token_log (action_cd);
CREATE INDEX idx_sy_user_token_log_login_log ON sy_user_token_log (login_log_id);
CREATE INDEX idx_sy_user_token_log_date      ON sy_user_token_log (reg_date);
