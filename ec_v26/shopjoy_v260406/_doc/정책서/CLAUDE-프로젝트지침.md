...
| `window.FO_SITE_NO` | `index.html` head 인라인 스크립트 | `'01' \| '02' \| ...` 사이트 번호. `?FO_SITE_NO=02` 쿼리로 오버라이드 후 localStorage(`modu-fo-siteNo`) 저장. (BO 측은 `window.BO_SITE_NO`, bo.html 관리) |
...
| URL 오버라이드 | `?FO_SITE_NO=01\|02\|03` → localStorage(`modu-fo-siteNo`) 저장 후 쿼리 자동 제거 |
