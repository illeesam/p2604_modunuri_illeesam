...
- [FO_SITE_NO 시스템](#fo_site_no-시스템) — 멀티사이트 관리
...
├─ components/modals/FoModals.js
├─ components/disp/DispX01Ui.js ~ DispX04Widget.js
├─ pages/base/foError401.js + foError404.js + foError500.js
├─ pages/fo/Home{NO}.js   Prod{NO}List.js   Prod{NO}View.js   (FO_SITE_NO 기준 document.write 동적 로드)
...
- `#page=prodList` → `window['Prod' + FO_SITE_NO + 'List']`
- `#page=prodView` → `window['Prod' + FO_SITE_NO + 'View']`
...
├─ lib/stores/bo/boAppInitStore.js + boAppStore.js + boAuthStore.js + boCodeStore.js
│                              + boConfigStore.js + boMenuStore.js + boPropStore.js + boRoleStore.js (Pinia)
├─ lib/utils/refDefaults.js + safeArrayUtils.js
├─ components/comp/BaseComp.js + BoComp.js + BoAreaComp.js + CoWidgetComp.js
├─ components/modals/BoModals.js
...
├─ pages/fo/xd/DispUi01.js ~ DispUi06.js + DispUiPage.js
...
## 공통 API 서비스 (`lib/services/`)

**목적**: 페이지 파일 전체에서 사용하는 모든 API 엔드포인트를 `lib/services/` 파일에 등록하여 한 곳에서 관리.  
단일 화면 전용이라도 `lib/services/`에 등록하는 것을 원칙으로 한다.  
...
| 파일 | 전역 객체 | 로드 대상 | 설명 |
|---|---|---|---|
| `lib/services/coApiSvc.js` | `window.coApiSvc` | FO (`index.html`) + BO (`bo.html`) | FO·BO 양쪽에서 사용하는 공통 엔드포인트 |
| `lib/services/foApiSvc.js` | `window.foApiSvc` | FO (`index.html`) | FO 전용 공통 엔드포인트 |
| `lib/services/boApiSvc.js` | `window.boApiSvc` | BO (`bo.html`) | BO 전용 공통 엔드포인트 |
...
```
bo.html:    lib/utils/boApiAxios.js → coUtil.js → boUtil.js → lib/services/coApiSvc.js → boApiSvc.js
index.html: lib/utils/foApiAxios.js → coUtil.js → foUtil.js → lib/services/coApiSvc.js → foApiSvc.js
```
...
각 `lib/services/` 파일 내 네임스페이스 블록은 **가나다(알파벳) 오름차순**으로 정렬한다.
...
const res = await boApiSvc.mbMember.getPage({ searchValue: '홍길동' });
...
1. 해당 `lib/services/` 파일에 도메인 네임스페이스로 추가
...
```js
// lib/services/boApiSvc.js 추가 예시 (pmEvent 다음 위치에 삽입)
boApiSvc.pmPoint = {
...
| 선택 팝업이 필요할 때 | `_doc/정책서/ec/cm/cm.01.공통팝업.md` ⭐ (새 모달 만들지 말고 팝업관리에 등록) |
...
`foApi` / `boApi` 가 `api/` 기준 상대경로로 읽는 정적 JSON.
...
- `api/admin/` - 관리자 API 응답 목업 (폐기됨 — 디렉터리 제거. 관리자 화면은 실 API(`boApiSvc`) 사용)
...
> **처리성 버튼은 항상 confirm 필수** ⭐ (2026-08-14) — `showConfirm(title, msg)` 없이 즉시 실행되는
> [저장]/[삭제]/[업로드]/[일괄처리]/[상태변경] 등 **데이터를 변경하는 처리 버튼**은 정책 위반이다.
> 대상: 저장(POST/PUT), 삭제(DELETE), 엑셀업로드, 일괄 저장/삭제(saveList), 상태 변경, 발송/전송 등
> **데이터를 만들거나 바꾸거나 지우는 모든 버튼**.
> **제외**: 조회(GET, 검색·필터·페이징 이동), 다운로드(엑셀 내보내기 등 읽기 전용 동작), 모달 열기/닫기,
> 미리보기, 탭 전환 — 데이터를 변경하지 않는 동작은 confirm 불필요.
> `handleSave*`/`handleDelete*`/`handleUpload*`/`handleBulk*` 류 함수를 새로 작성하거나 기존 코드를
> 손볼 때는 이 규칙을 항상 확인할 것 — 검증 통과 후 API 호출 직전에 `showConfirm` 가드를 빠뜨리기 쉽다.
...
**계층 (2026-06-11 직접 FK 로 단순화)**: `dp_ui ─< dp_area(ui_id) ─< dp_panel(area_id) ─< dp_panel_item` + `dp_widget_lib ← dp_widget`

- 매핑 테이블 `dp_ui_area` / `dp_area_panel` 은 **폐기** (DROP, 백엔드 코드 삭제) — 계층은 직접 FK 로만 표현
- 패널의 위젯 목록: `dp_panel.content_json = {rows:[...]}` 이 화면 저장 소스. 저장 시 백엔드(DpPanelService.syncPanelItems)가 `dp_panel_item` 으로 관계형 미러 동기화, 조회(getById) 시 `panelItems` 로 반환
- 위젯 유형 27종 (`sy_code DISP_WIDGET_TYPE`, 렌더러 DispX04Widget 과 정합): `image_banner, product_slider, product, cond_product, chart_bar/line/pie, text_banner, info_card, popup, file, file_list, coupon, html_editor, textarea, markdown, barcode, qrcode, barcode_qrcode, video_player, countdown, payment_widget, approval_widget, event_banner, cache_banner, widget_embed, map_widget`
...
- 샘플 데이터: 2604010000000001 단일 — UI 6 / 영역 18 / 패널 36 / 아이템 76 / 위젯Lib 27종×10 / 위젯 인스턴스 270 (`_doc/sample_insert_pgsql/dp_*.sql`, 마이그레이션 `_doc/ddl_pgsql/migration_20260611_dp_restructure.sql`)
...
- `.fo-site-badge` — 헤더 SITE NO 배지 + hover CSS 툴팁
...
| URL 오버라이드 | `?FO_SITE_NO=01\|02\|03` → localStorage(`modu-fo-sy-siteNo`) 저장 후 쿼리 자동 제거 |
...
**URL 페이지 ID는 Generic** (`home`, `prodList`, `prodView`). FO_SITE_NO가 바뀌어도 URL은 동일.

## 모달 디자인 시스템

`components/modals/BoModals.js` / `FoModals.js` 상단에서 전역 `<style>` 주입 → 모든 모달에 자동 적용:
- 오버레이: 반투명 다크네이비 + 블러
...
3-1. **⛔ 신규/수정 컴포넌트 크래시 체크 의무** ⭐ (2026-06-24) — 새 파일을 작성하거나 기존 파일을 대규모 수정한 뒤에는 다음 7개 항목을 반드시 점검하고 결과를 보고한다. 체크 없이 완료 선언 금지.
   1. 속성값에 `&` / `&&` 없는지 (`grep -nE ':[a-zA-Z-]+="[^"]*&'`)
   2. 백틱 template 속성값 안 리터럴 줄바꿈 없는지
   3. `setup()` return 에 템플릿 참조 변수 전부 포함됐는지
   4. `ref()` / `reactive()` 선언이 `watch()` 보다 먼저인지
   5. `v-model` + `:is` 동일 엘리먼트 금지
   6. **외부 라이브러리(ECharts 등) 추가 시** — `window.libName` null 가드, onUnmounted 정리, 로드 순서, 이벤트 콜백 null 가드 (상세 → §0-E)
   7. `ref<Array>` 를 mutate 대신 새 배열 교체하는지 (computed 무한루프 방지)
   - 상세 체크리스트 → `_doc/정책서/base/base.코드스타일-admin-vue.md` §0-E / §0-F
4. **`window.*` 전역 의존이 많음** — 모듈화 이전에 건드릴 때 주입 순서 주의
5. **`adminData` 직접 수정이 소스 오브 트루스** — 목업 변경하면 전 화면 즉시 반영
6. **Dtl 탭 구조 변경 시** 5개 뷰모드 + 영속화 + dtl-tab-card-title 헤더 패턴 유지
...
11. **`/api/base/` 직접 호출 금지** ⭐ — `base/` 패키지 컨트롤러(`/api/base/...`)는 내부 공통 레이어이며 프론트엔드에서 직접 호출하면 안 됨
    - BO 화면은 반드시 `/api/bo/...` 컨트롤러를 통해야 함
    - FO 화면은 반드시 `/api/fo/...` 컨트롤러를 통해야 함
    - 이유: 인증·권한 체크, 감사 로그, 헤더 검증이 bo/fo 레이어에서 수행됨
    - 예시: ❌ `boApi.get('/base/sy/user-token-log/page')` → ✅ `boApi.get('/bo/sy/user-token-log/page')`
    - 새 기능이 필요하면 `bo/` 또는 `fo/` 패키지에 Service + Controller 신규 생성
12. **Controller → base Service 직접 참조 금지** ⭐ — `bo/` · `fo/` · `co/` Controller 는 `base/` Service 를 직접 주입할 수 없다. 반드시 같은 레이어(bo/fo/co) Service 를 경유해야 한다.
    - ✅ 예외: 비즈니스 로직이 전혀 없는 단순 CRUD 위임 시 Controller 에서 base Service 직접 주입 허용
    - ❌ 금지: 권한 체크·DTO 가공·트랜잭션 조합·감사 기록 등이 있는 경우
    - 상세 규칙 → `_doc/정책서/base/base.기술-api.md` §3.5
13. **날짜 컬럼 타입 — DATE vs TIMESTAMP** ⭐ (2026-05-28) — 프론트 입력 방식 기준으로 결정
    - `<input type="date">` (일자만) → DDL `DATE` + Entity `LocalDate`
    - `<input type="datetime-local">` 또는 시스템 자동 기록 → DDL `TIMESTAMP` + Entity `LocalDateTime`
    - ❌ 금지: `TIMESTAMP` + Entity `LocalDateTime` + 프론트 `type="date"` 조합 → Jackson 역직렬화 실패 (`Text '2026-04-29' could not be parsed at index 10`)
    - 컬럼 코멘트 정합성: DATE 컬럼은 "...일", TIMESTAMP 컬럼은 "...일시"
    - LocalDate ↔ TIMESTAMP 는 JPA 자동 변환 가능하므로 Entity 만 먼저 바꿔도 동작 OK. DB 마이그레이션은 운영 시 점진적 적용
    - 상세 → `_doc/정책서/sy/sy.52.ddl단어사전규칙.md` §13-4, `_doc/정책서/base/base.DDL작성규칙.md`
...
- 상세 정책 → `_doc/정책서/base/base.UX-bo.md` §20
...
- 구현 참조: `pages/bo/` 관리자 컴포넌트 (Mng/Dtl/Hist)
