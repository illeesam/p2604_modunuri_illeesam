...
| 토큰 | 저장 위치 | 용도 | 만료 |
|---|---|---|---|
| accessToken | 클라이언트 localStorage (`modu-fo-auth-accessToken`) | API 요청마다 `Authorization: Bearer {token}` 헤더 자동 주입 | 15분 |
| refreshToken | 서버 DB (`mbh_member_token_log.refresh_token`) | accessToken 재발급 용도. 클라이언트 미전달 | 15일 (Sliding) |
| 사용자 정보 | 클라이언트 localStorage (`modu-fo-auth-authUser`) | 로그인한 회원 기본 정보 (JSON) | accessToken과 연동 |

> refreshToken을 클라이언트에 노출하지 않으므로 localStorage 탈취 시에도 세션 연장 불가.

### 2-1. localStorage 키 명명 규칙

모든 클라이언트 저장 키는 **`modu-{컨텍스트}-{도메인}-{항목}`** 형식을 따른다. 전체 도메인 분류·규칙·변경 이력은 [`base.인증-bo.md` §2-1](base.인증-bo.md) 에 단일 정의되어 있다 (BO/FO 공통).

FO 주요 키 예시:

| 도메인 | 키 |
|---|---|
| `auth` | `modu-fo-auth-accessToken`, `modu-fo-auth-authUser`, `modu-fo-auth-refreshToken`, `modu-fo-auth-tempAuthInfo` |
| `sy` | `modu-fo-sy-siteId`, `modu-fo-sy-siteNo`, `modu-fo-sy-theme`, `modu-fo-sy-apiLog`, `modu-fo-sy-apiLogOpen`, `modu-fo-sy-apiLogDock`, `modu-fo-sy-apiToastOpen`, `modu-fo-sy-toast-isShowDetail` |
| `od` / `pd` | `modu-fo-od-cart` (장바구니), `modu-fo-pd-like` (찜) |
| `xdev` | `modu-fo-xdev-sample07_v2` (샘플 페이지 — 운영 무관) |
...
4. 클라이언트 저장:
     modu-fo-auth-accessToken = accessToken
     modu-fo-auth-authUser    = JSON.stringify(authUser)
...
API 요청
  ...
  → 성공: 신규 accessToken → localStorage modu-fo-auth-accessToken 갱신
          → 실패했던 원 요청 자동 재시도
  → 실패: modu-fo-auth-accessToken / modu-fo-auth-authUser 삭제
          → foAuth.logout() 호출
...
state: () => {
  const token = localStorage.getItem('modu-fo-auth-accessToken') || null;
  let authUser = null;
  if (token) {
    authUser = JSON.parse(localStorage.getItem('modu-fo-auth-authUser') || 'null');
  }
...
단 BO와 달리 FO는 **비로그인이 정상 케이스**이므로, `prevAuthId === ''`(비로그인 둘러보기 중 다른 탭에서 로그인) 인 경우는 reload 하지 않는다 → 둘러보던 화면과 **장바구니(`modu-fo-od-cart`)를 보존**한다.
...
| 장바구니 담기 / 수정 / 삭제 | **허용 (비로그인 자유)** — `localStorage('modu-fo-od-cart')` 브라우저 기준, 로그인 무관. 결제 시점에 로그인 |
| 찜 버튼 (♡) | 로그인 모달 표시 |
...
> **장바구니 정책**: 회원별 분리 없이 브라우저 단일 키(`modu-fo-od-cart`). 비로그인으로 담아두고 로그인 후 결제하는 게스트 장바구니 흐름. 교차탭 reload 시에도 별도 키라 보존됨. 로그아웃해도 장바구니 유지(`onLogout` 은 장바구니 미변경, 마이페이지에 있으면 `home` 으로 이동).
