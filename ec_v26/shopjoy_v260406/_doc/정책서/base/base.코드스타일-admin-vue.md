...
setup() {
  ...
  // ===== 초기 함수 =====
  const api = window.axiosApi;           // ❌ 변수가 초기 함수 영역에
...
```js
// ❌ 금지 — 클라이언트 호환 fallback 변수
const api = window.axiosApi || window.adminApi;     // 폐기

// ❌ 금지 — URL 을 변수로 정의
const API = 'api/base/sy/zz-sample1';
await api.get(API, { cdGrp });
await api.post(`${API}/${id}`, body);
