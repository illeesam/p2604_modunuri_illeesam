...
## 16. BO 모달 표준 ⭐⭐ (2026-05-28)

### 16.1 모달 루트는 `<bo-modal>` 필수

모든 BO 모달의 template 루트는 `<bo-modal :show="..." @close="...">` 로 시작해야 한다.
페이지 컴포넌트(Mng/Dtl) 안에 인라인으로 정의하는 모달도 동일.
...
<!-- 수기 modal-overlay / modal-box 마크업 — 금지 -->
...
```html
<bo-modal :show="uiState.pickerOpen" title="구성품 상품 선택" width="640px"
          @close="handleBtnAction('prodPickModal-close')">
  <!-- 검색 영역 -->
  <bo-search-area :columns="pickerSearchColumns" :param="pickerParam"
                  @search="handleBtnAction('prodPickModal-search')" />
  <!-- 그리드 -->
  <bo-grid :columns="pickerGridColumns" :rows="pickerRows" />
  <!-- 페이지네이션 -->
  <bo-pagination :pager="pickerPager" :total="pickerTotal"
                 @change="n => handleSelectAction('prodPickModal-page', n)" />
  <template #footer>
    <button class="btn btn-secondary" @click="handleBtnAction('prodPickModal-close')">닫기</button>
  </template>
</bo-modal>
```
...
핑크 그라데이션 기본 헤더가 아닌 자체 헤더가 필요한 경우(예: AuthUserPickModal 의 부제 표시):
`body-pad="0" box-pad="0"` 로 bo-modal 기본 헤더를 비우고 슬롯 내부에 자체 헤더 그리기.
이 패턴은 허용되나 첫 자식은 여전히 `<bo-modal>` 이어야 한다.

#### 점검 명령

```bash
# pages/bo 안의 인라인 오버레이 모달 잔존 확인
grep -rn "position:\s*fixed.*z-index" pages/bo
# components/modals 안 — 신규 모달은 BoModals.js 에 bo-modal 루트로 작성
```

### 16.2 모달 그리드 페이징 ⭐ (2026-05-28)

모달 내부 그리드도 **서버사이드 페이징 필수**. 이전에 예외로 두었던
"모달 picker 는 클라이언트 슬라이싱 허용" 규칙은 **폐기**됨.

#### 적용 규칙

- 모달 내부에 `pickerPager = reactive({ pageNo:1, pageSize:10 })` 정의
- 검색·페이지 변경 시 API 호출 → 응답의 `pageList` / `pageTotalCount` 사용
- 모달 풋터 또는 그리드 직하단에 `<bo-pagination>` 배치
- 모달 닫을 때 `pickerPager.pageNo = 1` 리셋(선택)

#### 예외 (트리형 모달)

계층 구조 일괄 표시가 본질인 트리 모달은 페이징 없음:
- `MenuTreeModal`, `DeptTreeModal`, `RoleTreeModal`
- `CategoryTreeModal`, `PathPickModal`, `BoCodeGrpModal`
...
setup(props, { emit }) {
  ...
  const pickerParam = reactive({ searchValue: '', searchType: '' });
  const pickerPager = reactive({ pageNo: 1, pageSize: 10 });
  const pickerRows  = ref([]);
  const pickerTotal = ref(0);
  const loading     = ref(false);

  /* [04] 조회 함수 */
  const handleSearchPicker = async () => {
    loading.value = true;
    try {
      const params = { ...pickerParam, pageNo: pickerPager.pageNo, pageSize: pickerPager.pageSize };
      const res = await boApiSvc.pdProd.getPage(params, '구성품선택', '조회');
      const d = res.data?.data || {};
      pickerRows.value  = d.pageList       || [];
      pickerTotal.value = d.pageTotalCount || 0;
    } finally { loading.value = false; }
  };

  /* [05] dispatch */
  const handleBtnAction = (cmd, p) => {
    if (cmd === 'prodPickModal-search') { pickerPager.pageNo = 1; return handleSearchPicker(); }
    if (cmd === 'prodPickModal-close')  { return emit('close'); }
  };
  const handleSelectAction = (cmd, p) => {
    if (cmd === 'prodPickModal-page') { pickerPager.pageNo = p; return handleSearchPicker(); }
  };

  return { pickerParam, pickerPager, pickerRows, pickerTotal, loading,
           handleBtnAction, handleSelectAction };
}
```
...
- CSS 는 BO 사이트 테마 무관 동일 적용 (세 파일 모두 동일 블록)
- 신규 모달 클래스(`.modal-overlay-xxx`)는 같은 패턴으로 CSS 파일에 추가 (JS 인라인 금지)
...
- **`pages/bo` 인라인 오버레이 → `<bo-modal>` 루트 + `<bo-grid :pager>` 변환 완료 (3건)**:
  - [PdBundleMng.js](../../../pages/bo/ec/pd/PdBundleMng.js) — 구성품 상품 선택
  - [PdSetMng.js](../../../pages/bo/ec/pd/PdSetMng.js) — 구성품 상품 선택
  - [PdCategoryProdMng.js](../../../pages/bo/ec/pd/PdCategoryProdMng.js) — 상품 추가
- **[BoModals.js](../../../components/modals/BoModals.js) 일괄 보강**:
  - `bo-grid bare :pager` 8건 → `bo-grid :pager` (bare 제거 → 페이저 자동 노출). 영향: SiteSelectModal, VendorSelectModal, BoUserSelectModal, MemberSelectModal, OrderSelectModal, BbmSelectModal, CategorySelectModal, OdMemberPickModal, AuthUserPickModal
  - 모든 pager `reactive({...})` 객체에 `pageNums: [1]` + `pageSizes: [5,10,20,30,50]` 자동 보강
  - 누락된 `@size-change` 핸들러 자동 추가 (페이지사이즈 변경 시 1페이지로 리셋)
- **행번호 자동 표시** — `<bo-grid>` 기본 `showRowNo: true` 라 별도 작업 없이 페이징 그리드에 행번호 노출

#### 잔여 대상 (다음 작업)
- **인라인 오버레이 잔존 (비그리드 모달)** — `<bo-modal>` 루트로 통일 필요, 페이징은 무관:
  - OdOrderMng / OdClaimMng / OdDlivMng — "변경작업" 폼 모달
  - DpDispPanelDtl / DpDispPanelMng / DpDispUiSimul — 미리보기 / 시뮬레이션
  - PdProdDtl, PdReviewMng, PdCategoryMng, SyTemplateMng, SyPostman
- **부모가 전체 배열 prop 으로 넘기는 모달** — API 직접 호출 + 페이징으로 전환 필요:
  - SimpleProdPickModal, SimpleUserPickModal, SimpleVendorPickModal
  - 호출처(Pm Dtl 10개 + Od* Mng 4개 등) 도 함께 변경 필요 → 영향 범위 크므로 단계적 진행
- **트리형 모달** (예외, 페이징 없음): MenuTreeModal, DeptTreeModal, RoleTreeModal, CategoryTreeModal, PathPickModal, BoCodeGrpModal, BoCategoryTree
