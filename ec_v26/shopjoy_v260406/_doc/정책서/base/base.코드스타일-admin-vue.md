...
- **블럭 주석 길이 통일**: 들여쓰기 제외 마커 총 길이 **약 75자**로 맞춤. 라벨이 짧을수록 꼬리 `=` 가 길어진다. 가독성·정렬감 확보.
- **카드 내부 소구역** (툴바·테이블·페이징·탭 내부)도 들여쓰기 유지 + 같은 형식.
...
- 자동 추가 403건 (공통 컴포넌트 직전 표준 라벨).
- 길이 패딩 168개 파일 적용 완료. 잔존 구식 0건. SYNTAX FAIL 0.
- 최종 신식 마커 수: **2069건**.
