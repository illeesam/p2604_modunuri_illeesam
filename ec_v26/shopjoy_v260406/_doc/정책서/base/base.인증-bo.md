...
| 토큰 | 저장 위치 | 용도 | 만료 |
|---|---|---|---|
| accessToken | 클라이언트 localStorage (`modu-bo-auth-accessToken`) | API 요청마다 `Authorization: Bearer {token}` 헤더 자동 주입 | 15분 |
| refreshToken | 서버 DB (`syh_user_token_log.refresh_token`) | accessToken 재발급 용도. 클라이언트 미전달 | 2시간 (Sliding) |
| 사용자 정보 | 클라이언트 localStorage (`modu-bo-auth-authUser`) | 로그인한 관리자 기본 정보 (JSON) | accessToken과 연동 |

> refreshToken을 클라이언트에 노출하지 않으므로 localStorage 탈취 시에도 세션 연장 불가.

### 2-1. localStorage 키 명명 규칙 ⭐ (2026-06-13)

모든 클라이언트 저장 키는 **`modu-{컨텍스트}-{도메인}-{항목}`** 형식으로 통일한다.

- **컨텍스트**: `bo`(관리자) / `fo`(사용자). 컨텍스트가 없는 전역 항목은 `modu-sy-{항목}` (예: `modu-sy-license-no`).
- **도메인 세그먼트** (필수): 키의 성격을 나타내는 구분자.

| 도메인 | 의미 | 키 예시 |
|---|---|---|
| `auth` | 인증·세션 (토큰/사용자/만료) | `modu-bo-auth-accessToken`, `modu-bo-auth-refreshToken`, `modu-bo-auth-authUser`, `modu-bo-auth-accessExpiresIn`, `modu-bo-auth-refreshExpiresIn`, `modu-bo-auth-tempAuthInfo` (fo 동일) |
| `sy` | 시스템·설정 (사이트/API로그/테마/토스트) | `modu-bo-sy-siteId`, `modu-bo-sy-siteNo`, `modu-bo-sy-apiLog`, `modu-bo-sy-apiToastOpen`, `modu-bo-sy-toast-isShowDetail`, `modu-fo-sy-theme`, `modu-fo-sy-apiLogDock`, `modu-fo-sy-apiLogOpen` |
| `od` | 주문 (FO 장바구니) | `modu-fo-od-cart` |
| `pd` | 상품 (FO 찜) | `modu-fo-pd-like` |
| `xdev` | 개발자 도구 (운영 무관, BO API 테스터·FO 샘플 등) | `modu-bo-xdev-postman_v1`, `modu-fo-xdev-sample07_v2` |

**규칙 세부**:
- `siteId`/`siteNo` 는 **카멜케이스로 통일** (구 `site_id`/`site_no` 언더스코어 표기 폐기).
- 인증 외 모든 항목은 `sy` 로 분류한다 (설정/토스트/API로그/테마 등).
- 키 문자열은 변수·상수로 추상화하지 않고 **호출부에 실제 리터럴로 직접 기입**한다 (`localStorage.setItem('modu-bo-auth-accessToken', ...)`). 한 곳에서만 쓰는 상수(`ACCESS_TOKEN_KEY`, `TOAST_DETAIL_KEY` 등)는 두지 않는다.
- 변경 이력: 구 키(`modu-bo-accessToken`, `modu-fo-authUser`, `shopjoy_cart`, `shopjoy_likes`, `modu-bo-setting-*`, `modu-license-no`, 개발도구 `sj_admin_postman_v1`·`sj_sample07_v2` 등)는 2026-06-13 위 체계로 전면 개편됨. (코드·HTML 진입점·정책서 일괄 반영)
...
4. 클라이언트 저장:
     modu-bo-auth-accessToken = accessToken
     modu-bo-auth-authUser    = JSON.stringify(authUser)
...
API 요청
  ...
  → 성공: 신규 accessToken → localStorage modu-bo-auth-accessToken 갱신
          → 실패했던 원 요청 자동 재시도
  → 실패: modu-bo-auth-accessToken / modu-bo-auth-authUser 삭제
