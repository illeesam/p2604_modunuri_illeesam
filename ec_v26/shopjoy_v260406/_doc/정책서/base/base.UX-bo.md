...
- lib/app/boAppBase.js 가 이미 `.admin-wrap` 을 적용하므로 **컴포넌트 루트에 재사용 금지**
...
### 19-A. 목록 그리드 높이 — 아래에 다른 영역이 있으면 20행으로 제한 ⭐ (2026-08-15)

**규칙**: Mng 화면의 목록 그리드 **아래에 인라인 상세·폼 등 다른 영역이 있으면**
그리드에 `table-max-height="540px"` 를 준다.

| 항목 | 값 |
|---|---|
| 기준 | **20행** 정도가 보이는 높이 |
| 계산 | `.bo-table td` 24px × 20행 + 헤더 30px ≈ **540px** |
| 지정 | `<bo-grid ... table-max-height="540px">` |

- **왜**: 페이지 크기를 100·500 으로 올리면 목록이 화면을 통째로 밀어내
  아래 상세 영역까지 스크롤로 한참 내려가야 한다. 목록은 자체 스크롤을 갖고
  상세는 항상 손 닿는 곳에 있어야 한다
- `table-max-height` 를 주면 BoGrid 가 자체 스크롤 컨테이너를 만들고
  **헤더는 sticky 로 고정**된다 (§그리드 헤더 고정 정책과 정합)
- 목록만 있고 아래에 아무것도 없는 화면(전체화면 목록)은 **지정하지 않는다** — 화면 끝까지 쓰는 게 낫다
- 적용(2026-08-15): 하단 상세영역을 가진 BO Mng **33개** 전수 적용

```html
<bo-grid :columns="columns.baseGrid" :rows="rows" :pager="baseGridPager"
  table-max-height="540px">   <!-- 아래에 인라인 Dtl 이 있으므로 20행으로 제한 -->
```
