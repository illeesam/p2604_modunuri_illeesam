...
| **QueryDSL `Q*RepositoryImpl` 표준** ⭐ | `base/base.backend-EcAdminApi.md` §14.6 — 6개 섹션 구분, `buildCondition` 금지, 개별 `andXxx()` + `.where(...)` 직접 나열, `QSyUserRepositoryImpl` 표준 모델. §14.6.5 `QVwSyCode` alias는 매칭 필드명 기준(`codeDlivCourierCd`) 명명, §14.6.6 `searchType` 예 주석(최대 5개), §14.6.7 SELECT/WHERE/`FieldDef` 한글 주석은 DTO 필드 주석 재사용 |
