...
| **QueryDSL `Q*RepositoryImpl` 표준** ⭐ | `base/base.backend-EcAdminApi.md` §14.6 — 6개 섹션 구분, `buildCondition` 금지, 개별 `andXxx()` + `.where(...)` 직접 나열, `QSyUserRepositoryImpl` 표준 모델. §14.6.5 `QVwSyCode` alias는 매칭 필드명 기준(`codeDlivCourierCd`) 명명, §14.6.6 `searchType` 예 주석(최대 5개), §14.6.7 SELECT/WHERE/`FieldDef` 한글 주석은 DTO 필드 주석 재사용, §14.6.8 기본은 `selectList`/`selectPageData`/`updateSelective` 3개로 시작하고 UNIQUE 좌표조회·화면전용 서브쿼리·계산식 UPDATE 만 별도 메서드로 추가, **§14.6.9 `findById`/`findAllById` 외 모든 파생쿼리(find*, Service 안 raw EntityManager JPQL 포함)는 QueryDSL 로 — 재귀CTE·Q클래스 미생성 엔티티·런타임 동적테이블·무거운 분석 native SQL 4가지만 예외** ⭐ |
