...
### 1-A. 전시 공개 상태 — `disp_status_cd` (UI / Area / Panel / Widget 공통)
UI·Area·Panel·Widget 4개 계층이 공통으로 사용하는 노출 제어 코드.
실제 노출은 모든 상위 계층이 ACTIVE여야만 성립하며, 하나라도 INACTIVE이면 숨김 처리된다.
...
### 1-B. 위젯 유형 — `disp_widget.widget_type_cd`
Panel 내 rows[]에 배치되는 위젯의 종류를 지정. 유형에 따라 속성 스키마가 달라진다.
...
### 1-C. 전시 영역 코드 — `disp_area.disp_area_cd` (DISP_AREA 공통코드)
`adminData.codes`에서 `codeGrp === 'DISP_AREA'`로 관리되는 공통코드 기반 영역 코드.
영역 코드는 추가·변경 가능하며, 아래는 기본 제공 예시.
...
UI → Area → Panel → Widget 순으로 계층이 구성되며, 각 계층은 동일한 `disp_status_cd`를 사용.
상위 계층이 INACTIVE면 하위 계층 상태와 무관하게 노출 안 됨.
...
상품·쿠폰·이벤트 연결 위젯은 해당 도메인 마스터 ID를 속성으로 참조.
