...
window.useFoMyStore = Pinia.defineStore('foMy', () => {
  ...
  const handleLoadOrders = async () => {
    if (!Array.isArray(orders) || !orders.length) {
      ...
      try { const res = await window.foApi.get('/fo/my/order/list', { headers: { 'X-UI-Nm': '마이주문', 'X-Cmd-Nm': '목록조회' } }); orders.splice(0, orders.length, ...(res.data?.data || [])); }
  ...
  const handleLoadClaims = async () => {
    if (!Array.isArray(claims) || !claims.length) {
      ...
      try { const res = await window.foApi.get('/fo/my/claim/list', { headers: { 'X-UI-Nm': '마이클레임', 'X-Cmd-Nm': '목록조회' } }); claims.splice(0, claims.length, ...(res.data?.data || [])); }
  ...
  const handleLoadCoupons = async () => {
    if (!Array.isArray(coupons) || !coupons.length) {
      ...
      try { const res = await window.foApi.get('/fo/my/coupon/list', { headers: { 'X-UI-Nm': '마이쿠폰', 'X-Cmd-Nm': '목록조회' } }); coupons.splice(0, coupons.length, ...(res.data?.data || [])); }
  ...
  const handleLoadCash = async () => {
    if (!Array.isArray(cashHistory) || !cashHistory.length) {
      ...
      try {
        const res = await window.foApi.get('/fo/my/cash/info', { headers: { 'X-UI-Nm': '마이캐시', 'X-Cmd-Nm': '조회' } });
  ...
  const loadInquiries = async () => {
    if (!Array.isArray(inquiries) || !inquiries.length) {
      ...
      try { const res = await window.foApi.get('/fo/my/inquiry/list', { headers: { 'X-UI-Nm': '마이문의', 'X-Cmd-Nm': '목록조회' } }); inquiries.splice(0, inquiries.length, ...(res.data?.data || [])); }
  ...
  const loadChats = async () => {
    if (!Array.isArray(chats) || !chats.length) {
      ...
      try { const res = await window.foApi.get('/fo/my/chat/list', { headers: { 'X-UI-Nm': '마이채팅', 'X-Cmd-Nm': '목록조회' } }); chats.splice(0, chats.length, ...(res.data?.data || [])); }
