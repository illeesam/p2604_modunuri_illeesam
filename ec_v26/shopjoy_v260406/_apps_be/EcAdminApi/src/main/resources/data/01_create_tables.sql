...
-- pd_prod_opt: PO001=블랙, PO002=화이트, PO003=딥네이비
...
CREATE TABLE IF NOT EXISTS pd_prod_opt_type (
    prod_opt_type_id        VARCHAR(21)     NOT NULL,
    site_id                 VARCHAR(21),                            -- sy_site.site_id
    prod_id                 VARCHAR(21)     NOT NULL,
    prod_opt_type_nm        VARCHAR(50)     NOT NULL,               -- 예: 색상, 사이즈
    prod_opt_type_level     INTEGER         NOT NULL DEFAULT 1,     -- 옵션 축 순서 (1=첫번째, 2=두번째)
    prod_opt_type_level1_cd VARCHAR(20),                            -- 1단 분류 코드 (PROD_OPT_CATEGORY level=1)
    prod_opt_type_level2_cd VARCHAR(20),                            -- 2단 분류 코드 (NULL 가능)
    sort_ord                INTEGER         DEFAULT 0,
    reg_by                  VARCHAR(16),
    reg_date                TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
    upd_by                  VARCHAR(16),
    upd_date                TIMESTAMP,
    PRIMARY KEY (prod_opt_type_id)
);

COMMENT ON TABLE pd_prod_opt_type IS '상품 옵션 유형 (색상, 사이즈 등 옵션 축) — 구 테이블명: pd_prod_opt';
COMMENT ON COLUMN pd_prod_opt_type.prod_opt_type_id      IS '옵션유형ID (YYMMDDhhmmss+rand4)';
COMMENT ON COLUMN pd_prod_opt_type.site_id               IS '사이트ID (sy_site.site_id)';
COMMENT ON COLUMN pd_prod_opt_type.prod_id               IS '상품ID (pd_prod.prod_id)';
COMMENT ON COLUMN pd_prod_opt_type.prod_opt_type_nm      IS '옵션유형명 (예: 색상, 사이즈)';
COMMENT ON COLUMN pd_prod_opt_type.prod_opt_type_level   IS '옵션단계 — 1=첫번째(색상), 2=두번째(사이즈)';
COMMENT ON COLUMN pd_prod_opt_type.prod_opt_type_level1_cd IS '이 차원이 속한 1단 분류 코드 (코드: PROD_OPT_CATEGORY level=1 — COLOR/SIZE 등)';
COMMENT ON COLUMN pd_prod_opt_type.prod_opt_type_level2_cd IS '이 차원이 속한 2단 분류 코드 (NULL 가능)';
COMMENT ON COLUMN pd_prod_opt_type.sort_ord              IS '정렬순서';
COMMENT ON COLUMN pd_prod_opt_type.reg_by                IS '등록자 (sy_user.user_id, mb_member.member_id)';
COMMENT ON COLUMN pd_prod_opt_type.reg_date              IS '등록일시';
COMMENT ON COLUMN pd_prod_opt_type.upd_by                IS '수정자 (sy_user.user_id, mb_member.member_id)';
COMMENT ON COLUMN pd_prod_opt_type.upd_date              IS '수정일시';

CREATE INDEX IF NOT EXISTS idx_pd_prod_opt_type_prod ON pd_prod_opt_type (prod_id);
CREATE INDEX IF NOT EXISTS idx_pd_prod_opt_type_site ON pd_prod_opt_type (site_id);

-- pd_prod_opt: 상품 옵션 값 (빨강, M 등) — 구 테이블명: pd_prod_opt_item (2026-07-12 rename)
-- 2026-07-13: prod_opt_val_code_id 제거, opt_style → prod_opt_style, prod_opt_type_level1_cd / prod_opt_type_level2_cd 추가

-- ============================================================
-- 데이터 예제 (상품 P001 — 티셔츠, 색상+사이즈 2단 옵션)
-- ============================================================
-- prod_id='P001'
--   ├─ prod_opt_type_id='OT001', prod_opt_type_level=1, prod_opt_type_nm='색상',   prod_opt_type_level1_cd='COLOR'
--   └─ prod_opt_type_id='OT002', prod_opt_type_level=2, prod_opt_type_nm='사이즈', prod_opt_type_level1_cd='COLOR', prod_opt_type_level2_cd='SIZE'

CREATE TABLE IF NOT EXISTS pd_prod_opt (
    prod_opt_id             VARCHAR(21)     NOT NULL,
    site_id                 VARCHAR(21),                            -- sy_site.site_id
    prod_opt_type_id        VARCHAR(21)     NOT NULL,               -- pd_prod_opt_type.prod_opt_type_id
    prod_id                 VARCHAR(21)     NOT NULL,               -- pd_prod.prod_id
    prod_opt_nm             VARCHAR(100)    NOT NULL,               -- 옵션항목명 (예: 빨강, M)
    prod_opt_val            VARCHAR(50),                            -- 실제 저장값 (직접입력 또는 코드값 자동 채움)
    prod_opt_type_level1_cd VARCHAR(20),                            -- 1단 분류 코드 비정규화 (pd_prod 기준)
    prod_opt_type_level2_cd VARCHAR(20),                            -- 2단 분류 코드 비정규화 (NULL 가능)
    parent_prod_opt_id      VARCHAR(21),                            -- 상위 옵션값ID (2단 옵션에서 1단 값 참조, pd_prod_opt.prod_opt_id)
    prod_opt_style          VARCHAR(200),                           -- 항목 스타일 (컬러 hex, 아이콘 클래스 등)
    sort_ord                INTEGER         DEFAULT 0,
    use_yn                  CHAR(1)         DEFAULT 'Y',
    reg_by                  VARCHAR(16),
    reg_date                TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
    upd_by                  VARCHAR(16),
    upd_date                TIMESTAMP,
    PRIMARY KEY (prod_opt_id)
);

COMMENT ON TABLE pd_prod_opt IS '상품 옵션 값 (빨강/M 등 항목) — 구 테이블명: pd_prod_opt_item';
COMMENT ON COLUMN pd_prod_opt.prod_opt_id             IS '옵션값ID (YYMMDDhhmmss+rand4)';
COMMENT ON COLUMN pd_prod_opt.site_id                 IS '사이트ID (sy_site.site_id)';
COMMENT ON COLUMN pd_prod_opt.prod_opt_type_id        IS '옵션유형ID (pd_prod_opt_type.prod_opt_type_id)';
COMMENT ON COLUMN pd_prod_opt.prod_id                 IS '상품ID (pd_prod.prod_id)';
COMMENT ON COLUMN pd_prod_opt.prod_opt_nm             IS '옵션항목명 (예: 빨강, M)';
COMMENT ON COLUMN pd_prod_opt.prod_opt_val            IS '실제 저장값 — 직접입력 또는 prod_opt_type_level2_cd 유형의 코드값 자동 채움';
COMMENT ON COLUMN pd_prod_opt.prod_opt_type_level1_cd IS '1단 분류 코드 — pd_prod.prod_opt_type_level1_cd 비정규화 (COLOR/SIZE 등)';
COMMENT ON COLUMN pd_prod_opt.prod_opt_type_level2_cd IS '2단 분류 코드 — pd_prod_opt_type.prod_opt_type_level2_cd 비정규화 (NULL 가능)';
COMMENT ON COLUMN pd_prod_opt.parent_prod_opt_id      IS '상위 옵션값ID — 2단 옵션에서 상위 1단 옵션값 참조 (pd_prod_opt.prod_opt_id), NULL이면 독립값(전체 공통)';
COMMENT ON COLUMN pd_prod_opt.prod_opt_style          IS '항목 스타일 (컬러 hex 값, 아이콘 클래스 등 자유 문자열)';
COMMENT ON COLUMN pd_prod_opt.sort_ord                IS '정렬순서';
COMMENT ON COLUMN pd_prod_opt.use_yn                  IS '사용여부 Y/N';
COMMENT ON COLUMN pd_prod_opt.reg_by                  IS '등록자 (sy_user.user_id, mb_member.member_id)';
COMMENT ON COLUMN pd_prod_opt.reg_date                IS '등록일시';
COMMENT ON COLUMN pd_prod_opt.upd_by                  IS '수정자 (sy_user.user_id, mb_member.member_id)';
COMMENT ON COLUMN pd_prod_opt.upd_date                IS '수정일시';

CREATE INDEX IF NOT EXISTS idx_pd_prod_opt_type  ON pd_prod_opt (prod_opt_type_id);
CREATE INDEX IF NOT EXISTS idx_pd_prod_opt_parent ON pd_prod_opt (parent_prod_opt_id);
CREATE INDEX IF NOT EXISTS idx_pd_prod_opt_prod  ON pd_prod_opt (prod_id);

-- ============================================================
-- 데이터 예제 (상품 P001 — 티셔츠, 색상+사이즈 2단 옵션)
-- pd_prod_opt_type: OT001=색상(level=1), OT002=사이즈(level=2)
-- ============================================================
-- prod_id='P001'
--   ├─ prod_opt_type_id='OT001' (level=1, level1_cd='COLOR')
--   │   ├─ prod_opt_id='PO001', prod_opt_nm='블랙',     prod_opt_val='BLACK', prod_opt_type_level1_cd='COLOR', parent_prod_opt_id=NULL
--   │   ├─ prod_opt_id='PO002', prod_opt_nm='화이트',   prod_opt_val='WHITE', prod_opt_type_level1_cd='COLOR', parent_prod_opt_id=NULL
--   │   └─ prod_opt_id='PO003', prod_opt_nm='딥네이비', prod_opt_val='NAVY',  prod_opt_type_level1_cd='COLOR', parent_prod_opt_id=NULL
--   └─ prod_opt_type_id='OT002' (level=2, level1_cd='COLOR', level2_cd='SIZE')
--       ├─ prod_opt_id='PO004', prod_opt_nm='S', prod_opt_val='S', parent_prod_opt_id=NULL  → 전체 색상 공통
--       ├─ prod_opt_id='PO005', prod_opt_nm='M', prod_opt_val='M', parent_prod_opt_id=NULL  → 전체 색상 공통
--       └─ prod_opt_id='PO006', prod_opt_nm='L', prod_opt_val='L', parent_prod_opt_id='PO001' → 블랙만 L 사이즈 제공
...
CREATE TABLE IF NOT EXISTS dp_ui_area (
    ...
    disp_start_dt       TIMESTAMP,                              -- 전시시작일시
    disp_end_dt         TIMESTAMP,                              -- 전시종료일시
    use_yn              CHAR(1)         DEFAULT 'Y',            -- 사용여부 Y/N
    reg_by              VARCHAR(16),
    reg_date            TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
    upd_by              VARCHAR(16),
...
COMMENT ON COLUMN dp_ui_area.disp_start_dt      IS '전시시작일시';
COMMENT ON COLUMN dp_ui_area.disp_end_dt        IS '전시종료일시';
...
CREATE INDEX idx_dp_ui_area_disp_date ON dp_ui_area (disp_start_dt, disp_end_dt);
...
COMMENT ON COLUMN dp_area_panel.disp_start_dt      IS '전시시작일시';
COMMENT ON COLUMN dp_area_panel.disp_end_dt        IS '전시종료일시';
...
CREATE INDEX idx_dp_area_panel_disp_date ON dp_area_panel (disp_start_dt, disp_end_dt);
...
CREATE TABLE IF NOT EXISTS dp_panel_item (
    ...
    sort_ord                INTEGER         DEFAULT 0,              -- 항목정렬순서
...
COMMENT ON COLUMN dp_panel_item.sort_ord              IS '항목정렬순서';
...
COMMENT ON COLUMN dp_panel_item.disp_start_dt         IS '전시시작일시';
COMMENT ON COLUMN dp_panel_item.disp_end_dt           IS '전시종료일시';
...
CREATE INDEX idx_dp_panel_item_ord ON dp_panel_item (panel_id, sort_ord);
CREATE INDEX idx_dp_panel_item_disp_yn ON dp_panel_item (disp_yn);
CREATE INDEX idx_dp_panel_item_disp_date ON dp_panel_item (disp_start_dt, disp_end_dt);
...
CREATE TABLE IF NOT EXISTS dp_widget (
    ...
    thumbnail_url        VARCHAR(500),                          -- 미리보기 썸네일URL
    sort_ord             INTEGER         DEFAULT 0,
    use_yn               CHAR(1)         DEFAULT 'Y',
    disp_env             VARCHAR(50)     DEFAULT '^PROD^',       -- 전시 환경 (^PROD^DEV^TEST^ 형식)
    reg_by               VARCHAR(16),
...
COMMENT ON COLUMN dp_widget.thumbnail_url        IS '미리보기 썸네일URL';
