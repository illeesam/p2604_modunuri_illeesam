...
public class QOdOrderItemRepositoryImpl implements QOdOrderItemRepository {
    ...
    private BooleanBuilder buildCondition(OdOrderItemDto.Request s) {
        ...
        if (!CollectionUtils.isEmpty(s.getOrderIds())) w.and(oi.orderId.in(s.getOrderIds()));
        if (StringUtils.hasText(s.getOrderId()))     w.and(oi.orderId.eq(s.getOrderId()));
    ...
    private List<OrderSpecifier<?>> buildOrder(OdOrderItemDto.Request s) {
        ...
        if (!StringUtils.hasText(sort)) {
            ...
            if (orders.isEmpty()) orders.add(new OrderSpecifier<>(Order.DESC, oi.regDate));
            return orders;
        }
