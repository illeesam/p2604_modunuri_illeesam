...
public class QMbMemberAddrRepositoryImpl implements QMbMemberAddrRepository {
    ...
    public List<MbMemberAddrDto.Item> selectList(MbMemberAddrDto.Request search) {
        ...
        wheres.add(QdslUtil.strIn(mbMemberAddr.memberId, search.getMemberIds()));
        wheres.add(QdslUtil.strEq(mbMemberAddr.memberAddrId, search.getMemberAddrId()));
        wheres.add(QdslUtil.strEq(mbMemberAddr.memberId, search.getMemberId()));
        /* 기간검색 — dateRangeType 값에 따라 대상 컬럼을 직접 지정 */
        if ("upd_date".equals(search.getDateRangeType())) {
            wheres.add(QdslUtil.dateBetween(mbMemberAddr.updDate, search.getDateRangeStart(), search.getDateRangeEnd()));
        } else {
            wheres.add(QdslUtil.dateBetween(mbMemberAddr.regDate, search.getDateRangeStart(), search.getDateRangeEnd()));   // reg_date (기본)
        }
    ...
    public BasePage<MbMemberAddrDto.Item> selectPageData(MbMemberAddrDto.Request search) {
        ...
        /* 기간검색 — dateRangeType 값에 따라 대상 컬럼을 직접 지정 */
        if ("upd_date".equals(search.getDateRangeType())) {
            whereList.add(QdslUtil.dateBetween(mbMemberAddr.updDate, search.getDateRangeStart(), search.getDateRangeEnd()));
        } else if ("reg_date".equals(search.getDateRangeType())) {
            whereList.add(QdslUtil.dateBetween(mbMemberAddr.regDate, search.getDateRangeStart(), search.getDateRangeEnd()));
        }
