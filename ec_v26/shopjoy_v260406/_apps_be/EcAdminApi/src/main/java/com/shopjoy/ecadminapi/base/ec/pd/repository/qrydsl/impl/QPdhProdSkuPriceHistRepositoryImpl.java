...
public class QPdhProdSkuPriceHistRepositoryImpl implements QPdhProdSkuPriceHistRepository {
    ...
    public List<PdhProdSkuPriceHistDto.Item> selectList(PdhProdSkuPriceHistDto.Request search) {
        ...
        JPAQuery<PdhProdSkuPriceHistDto.Item> query = buildBaseQuery().where(
                andSiteId(search),
                andHistId(search),
    ...
    /* siteId 정확 일치 */
    private BooleanExpression andSiteId(PdhProdSkuPriceHistDto.Request search) {
        return search != null && StringUtils.hasText(search.getSiteId())
                ? h.siteId.eq(search.getSiteId()) : null;
    }

    /* histId 정확 일치 */
    private BooleanExpression andHistId(PdhProdSkuPriceHistDto.Request search) {
        return search != null && StringUtils.hasText(search.getHistId())
                ? h.histId.eq(search.getHistId()) : null;
    }

    /* searchValue LIKE OR — searchType csv 분기 (없으면 전체 필드) */
    private BooleanExpression andSearchValue(PdhProdSkuPriceHistDto.Request search) {
