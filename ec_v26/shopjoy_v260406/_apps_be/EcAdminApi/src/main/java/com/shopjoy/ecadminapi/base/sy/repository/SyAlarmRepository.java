...
public interface SyAlarmRepository extends JpaRepository<SyAlarm, String>, QSyAlarmRepository {
    /* 표시경로 노드별 SyAlarm 수 집계 (자손 누적, PostgreSQL 재귀 CTE)
    ...
    @Query(value = """
            ...
            SELECT d.root_id AS path_id, COUNT(t.alarm_id) AS cnt
              FROM descendants d
              LEFT JOIN sy_alarm t ON t.path_id = d.leaf_id
             GROUP BY d.root_id
            UNION ALL
            SELECT '__total__' AS path_id, COUNT(*) AS cnt FROM sy_alarm
            UNION ALL
            SELECT '__orphan__' AS path_id, COUNT(*) AS cnt FROM sy_alarm WHERE path_id IS NULL
            """, nativeQuery = true)
    List<Object[]> findPathSyAlarmCounts();
