...
public class QPdhProdContentChgHistRepositoryImpl implements QPdhProdContentChgHistRepository {
    ...
    /* siteId 정확 일치 */
    private BooleanExpression andSiteId(PdhProdContentChgHistDto.Request search) {
        return search != null && StringUtils.hasText(search.getSiteId())
                ? h.siteId.eq(search.getSiteId()) : null;
    }

    /* histId 정확 일치 */
    private BooleanExpression andHistId(PdhProdContentChgHistDto.Request search) {
        return search != null && StringUtils.hasText(search.getHistId())
                ? h.histId.eq(search.getHistId()) : null;
    }

    /* 기간 — dateType + dateStart + dateEnd (yyyy-MM-dd, 끝일 포함) */
    private BooleanExpression andDateRange(PdhProdContentChgHistDto.Request search) {
        if (search == null
    ...
    private BooleanExpression andSearchValue(PdhProdContentChgHistDto.Request search) {
        ...
        or = orLike(or, all, types, ",contentAfter,", h.contentAfter, pattern);
        or = orLike(or, all, types, ",contentBefore,", h.contentBefore, pattern);
        or = orLike(or, all, types, ",contentTypeCd,", h.contentTypeCd, pattern);
        or = orLike(or, all, types, ",histId,", h.histId, pattern);
        or = orLike(or, all, types, ",prodContentId,", h.prodContentId, pattern);
