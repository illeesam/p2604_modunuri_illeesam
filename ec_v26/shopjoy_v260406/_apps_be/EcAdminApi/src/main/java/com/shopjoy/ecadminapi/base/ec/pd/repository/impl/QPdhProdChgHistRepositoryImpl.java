...
import com.shopjoy.ecadminapi.base.ec.pd.repository.QPdhProdChgHistRepository;
...
public class QPdhProdChgHistRepositoryImpl implements QPdhProdChgHistRepository {
    ...
    /** 정렬조건 빌드 — Mapper XML pdhProdChgHistSort 와 동일 토큰 */
    ...
    private List<OrderSpecifier<?>> buildOrder(PdhProdChgHistDto.Request s) {
        ...
        switch (sort) {
            case "id_asc":   orders.add(new OrderSpecifier(Order.ASC,  h.prodChgHistId)); break;
            case "id_desc":  orders.add(new OrderSpecifier(Order.DESC, h.prodChgHistId)); break;
