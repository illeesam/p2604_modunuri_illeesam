...
public class MbLikeController {
    ...
    @PostMapping("/save")
    public ResponseEntity<ApiResponse<MbLike>> saveDefault(@RequestBody MbLike entity) {
        return ResponseEntity.ok(ApiResponse.ok(service.save("base", entity), "저장되었습니다."));
    }

    /** save -- rowStatus 단건 분기 저장 (cmd 변형) */
    @PostMapping("/save/{cmd}")
    public ResponseEntity<ApiResponse<MbLike>> saveCmd(
            @PathVariable("cmd") String cmd, @RequestBody MbLike entity) {
    ...
    public ResponseEntity<ApiResponse<Void>> saveListCmd(
            @PathVariable("cmd") String cmd, @RequestBody List<MbLike> rows) {
