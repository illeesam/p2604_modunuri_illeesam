...
public class QMbLikeRepositoryImpl implements QMbLikeRepository {
    ...
    public List<MbLikeDto.Item> selectList(MbLikeDto.Request search) {
        ...
        wheres.add(QdslUtil.strEq(mbLike.likeId, search.getLikeId()));
        wheres.add(QdslUtil.strEq(mbLike.memberId, search.getMemberId()));
        wheres.add(QdslUtil.strEq(mbLike.targetId, search.getTargetId()));
        wheres.add(QdslUtil.strEq(mbLike.targetTypeCd, search.getTargetTypeCd()));
        /* 기간검색 — dateRangeType 값에 따라 대상 컬럼을 직접 지정 */
        if ("upd_date".equals(search.getDateRangeType())) {
            wheres.add(QdslUtil.dateBetween(mbLike.updDate, search.getDateRangeStart(), search.getDateRangeEnd()));
        } else {
            wheres.add(QdslUtil.dateBetween(mbLike.regDate, search.getDateRangeStart(), search.getDateRangeEnd()));   // reg_date (기본)
        }
    ...
    public BasePage<MbLikeDto.Item> selectPageData(MbLikeDto.Request search) {
        ...
        /* 기간검색 — dateRangeType 값에 따라 대상 컬럼을 직접 지정 */
        if ("upd_date".equals(search.getDateRangeType())) {
            whereList.add(QdslUtil.dateBetween(mbLike.updDate, search.getDateRangeStart(), search.getDateRangeEnd()));
        } else if ("reg_date".equals(search.getDateRangeType())) {
            whereList.add(QdslUtil.dateBetween(mbLike.regDate, search.getDateRangeStart(), search.getDateRangeEnd()));
        }
