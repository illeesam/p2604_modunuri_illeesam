...
public interface DpUiRepository extends JpaRepository<DpUi, String>, QDpUiRepository {
    /* 표시경로 노드별 DpUi 수 집계 (자손 누적, PostgreSQL 재귀 CTE)
    ...
    @Query(value = """
            ...
            SELECT d.root_id AS path_id, COUNT(t.ui_id) AS cnt
              FROM descendants d
              LEFT JOIN dp_ui t ON t.path_id = d.leaf_id
             GROUP BY d.root_id
            UNION ALL
            SELECT '__total__' AS path_id, COUNT(*) AS cnt FROM dp_ui
            UNION ALL
            SELECT '__orphan__' AS path_id, COUNT(*) AS cnt FROM dp_ui WHERE path_id IS NULL
            """, nativeQuery = true)
    List<Object[]> findPathDpUiCounts();
