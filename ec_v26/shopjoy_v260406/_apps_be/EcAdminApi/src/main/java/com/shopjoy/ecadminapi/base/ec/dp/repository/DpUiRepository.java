...
public interface DpUiRepository extends JpaRepository<DpUi, String>, QDpUiRepository {
        /* 표시경로 노드별 DpUi 수 집계 (검색조건 + 자손 누적, PostgreSQL 재귀 CTE)
    ...
    @Query(value = """
                ...
                FROM dp_ui t
                 WHERE 1=1
                   ...
                   AND (CAST(:searchValue AS varchar) IS NULL OR (
                             ((CAST(:searchType AS varchar) IS NULL OR :searchType = '' OR :searchType LIKE '%,uiNm,%') AND t.ui_nm ILIKE '%' || :searchValue || '%')
                          OR ((CAST(:searchType AS varchar) IS NULL OR :searchType = '' OR :searchType LIKE '%,uiDesc,%') AND t.ui_desc ILIKE '%' || :searchValue || '%')
                          ))
            ...
            """, nativeQuery = true)
    List<Object[]> findPathDpUiTreeNodeCounts(@Param("useYn")       String useYn,
