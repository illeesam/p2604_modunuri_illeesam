...
public interface DpAreaRepository extends JpaRepository<DpArea, String>, QDpAreaRepository {
        /* 표시경로 노드별 DpArea 수 집계 (검색조건 + 자손 누적, PostgreSQL 재귀 CTE)
     ...
     *     - useYn      : use_yn 일치
     *     - searchValue : area_nm, area_desc 부분일치 OR
     *     - dateStart/End : reg_date 범위 */
    @Query(value = """
            /* com.shopjoy.ecadminapi.base.ec.dp.repository.DpAreaRepository :: findPathDpAreaTreeNodeCounts() */
                ...
                FROM dp_area t
                 WHERE 1=1
                   ...
                   AND (CAST(:searchValue AS varchar) IS NULL OR (
                             ((CAST(:searchType AS varchar) IS NULL OR :searchType = '' OR :searchType LIKE '%,areaNm,%') AND t.area_nm ILIKE '%' || :searchValue || '%')
                          OR ((CAST(:searchType AS varchar) IS NULL OR :searchType = '' OR :searchType LIKE '%,areaDesc,%') AND t.area_desc ILIKE '%' || :searchValue || '%')
                          ))
            ...
            """, nativeQuery = true)
    List<Object[]> findPathDpAreaTreeNodeCounts(@Param("bizCd")     String bizCd,
