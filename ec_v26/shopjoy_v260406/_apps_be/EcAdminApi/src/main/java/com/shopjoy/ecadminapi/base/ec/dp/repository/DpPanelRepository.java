...
public interface DpPanelRepository extends JpaRepository<DpPanel, String>, QDpPanelRepository {
        /* 표시경로 노드별 DpPanel 수 집계 (검색조건 + 자손 누적, PostgreSQL 재귀 CTE)
     ...
     *     - status     : disp_panel_status_cd 일치
     *     - searchValue : panel_nm 부분일치 OR
     *     - dateStart/End : reg_date 범위 */
    @Query(value = """
            /* com.shopjoy.ecadminapi.base.ec.dp.repository.DpPanelRepository :: findPathDpPanelTreeNodeCounts() */
            ...
            filtered AS (
                ...
                FROM dp_panel t
                 WHERE 1=1
                   AND (CAST(:statusCd AS varchar) IS NULL OR t.disp_panel_status_cd = :statusCd)
                   AND (CAST(:searchValue AS varchar) IS NULL OR (
                             ((CAST(:searchType AS varchar) IS NULL OR :searchType = '' OR :searchType LIKE '%,panelNm,%') AND t.panel_nm ILIKE '%' || :searchValue || '%')
                          ))
            ...
            """, nativeQuery = true)
    List<Object[]> findPathDpPanelTreeNodeCounts(@Param("bizCd")     String bizCd,
