...
public class QCmBlogGoodRepositoryImpl implements QCmBlogGoodRepository {
    ...
    private BooleanBuilder buildCondition(CmBlogGoodDto.Request s) {
        ...
        /* searchValue LIKE OR — QCmBlogGood 의 String 필드 (감사필드 제외) */
        if (s != null && StringUtils.hasText(s.getSearchValue())) {
            String pattern = "%" + s.getSearchValue() + "%";
            BooleanBuilder or = new BooleanBuilder();
            or.or(g.blogId.likeIgnoreCase(pattern));
            or.or(g.likeId.likeIgnoreCase(pattern));
            or.or(g.siteId.likeIgnoreCase(pattern));
            or.or(g.userId.likeIgnoreCase(pattern));
