...
import com.shopjoy.ecadminapi.base.ec.mb.repository.QMbMemberGradeRepository;
...
public class QMbMemberGradeRepositoryImpl implements QMbMemberGradeRepository {
    ...
    private List<OrderSpecifier<?>> buildOrder(MbMemberGradeDto.Request s) {
        ...
        if (!StringUtils.hasText(sort)) { orders.add(new OrderSpecifier(Order.DESC, g.regDate)); return orders; }
        switch (sort) {
            case "id_asc":   orders.add(new OrderSpecifier(Order.ASC,  g.memberGradeId)); break;
            case "id_desc":  orders.add(new OrderSpecifier(Order.DESC, g.memberGradeId)); break;
            case "nm_asc":   orders.add(new OrderSpecifier(Order.ASC,  g.gradeNm));       break;
            case "nm_desc":  orders.add(new OrderSpecifier(Order.DESC, g.gradeNm));       break;
