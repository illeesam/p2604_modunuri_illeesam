...
public interface SyTemplateRepository extends JpaRepository<SyTemplate, String>, QSyTemplateRepository {
        /* 표시경로 노드별 SyTemplate 수 집계 (검색조건 + 자손 누적, PostgreSQL 재귀 CTE)
    ...
    @Query(value = """
                ...
                FROM sy_template t
                 WHERE 1=1
                   ...
                   AND (CAST(:searchValue AS varchar) IS NULL OR (
                             ((CAST(:searchType AS varchar) IS NULL OR :searchType = '' OR :searchType LIKE '%,templateCode,%') AND t.template_code ILIKE '%' || :searchValue || '%')
                          OR ((CAST(:searchType AS varchar) IS NULL OR :searchType = '' OR :searchType LIKE '%,templateNm,%') AND t.template_nm ILIKE '%' || :searchValue || '%')
                          ))
            ...
            """, nativeQuery = true)
    List<Object[]> findPathSyTemplateTreeNodeCounts(@Param("bizCd")     String bizCd,
