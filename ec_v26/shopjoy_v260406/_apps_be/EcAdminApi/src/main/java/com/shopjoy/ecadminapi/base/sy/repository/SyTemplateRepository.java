...
public interface SyTemplateRepository extends JpaRepository<SyTemplate, String>, QSyTemplateRepository {
    /* 표시경로 노드별 SyTemplate 수 집계 (자손 누적, PostgreSQL 재귀 CTE)
    ...
    @Query(value = """
            ...
            SELECT d.root_id AS path_id, COUNT(t.template_id) AS cnt
              FROM descendants d
              LEFT JOIN sy_template t ON t.path_id = d.leaf_id
             GROUP BY d.root_id
            UNION ALL
            SELECT '__total__' AS path_id, COUNT(*) AS cnt FROM sy_template
            UNION ALL
            SELECT '__orphan__' AS path_id, COUNT(*) AS cnt FROM sy_template WHERE path_id IS NULL
            """, nativeQuery = true)
    List<Object[]> findPathSyTemplateCounts();
