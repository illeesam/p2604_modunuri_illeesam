...
import com.shopjoy.ecadminapi.base.ec.mb.repository.QMbMemberSnsRepository;
...
public class QMbMemberSnsRepositoryImpl implements QMbMemberSnsRepository {
    ...
    private List<OrderSpecifier<?>> buildOrder(MbMemberSnsDto.Request s) {
        ...
        if (!StringUtils.hasText(sort)) { orders.add(new OrderSpecifier(Order.DESC, m.regDate)); return orders; }
        switch (sort) {
            case "id_asc":   orders.add(new OrderSpecifier(Order.ASC,  m.memberSnsId)); break;
            case "id_desc":  orders.add(new OrderSpecifier(Order.DESC, m.memberSnsId)); break;
