...
public class QPmSaveIssueRepositoryImpl implements QPmSaveIssueRepository {
    ...
    public List<PmSaveIssueDto.Item> selectList(PmSaveIssueDto.Request search) {
        ...
        wheres.add(QdslUtil.strEq(pmSaveIssue.saveIssueId, search.getSaveIssueId()));
        /* 기간검색 — dateRangeType 값에 따라 대상 컬럼을 직접 지정 */
        if ("upd_date".equals(search.getDateRangeType())) {
            wheres.add(QdslUtil.dateBetween(pmSaveIssue.updDate, search.getDateRangeStart(), search.getDateRangeEnd()));
        } else {
            wheres.add(QdslUtil.dateBetween(pmSaveIssue.regDate, search.getDateRangeStart(), search.getDateRangeEnd()));   // reg_date (기본)
        }
    ...
    public BasePage<PmSaveIssueDto.Item> selectPageData(PmSaveIssueDto.Request search) {
        ...
        whereList.add(QdslUtil.strEq(pmSaveIssue.saveIssueId, search.getSaveIssueId()));
        /* 기간검색 — dateRangeType 값에 따라 대상 컬럼을 직접 지정 */
        if ("upd_date".equals(search.getDateRangeType())) {
            whereList.add(QdslUtil.dateBetween(pmSaveIssue.updDate, search.getDateRangeStart(), search.getDateRangeEnd()));
        } else if ("reg_date".equals(search.getDateRangeType())) {
            whereList.add(QdslUtil.dateBetween(pmSaveIssue.regDate, search.getDateRangeStart(), search.getDateRangeEnd()));
        }
