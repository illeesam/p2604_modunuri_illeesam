...
public class QPmDiscntItemRepositoryImpl implements QPmDiscntItemRepository {
    ...
    public List<PmDiscntItemDto.Item> selectList(PmDiscntItemDto.Request search) {
        ...
        JPAQuery<PmDiscntItemDto.Item> query = baseQuery().where(
                andSiteId(search),
                andDiscntItemId(search),
                andDateRange(search),
    ...
    /* siteId 정확 일치 */
    private BooleanExpression andSiteId(PmDiscntItemDto.Request search) {
        return search != null && StringUtils.hasText(search.getSiteId())
                ? i.siteId.eq(search.getSiteId()) : null;
    }

    /* discntItemId 정확 일치 */
    private BooleanExpression andDiscntItemId(PmDiscntItemDto.Request search) {
        return search != null && StringUtils.hasText(search.getDiscntItemId())
                ? i.discntItemId.eq(search.getDiscntItemId()) : null;
    }

    /* 기간 — dateType + dateStart + dateEnd (yyyy-MM-dd, 끝일 포함) */
    private BooleanExpression andDateRange(PmDiscntItemDto.Request search) {
        if (search == null
    ...
    private BooleanExpression andSearchValue(PmDiscntItemDto.Request search) {
        ...
        or = orLike(or, all, types, ",discntId,", i.discntId, pattern);
        or = orLike(or, all, types, ",discntItemId,", i.discntItemId, pattern);
    ...
    private List<OrderSpecifier<?>> buildOrder(PmDiscntItemDto.Request s) {
        ...
        if (!StringUtils.hasText(sort)) {
            ...
            orders.add(new OrderSpecifier<>(Order.ASC, i.discntItemId));
            return orders;
        }
        ...
        for (String part : sortParts) {
            ...
            if (fieldAndDir.length == 2) {
                ...
                if ("discntItemId".equals(field)) {
                    orders.add(new OrderSpecifier(order, i.discntItemId));
