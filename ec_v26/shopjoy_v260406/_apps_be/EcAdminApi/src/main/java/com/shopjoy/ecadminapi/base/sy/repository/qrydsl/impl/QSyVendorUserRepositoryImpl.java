...
public class QSyVendorUserRepositoryImpl implements QSyVendorUserRepository {
    ...
    public List<SyVendorUserDto.Item> selectList(SyVendorUserDto.Request search) {
        ...
        wheres.add(QdslUtil.strEq(syVendorUser.vendorUserId, search.getVendorUserId()));
        wheres.add(QdslUtil.strEq(syVendorUser.userId, search.getUserId()));
        wheres.add(QdslUtil.strEq(syVendorUser.vendorId, search.getVendorId()));
        wheres.add(QdslUtil.strEq(syVendorUser.vendorUserStatusCd, search.getStatus()));
        /* 기간검색 — dateRangeType 값에 따라 대상 컬럼을 직접 지정 */
        if ("upd_date".equals(search.getDateRangeType())) {
            wheres.add(QdslUtil.dateBetween(syVendorUser.updDate, search.getDateRangeStart(), search.getDateRangeEnd()));
        } else {
            wheres.add(QdslUtil.dateBetween(syVendorUser.regDate, search.getDateRangeStart(), search.getDateRangeEnd()));   // reg_date (기본)
        }
    ...
    public BasePage<SyVendorUserDto.Item> selectPageData(SyVendorUserDto.Request search) {
        ...
        /* 기간검색 — dateRangeType 값에 따라 대상 컬럼을 직접 지정 */
        if ("upd_date".equals(search.getDateRangeType())) {
            whereList.add(QdslUtil.dateBetween(syVendorUser.updDate, search.getDateRangeStart(), search.getDateRangeEnd()));
        } else if ("reg_date".equals(search.getDateRangeType())) {
            whereList.add(QdslUtil.dateBetween(syVendorUser.regDate, search.getDateRangeStart(), search.getDateRangeEnd()));
        }
