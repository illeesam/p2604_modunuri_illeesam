...
public interface DpWidgetLibRepository extends JpaRepository<DpWidgetLib, String>, QDpWidgetLibRepository {
        /* 표시경로 노드별 DpWidgetLib 수 집계 (검색조건 + 자손 누적, PostgreSQL 재귀 CTE)
    ...
    @Query(value = """
            WITH RECURSIVE descendants AS (
                SELECT path_id AS root_id,
                       path_id AS leaf_id
                FROM sy_path
                WHERE biz_cd = 'dp_widget_lib'
                UNION ALL
                SELECT d.root_id,
                       c.path_id
                  FROM descendants d
                  JOIN sy_path c ON c.parent_path_id = d.leaf_id
                 WHERE c.biz_cd = 'dp_widget_lib'
            ),
            filtered AS (
                SELECT widget_lib_id,
                       path_id
                FROM dp_widget_lib t
                 WHERE 1=1
                   AND (CAST(:useYn AS varchar) IS NULL OR t.use_yn = :useYn)
                   AND (CAST(:searchValue AS varchar) IS NULL OR (
                             ((CAST(:searchType AS varchar) IS NULL OR :searchType = '' OR :searchType LIKE '%,widgetCode,%') AND t.widget_code ILIKE '%' || :searchValue || '%')
                          OR ((CAST(:searchType AS varchar) IS NULL OR :searchType = '' OR :searchType LIKE '%,widgetNm,%') AND t.widget_nm ILIKE '%' || :searchValue || '%')
                          OR ((CAST(:searchType AS varchar) IS NULL OR :searchType = '' OR :searchType LIKE '%,widgetLibDesc,%') AND t.widget_lib_desc ILIKE '%' || :searchValue || '%')
                          ))
            ...
            """, nativeQuery = true)
    List<Object[]> findPathDpWidgetLibTreeNodeCounts(@Param("useYn")       String useYn,
