...
public interface DpWidgetRepository extends JpaRepository<DpWidget, String>, QDpWidgetRepository {
    /* 표시경로 노드별 DpWidget 수 집계 (자손 누적, PostgreSQL 재귀 CTE)
    ...
    @Query(value = """
            ...
            SELECT d.root_id AS path_id, COUNT(t.widget_id) AS cnt
              FROM descendants d
              LEFT JOIN dp_widget t ON t.path_id = d.leaf_id
             GROUP BY d.root_id
            UNION ALL
            SELECT '__total__' AS path_id, COUNT(*) AS cnt FROM dp_widget
            UNION ALL
            SELECT '__orphan__' AS path_id, COUNT(*) AS cnt FROM dp_widget WHERE path_id IS NULL
            """, nativeQuery = true)
    List<Object[]> findPathDpWidgetCounts();
