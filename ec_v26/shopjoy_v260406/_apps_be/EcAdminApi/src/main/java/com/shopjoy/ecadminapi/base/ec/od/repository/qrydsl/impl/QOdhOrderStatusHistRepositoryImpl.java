...
public class QOdhOrderStatusHistRepositoryImpl implements QOdhOrderStatusHistRepository {
    ...
    public List<OdhOrderStatusHistDto.Item> selectList(OdhOrderStatusHistDto.Request search) {
        ...
        JPAQuery<OdhOrderStatusHistDto.Item> query = baseQuery().where(
                andSiteId(search),
                andOrderStatusHistId(search),
    ...
    /* siteId 정확 일치 */
    private BooleanExpression andSiteId(OdhOrderStatusHistDto.Request search) {
        return search != null && StringUtils.hasText(search.getSiteId())
                ? h.siteId.eq(search.getSiteId()) : null;
    }

    /* orderStatusHistId 정확 일치 */
    private BooleanExpression andOrderStatusHistId(OdhOrderStatusHistDto.Request search) {
        return search != null && StringUtils.hasText(search.getOrderStatusHistId())
                ? h.orderStatusHistId.eq(search.getOrderStatusHistId()) : null;
    }

    /* searchValue LIKE OR — searchType csv 분기 (없으면 전체 필드) */
    private BooleanExpression andSearchValue(OdhOrderStatusHistDto.Request search) {
        ...
        or = orLike(or, all, types, ",orderStatusCd,", h.orderStatusCd, pattern);
        or = orLike(or, all, types, ",orderStatusCdBefore,", h.orderStatusCdBefore, pattern);
        or = orLike(or, all, types, ",orderStatusHistId,", h.orderStatusHistId, pattern);
    ...
    private List<OrderSpecifier<?>> buildOrder(OdhOrderStatusHistDto.Request s) {
        ...
        if (!StringUtils.hasText(sort)) {
            ...
            orders.add(new OrderSpecifier<>(Order.ASC, h.orderStatusHistId));
            return orders;
        }
