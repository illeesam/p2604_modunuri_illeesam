...
public class QSyVendorContentRepositoryImpl implements QSyVendorContentRepository {
    ...
    public List<SyVendorContentDto.Item> selectList(SyVendorContentDto.Request search) {
        ...
        wheres.add(QdslUtil.strEq(syVendorContent.vendorContentId, search.getVendorContentId()));
        wheres.add(QdslUtil.strEq(syVendorContent.vendorId, search.getVendorId()));
        wheres.add(QdslUtil.strEq(syVendorContent.vendorContentStatusCd, search.getStatus()));
        wheres.add(QdslUtil.strEq(syVendorContent.contentTypeCd, search.getContentTypeCd()));
        /* 기간검색 — dateRangeType 값에 따라 대상 컬럼을 직접 지정 */
        if ("upd_date".equals(search.getDateRangeType())) {
            wheres.add(QdslUtil.dateBetween(syVendorContent.updDate, search.getDateRangeStart(), search.getDateRangeEnd()));
        } else {
            wheres.add(QdslUtil.dateBetween(syVendorContent.regDate, search.getDateRangeStart(), search.getDateRangeEnd()));   // reg_date (기본)
        }
    ...
    public BasePage<SyVendorContentDto.Item> selectPageData(SyVendorContentDto.Request search) {
        ...
        /* 기간검색 — dateRangeType 값에 따라 대상 컬럼을 직접 지정 */
        if ("upd_date".equals(search.getDateRangeType())) {
            whereList.add(QdslUtil.dateBetween(syVendorContent.updDate, search.getDateRangeStart(), search.getDateRangeEnd()));
        } else if ("reg_date".equals(search.getDateRangeType())) {
            whereList.add(QdslUtil.dateBetween(syVendorContent.regDate, search.getDateRangeStart(), search.getDateRangeEnd()));
        }
