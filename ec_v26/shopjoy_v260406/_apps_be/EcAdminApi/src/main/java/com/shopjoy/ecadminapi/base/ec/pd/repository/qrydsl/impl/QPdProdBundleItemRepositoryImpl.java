...
public class QPdProdBundleItemRepositoryImpl implements QPdProdBundleItemRepository {
    ...
    private BooleanBuilder buildCondition(PdProdBundleItemDto.Request s) {
        ...
        /* searchValue LIKE OR — QPdProdBundleItem 의 String 필드 (감사필드 제외) */
        if (s != null && StringUtils.hasText(s.getSearchValue())) {
            String pattern = "%" + s.getSearchValue() + "%";
            BooleanBuilder or = new BooleanBuilder();
            or.or(i.bundleItemId.likeIgnoreCase(pattern));
            or.or(i.bundleProdId.likeIgnoreCase(pattern));
        ...
        for (String part : sortParts) {
            ...
            if (fieldAndDir.length == 2) {
                ...
                if ("bundleItemId".equals(field)) {
                    orders.add(new OrderSpecifier(order, i.bundleItemId));
