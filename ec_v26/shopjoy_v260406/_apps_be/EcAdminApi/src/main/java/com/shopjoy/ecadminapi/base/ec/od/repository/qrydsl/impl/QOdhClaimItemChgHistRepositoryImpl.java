...
public class QOdhClaimItemChgHistRepositoryImpl implements QOdhClaimItemChgHistRepository {
    ...
    public List<OdhClaimItemChgHistDto.Item> selectList(OdhClaimItemChgHistDto.Request search) {
        ...
        JPAQuery<OdhClaimItemChgHistDto.Item> query = baseQuery().where(
                andSiteId(search),
                andClaimItemChgHistId(search),
    ...
    /* siteId 정확 일치 */
    private BooleanExpression andSiteId(OdhClaimItemChgHistDto.Request search) {
        return search != null && StringUtils.hasText(search.getSiteId())
                ? h.siteId.eq(search.getSiteId()) : null;
    }

    /* claimItemChgHistId 정확 일치 */
    private BooleanExpression andClaimItemChgHistId(OdhClaimItemChgHistDto.Request search) {
        return search != null && StringUtils.hasText(search.getClaimItemChgHistId())
                ? h.claimItemChgHistId.eq(search.getClaimItemChgHistId()) : null;
    }

    /* searchValue LIKE OR — searchType csv 분기 (없으면 전체 필드) */
    private BooleanExpression andSearchValue(OdhClaimItemChgHistDto.Request search) {
        ...
        or = orLike(or, all, types, ",claimItemChgHistId,", h.claimItemChgHistId, pattern);
    ...
    private List<OrderSpecifier<?>> buildOrder(OdhClaimItemChgHistDto.Request s) {
        ...
        if (!StringUtils.hasText(sort)) {
            ...
            orders.add(new OrderSpecifier<>(Order.ASC, h.claimItemChgHistId));
            return orders;
        }
        ...
        for (String part : sortParts) {
            ...
            if (fieldAndDir.length == 2) {
                ...
                if ("claimItemChgHistId".equals(field)) {
                    orders.add(new OrderSpecifier(order, h.claimItemChgHistId));
