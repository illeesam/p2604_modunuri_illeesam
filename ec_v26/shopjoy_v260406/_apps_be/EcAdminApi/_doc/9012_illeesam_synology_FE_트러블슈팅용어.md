...
작성일: 2026-09-02
관련 문서: [12_illeesam_synology_FE_수동배포가이드(synology).md](<12_illeesam_synology_FE_수동배포가이드(synology).md>)
...
| 2 | 화면은 정상 뜨는데 로그인/목록 조회가 안 됨 | 프론트(화면)와 백엔드(API 서버)는 **서로 다른 배포** — 백엔드(EcAdminApi) 컨테이너가 안 떠 있거나 문제 있는 것 | [11_illeesam_synology_BE_수동배포가이드(synology).md](<11_illeesam_synology_BE_수동배포가이드(synology).md>) STEP 5로 백엔드 상태부터 확인 |
