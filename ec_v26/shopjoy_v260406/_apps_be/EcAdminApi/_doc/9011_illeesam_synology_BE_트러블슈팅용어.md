...
| 3 | 배포 계정을 바꾼 뒤(예: `illeesam` → `appuser`) `deploy:dev-synol-be-ecAdminApi`/`fe` 실행 시 SFTP 업로드 단계에서 `SFTP 업로드 실패: No such file` | 새 계정이 옛 계정과 **동일한 NAS 권한**을 안 갖고 있음 — SFTP는 로그인 자체는 되지만(비밀번호 인증 통과) 그 다음 파일 접근 권한이 없어서 "파일이 없다"로 나타남(실제로는 권한 문제) | 아래 [계정 설정 시 주의사항](#계정-설정-시-주의사항-nas-쪽-배포-계정) 참조 — 4가지를 옛 계정과 동일하게 맞춰야 함 |
