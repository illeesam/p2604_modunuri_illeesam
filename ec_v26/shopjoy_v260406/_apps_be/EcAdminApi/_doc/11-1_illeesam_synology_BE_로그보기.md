...
>
> NAS 접속 방법(호스트/포트/계정, 접속 시 실제 프롬프트 예시)은 [11번 문서](<11_illeesam_synology_BE_수동배포가이드(synology).md>) STEP 2 참조. 아래 명령은 전부 SSH 접속 후, `cd /volume1/docker/shopjoy/backend` 상태에서 입력합니다.
...
**명령어**:
```
illeesam@illeesam:backend$ /usr/local/bin/docker compose logs -f ecadminapi
```
...
**명령어**:
```
illeesam@illeesam:backend$ /usr/local/bin/docker compose logs --tail 100 ecadminapi
```
...
```
illeesam@illeesam:backend$ /usr/local/bin/docker compose logs -f --tail 50 ecadminapi
```
...
**명령어**:
```
illeesam@illeesam:backend$ /usr/local/bin/docker compose logs --since 30m ecadminapi
illeesam@illeesam:backend$ /usr/local/bin/docker compose logs --since "2026-09-05T10:00:00" --until "2026-09-05T10:30:00" ecadminapi
```
...
**명령어**:
```
illeesam@illeesam:backend$ /usr/local/bin/docker compose logs --tail 500 ecadminapi | grep -i "ERROR"
```
...
```
illeesam@illeesam:backend$ /usr/local/bin/docker compose logs --tail 500 ecadminapi | grep -i -B 2 -A 5 "ERROR"
```
...
```
illeesam@illeesam:backend$ /usr/local/bin/docker compose logs --tail 1000 ecadminapi | grep "user-pref"
```
...
**명령어**:
```
illeesam@illeesam:backend$ /usr/local/bin/docker compose logs --tail 2000 ecadminapi | less
```
...
```
illeesam@illeesam:backend$ /usr/local/bin/docker compose logs -f ecadminapi | less +F
```
...
```
illeesam@illeesam:backend$ /usr/local/bin/docker compose logs --tail 2000 ecadminapi | sed -n '100,200p'
```
...
```
illeesam@illeesam:backend$ /usr/local/bin/docker compose logs --tail 2000 ecadminapi | sed -n '1,100p'     # 1페이지
illeesam@illeesam:backend$ /usr/local/bin/docker compose logs --tail 2000 ecadminapi | sed -n '101,200p'   # 2페이지
illeesam@illeesam:backend$ /usr/local/bin/docker compose logs --tail 2000 ecadminapi | sed -n '201,300p'   # 3페이지
```
...
```
illeesam@illeesam:backend$ /usr/local/bin/docker compose logs --tail 5000 ecadminapi > ~/ecadminapi-$(date +%Y%m%d-%H%M).log
```
...
```
~\ec_v26\shopjoy_v260406> scp -P 10022 illeesam@illeesam.synology.me:~/ecadminapi-20260905-1430.log .
```
...
```
illeesam@illeesam:backend$ ls -la /volume1/docker/ecadminapi/logs
```
