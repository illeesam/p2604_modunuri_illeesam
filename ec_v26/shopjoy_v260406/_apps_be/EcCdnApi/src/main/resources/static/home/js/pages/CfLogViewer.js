 ...
 *
 * 2026-09-06 (2차): 아래 요청사항 반영 —
 *   1) "좌측에 파일목록 tree 형식으로" — 파일선택 bo-grid(표) 를 좌측 사이드 tree(.folder-item,
 *      CfFileFileList.js 의 폴더트리와 동일한 시각언어)로 교체. 2단 그리드(.log-viewer-2col).
 *   2) "스크롤 내리면 다음, 다음" + "하단에 더보기 버튼" — 로그 본문을 전부 한번에 DOM 에
 *      그리지 않고 CHUNK(300줄)씩 점진 렌더. logBoxEl 스크롤이 바닥 근처에 닿으면 자동으로
 *      한 청크 더 붙이고(onLogScroll), 같은 동작을 하는 [더보기] 버튼도 로그박스 바로 아래
 *      둔다. 단, "자동 스크롤"(autoScroll) 이 켜져 있으면 원래 취지(항상 최신 로그 보기)를
 *      살려 전체를 렌더하고 바닥으로 스크롤한다 — 이 모드에서만 예외적으로 청크 로딩을 건너뜀.
 *   3) "검색기능" — 기존 키워드 "필터"는 그대로 두되(입력 즉시 클라이언트 필터링), 일치하는
 *      부분을 <mark> 로 강조 표시(fnHighlightHtml, XSS 방지를 위해 원본을 escape 후 강조 삽입).
 *   4) "검색어 템플릿" — 자주 찾는 키워드를 칩 버튼으로 제공(SEARCH_TEMPLATES), 클릭 한 번으로
 *      검색어 채움.
 *
 * 2026-09-06 (3차): 아래 요청사항 추가 반영 —
 *   5) "파일선택 란 tree 형식으로" + "logs 경로부터 트리로" — 좌측을 진짜 트리(펼침/접힘 가능한
 *      루트 폴더 노드 + 그 아래 파일 2개)로. 루트 라벨은 /api/cdn/log/files 응답의 fullPath 에서
 *      파일명을 뗀 실제 디스크 경로(fnDirPath) — 로컬/NAS 어디서 떠도 정확한 값이 보인다.
...
window.CfLogViewer = {
  setup() {
    ...
    // 검색어 템플릿(요청사항) — EcCdnApi 로그에 자주 등장하는 패턴 위주.
    ...
    // 검색 강조(mark) — 로그 원문에 사용자 입력이 그대로 남는 경우(로그인 실패 시 clientId 등)가
    // 있어 v-html 삽입 전 반드시 HTML escape 후 강조 태그를 씌운다(XSS 방지).
    ...
    const fnLoadFileInfo = async () => {
      try {
        fileInfoList.list = await cfAuth.cfApi('/api/cdn/log/files');
    ...
    const fnLoadTail = async () => {
      ...
      try {
        ...
        const data = await cfAuth.cfApi('/api/cdn/log/tail?' + qs.toString());
