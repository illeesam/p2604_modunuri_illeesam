...
import com.shopjoy.ecBeBo.base.ec.pm.data.dto.PmEventChangeStatusDto;
import com.shopjoy.ecBeBo.base.ec.pm.data.entity.PmEvent;
import com.shopjoy.ecBeBo.bo.ec.pm.service.BoPmEventService;
...
public class BoPmEventController {
    ...
    /** update — 수정 */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<PmEvent>> update(@PathVariable("id") String id, @Valid @RequestBody PmEvent body) {
    ...
    /** upsert */
    @PostMapping("/{id}")
    public ResponseEntity<ApiResponse<PmEvent>> upsert(@PathVariable("id") String id, @Valid @RequestBody PmEvent body) {
    ...
    public ResponseEntity<ApiResponse<PmEventDto.Item>> changeStatus(
            @PathVariable("id") String id, @RequestBody PmEventChangeStatusDto.Request req) {
        return ResponseEntity.ok(ApiResponse.ok(boPmEventService.changeStatus(id, req.getStatusCd())));
    }
    /** saveList -- 일괄 저장 (cmd 변형: order 등) */
    @PostMapping("/save-list/{cmd}")
    public ResponseEntity<ApiResponse<Void>> saveListCmd(
            @PathVariable("cmd") String cmd, @Valid @RequestBody List<PmEvent> rows) {
        switch (cmd) {
            case "base" -> boPmEventService.saveListBase(rows);
