...
public class CmUploadService {
    ...
    /* yml 기본값 — sy_prop(app.file.storage-type) 조회가 실패했을 때만 쓰는 최후 폴백.
       2026-09-06: EcBeCdn 실연동(요청사항 "ecBeBo 에 업로드하면 ecBeCdn 로 파일정보 전달하여
    ...
    /** EcBeCdn 가 돌려주는 fileUrl/thumbnailUrl 은 상대경로 — 브라우저가 직접 붙을 절대 URL을
     ...
     * "CDN" 이면 uploadMulti() 가 로컬 디스크 대신 EcBeCdn(CfCdnApiClient)로 업로드를 위임한다.
     */
    ...
    public Map<String, Object> uploadMulti(MultipartFile[] files, String businessCode) {
        ...
        String cdnBase = cdnHost.endsWith("/") ? cdnHost.substring(0, cdnHost.length() - 1) : cdnHost;
            // 2026-09-06: storage-type=CDN 이면 이 배치 전체를 EcBeCdn 로 위임(로컬 디스크 미사용).
            ...
            for (int i = 0; i < files.length; i++) {
                ...
                try {
                    ...
                    if (useCdn) {
                        ...
                        // 전부 EcBeCdn 가 처리한다(120MB 제한도 그쪽에서 검증). 로컬 디스크 기록 없음.
                        ...
                        SyAttach syAttach = SyAttach.builder()
                                ...
                                .fileExt(ext)
                                .mimeTypeCd(file.getContentType())
                                // storedNm/storagePath 는 로컬 파일명이 없는 CDN 저장 특성상 EcBeCdn
