...
import com.shopjoy.ecBeBo.base.zz.data.entity.QZzSample0;
    ...
    private JPAQuery<ZzSample0Dto.Item> baseSelColumnQuery() {
        return queryFactory
                .select(Projections.bean(ZzSample0Dto.Item.class,
                        ...
                        zzSample0.regSiteId,  // 등록사이트ID
                        regSiteEx.siteNm.as("regSiteNm"),  // 등록사이트명 (조인)
                        regUserEx.userNm.as("regUserNm")   // 등록자명 (조인)
                ))
                .from(zzSample0)
                .leftJoin(regSiteEx).on(regSiteEx.siteId.eq(zzSample0.regSiteId)) // 등록사이트
                .leftJoin(regUserEx).on(regUserEx.userId.eq(zzSample0.regBy)) // 등록자
                ;
    }
