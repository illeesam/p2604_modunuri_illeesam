...
public class BoSySiteController {
    ...
    /** page — 페이징조회 */
    @GetMapping("/page")
    public ResponseEntity<ApiResponse<BasePage<SySiteDto.Item>>> page(@Valid @ModelAttribute SySiteDto.Request req) {
     ...
     *   검색조건(searchValue / status / typeCd / dateRangeStart / dateRangeEnd) 이 있으면 그 조건에 부합하는 사이트만 카운트. */
    ...
    /** update — 수정 */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<SySite>> update(@PathVariable("id") String id, @Valid @RequestBody SySite body) {
    ...
    /** upsert */
    @PostMapping("/{id}")
    public ResponseEntity<ApiResponse<SySite>> upsert(@PathVariable("id") String id, @Valid @RequestBody SySite body) {
    ...
    public ResponseEntity<ApiResponse<Void>> saveListCmd(
        ...
        switch (cmd) {
            case "base" -> boSySiteService.saveListBase(rows);
