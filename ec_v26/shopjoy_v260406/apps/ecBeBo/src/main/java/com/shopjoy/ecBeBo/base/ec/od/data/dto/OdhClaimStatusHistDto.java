...
public class OdhClaimStatusHistDto {
    ...
    public static class Request extends BaseRequest {
        ...
        @Size(max = 21) private String claimStatusHistId;  // 클레임상태이력ID (YYMMDDhhmmss+rand4)
        @Size(max = 21) private String claimId;  // 클레임ID (od_claim.claim_id)
    }

    @Getter @Setter @NoArgsConstructor
    public static class Item {
        private String claimStatusHistId;  // 클레임상태이력ID (YYMMDDhhmmss+rand4)
        private String claimId;  // 클레임ID (od_claim.claim_id)
        private String orderId;  // 주문ID (od_order.order_id)
        private String claimStatusCdBefore;  // 변경 전 클레임상태 (코드: CLAIM_STATUS)
        private String claimStatusCd;  // 변경 후 클레임상태 (코드: CLAIM_STATUS)
