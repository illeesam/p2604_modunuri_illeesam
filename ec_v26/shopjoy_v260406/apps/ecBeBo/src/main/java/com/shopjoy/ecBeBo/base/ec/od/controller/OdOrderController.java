...
public class OdOrderController {
    ...
    /* 주문 페이지조회 */
    @GetMapping("/page")
    public ResponseEntity<ApiResponse<BasePage<OdOrderDto.Item>>> page(@Valid @ModelAttribute OdOrderDto.Request req) {
        return ResponseEntity.ok(ApiResponse.ok(service.getPageData(req)));
    }

    /* 주문 등록 */
    @PostMapping
    public ResponseEntity<ApiResponse<OdOrder>> create(@Valid @RequestBody OdOrder entity) {
        return ResponseEntity.status(201).body(ApiResponse.created(service.create(entity)));
    }

    /* 주문 저장 */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<OdOrder>> save(@PathVariable("id") String id, @Valid @RequestBody OdOrder entity) {
    ...
    /* 주문 수정 */
    @PatchMapping("/{id}")
    public ResponseEntity<ApiResponse<OdOrder>> updateSelective(@PathVariable("id") String id, @Valid @RequestBody OdOrder entity) {
        entity.setOrderId(id);
    ...
    public ResponseEntity<ApiResponse<OdOrder>> saveOneCmd(
            @PathVariable("cmd") String cmd, @Valid @RequestBody OdOrder entity) {
        OdOrder result = switch (cmd) {
