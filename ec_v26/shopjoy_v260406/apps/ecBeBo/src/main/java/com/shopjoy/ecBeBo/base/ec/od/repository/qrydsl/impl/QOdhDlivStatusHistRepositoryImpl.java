...
import com.shopjoy.ecBeBo.base.ec.od.data.entity.QOdhDlivStatusHist;
...
/** OdhDlivStatusHist(배송 상태 이력) QueryDSL Custom 구현체 */
...
public class QOdhDlivStatusHistRepositoryImpl implements QOdhDlivStatusHistRepository {
    ...
    private JPAQuery<OdhDlivStatusHistDto.Item> baseSelColumnQuery() {
        return queryFactory
                .select(Projections.bean(OdhDlivStatusHistDto.Item.class,
                        ...
                        odhDlivStatusHist.regBy,      // 등록자
                        odhDlivStatusHist.regDate,    // 등록일시
                        odhDlivStatusHist.updBy,      // 수정자
                        odhDlivStatusHist.updDate,    // 수정일시
                        odhDlivStatusHist.regSiteId,  // 등록사이트ID
                        regSiteEx.siteNm.as("regSiteNm"),  // 등록사이트명 (조인)
                        regUserEx.userNm.as("regUserNm")   // 등록자명 (조인)
                ))
                .from(odhDlivStatusHist)
                .leftJoin(regSiteEx).on(regSiteEx.siteId.eq(odhDlivStatusHist.regSiteId)) // 등록사이트
                .leftJoin(regUserEx).on(regUserEx.userId.eq(odhDlivStatusHist.regBy)) // 등록자
                ;
    }
    ...
    public List<OdhDlivStatusHistDto.Item> selectList(OdhDlivStatusHistDto.Request search) {
        ...
        whereList.add(QdslUtil.strEq(odhDlivStatusHist.dlivStatusHistId, search.getDlivStatusHistId())); // 배송상태이력ID (YYMMDDhhmmss+rand4)
    ...
    /* 배송 상태 이력 페이지조회 */
    @Override
    public BasePage<OdhDlivStatusHistDto.Item> selectPageData(OdhDlivStatusHistDto.Request search) {
    ...
    /* searchType 예: "chgUserId,dlivId,dlivStatusCd,dlivStatusCdBefore,dlivStatusHistId" 등 (콤마 조합, 미지정 시 전체 OR) */
    private BooleanExpression andSearchValue(String searchValue, String searchType) {
        return QdslUtil.searchValueFields(searchValue, searchType, List.of(
            QdslUtil.FieldDef.like("chgUserId", odhDlivStatusHist.chgUserId), // 변경 담당자 (sy_user.user_id, mb_member.member_id)
            QdslUtil.FieldDef.like("dlivId", odhDlivStatusHist.dlivId), // 배송ID (od_dliv.dliv_id)
            QdslUtil.FieldDef.like("dlivStatusCd", odhDlivStatusHist.dlivStatusCd), // 변경 후 배송상태 (코드: DLIV_STATUS)
            QdslUtil.FieldDef.like("dlivStatusCdBefore", odhDlivStatusHist.dlivStatusCdBefore), // 변경 전 배송상태 (코드: DLIV_STATUS)
            QdslUtil.FieldDef.like("dlivStatusHistId", odhDlivStatusHist.dlivStatusHistId), // 배송상태이력ID (YYMMDDhhmmss+rand4)
            QdslUtil.FieldDef.like("memo", odhDlivStatusHist.memo), // 메모
            QdslUtil.FieldDef.like("orderId", odhDlivStatusHist.orderId), // 주문ID (od_order.order_id)
            QdslUtil.FieldDef.like("statusReason", odhDlivStatusHist.statusReason) // 상태 변경 사유
        ));
    }
    ...
    private List<OrderSpecifier<?>> buildOrder(String sort) {
        return QdslUtil.buildOrder(sort,
            Map.of("dlivStatusHistId", odhDlivStatusHist.dlivStatusHistId,
                   "regDate", odhDlivStatusHist.regDate),
        new OrderSpecifier<>(Order.DESC, odhDlivStatusHist.regDate),
        new OrderSpecifier<>(Order.ASC, odhDlivStatusHist.dlivStatusHistId));
    }
