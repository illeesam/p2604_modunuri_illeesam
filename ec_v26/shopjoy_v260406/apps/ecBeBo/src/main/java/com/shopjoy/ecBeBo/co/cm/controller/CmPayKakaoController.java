...
import com.shopjoy.ecBeBo.co.cm.data.vo.PayKakaoApproveReq;
import com.shopjoy.ecBeBo.co.cm.data.vo.PayKakaoCancelReq;
import com.shopjoy.ecBeBo.co.cm.data.vo.PayKakaoReadyReq;
import com.shopjoy.ecBeBo.co.cm.service.CmPayKakaoService;
...
public class CmPayKakaoController {
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> ready(
        ...
        return ResponseEntity.ok(ApiResponse.ok(payKakaoService.ready(request)));
    }

    @PostMapping("/approve")
    public ResponseEntity<ApiResponse<Map<String, Object>>> approve(
            @RequestBody @Valid PayKakaoApproveReq request) {
        return ResponseEntity.ok(ApiResponse.ok(payKakaoService.approve(request)));
    }

    @PostMapping("/cancel")
    public ResponseEntity<ApiResponse<Map<String, Object>>> cancel(
            @RequestBody @Valid PayKakaoCancelReq request) {
        return ResponseEntity.ok(ApiResponse.ok(payKakaoService.cancel(request)));
    }
}
