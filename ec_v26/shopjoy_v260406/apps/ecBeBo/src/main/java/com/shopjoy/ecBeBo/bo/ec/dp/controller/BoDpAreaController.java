...
import com.shopjoy.ecBeBo.bo.ec.dp.service.BoDpAreaService;
...
public class BoDpAreaController {
    ...
    /* 페이지조회 */
    @GetMapping("/page")
    public ResponseEntity<ApiResponse<BasePage<DpAreaDto.Item>>> page(@Valid @ModelAttribute DpAreaDto.Request req) {
    ...
    /* 수정 */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<DpArea>> update(@PathVariable("id") String id, @Valid @RequestBody DpArea body) {
        return ResponseEntity.ok(ApiResponse.ok(boDpAreaService.update(id, body)));
    }

    /* upsert */
    @PostMapping("/{id}")
    public ResponseEntity<ApiResponse<DpArea>> upsert(@PathVariable("id") String id, @Valid @RequestBody DpArea body) {
        return ResponseEntity.ok(ApiResponse.ok(boDpAreaService.update(id, body)));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> saveListCmd(
            @PathVariable("cmd") String cmd, @Valid @RequestBody List<DpArea> rows) {
        switch (cmd) {
            case "base" -> boDpAreaService.saveListBase(rows);
            default -> throw new CmBizException("알 수 없는 saveList cmd: " + cmd);
        }
        return ResponseEntity.ok(ApiResponse.ok(null, "저장되었습니다."));
    }
    /** pathCounts — 표시경로 노드별 DpArea 수 (자손 누적, 트리 우측 뱃지용) */
    @GetMapping("/path-counts")
    public ResponseEntity<ApiResponse<java.util.List<java.util.Map<String, Object>>>> pathCounts(@Valid @ModelAttribute DpAreaDto.Request req) {
        return ResponseEntity.ok(ApiResponse.ok(boDpAreaService.getPathTreeNodeCounts(req)));
    }
