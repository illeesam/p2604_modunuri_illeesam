...
import com.shopjoy.ecBeBo.base.ec.od.data.entity.QOdhOrderStatusHist;
import com.shopjoy.ecBeBo.base.ec.od.repository.qrydsl.QOdhOrderStatusHistRepository;
...
/** OdhOrderStatusHist(주문 상태 이력) QueryDSL Custom 구현체 */
...
public class QOdhOrderStatusHistRepositoryImpl implements QOdhOrderStatusHistRepository {
    ...
    private JPAQuery<OdhOrderStatusHistDto.Item> baseSelColumnQuery() {
        return queryFactory
                .select(Projections.bean(OdhOrderStatusHistDto.Item.class,
                        ...
                        odhOrderStatusHist.regBy,      // 등록자
                        odhOrderStatusHist.regDate,    // 등록일시
                        odhOrderStatusHist.updBy,      // 수정자
                        odhOrderStatusHist.updDate,    // 수정일시
                        odhOrderStatusHist.regSiteId,  // 등록사이트ID
                        regSiteEx.siteNm.as("regSiteNm"),  // 등록사이트명 (조인)
                        regUserEx.userNm.as("regUserNm")   // 등록자명 (조인)
                ))
                .from(odhOrderStatusHist)
                .leftJoin(regSiteEx).on(regSiteEx.siteId.eq(odhOrderStatusHist.regSiteId)) // 등록사이트
                .leftJoin(regUserEx).on(regUserEx.userId.eq(odhOrderStatusHist.regBy)) // 등록자
                ;
    }
    ...
    public Optional<OdhOrderStatusHistDto.Item> selectById(String id) {
        OdhOrderStatusHistDto.Item dtl = baseSelColumnQuery()
    ...
    public List<OdhOrderStatusHistDto.Item> selectList(OdhOrderStatusHistDto.Request search) {
        ...
        List<OdhOrderStatusHistDto.Item> list = query.fetch();
        return list;
    }

    /* 주문 상태 이력 페이지조회 */
    @Override
    public BasePage<OdhOrderStatusHistDto.Item> selectPageData(OdhOrderStatusHistDto.Request search) {
        ...
        BooleanExpression[] wheres = whereList.toArray(BooleanExpression[]::new);
        OrderSpecifier<?>[] orders = orderList.toArray(OrderSpecifier[]::new);
        List<OdhOrderStatusHistDto.Item> pageList = query.clone()
        ...
        BasePage<OdhOrderStatusHistDto.Item> res = new BasePage<>();
    ...
    private List<OrderSpecifier<?>> buildOrder(String sort) {
        return QdslUtil.buildOrder(sort,
            Map.of("orderStatusHistId", odhOrderStatusHist.orderStatusHistId,
                   "regDate", odhOrderStatusHist.regDate),
        new OrderSpecifier<>(Order.DESC, odhOrderStatusHist.regDate),
        new OrderSpecifier<>(Order.ASC, odhOrderStatusHist.orderStatusHistId));
    }
