...
import com.shopjoy.ecBeBo.base.sy.data.entity.QSyI18n;
    ...
    private JPAQuery<SyI18nDto.Item> baseSelColumnQuery() {
        return queryFactory
                .select(Projections.bean(SyI18nDto.Item.class,
                        ...
                        syI18n.updBy,          // 수정자
                        syI18n.updDate,        // 수정일시
                        syI18n.regSiteId,  // 등록사이트ID
                        regSiteEx.siteNm.as("regSiteNm"),  // 등록사이트명 (조인)
                        regUserEx.userNm.as("regUserNm")   // 등록자명 (조인)
                ))
                .from(syI18n)
                .leftJoin(regSiteEx).on(regSiteEx.siteId.eq(syI18n.regSiteId)) // 등록사이트
                .leftJoin(regUserEx).on(regUserEx.userId.eq(syI18n.regBy)) // 등록자
                ;
    }
