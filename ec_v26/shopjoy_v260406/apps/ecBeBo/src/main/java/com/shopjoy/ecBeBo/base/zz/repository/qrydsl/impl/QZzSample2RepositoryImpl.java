package com.shopjoy.ecBeBo.base.zz.repository.qrydsl.impl;
...
import com.shopjoy.ecBeBo.base.zz.data.entity.QZzSample2;
...
/** ZzSample2(다목적 샘플/코드성 데이터 저장소 2) QueryDSL Custom 구현체 */
...
public class QZzSample2RepositoryImpl implements QZzSample2Repository {
    ...
    private JPAQuery<ZzSample2Dto.Item> baseSelColumnQuery() {
        return queryFactory
                .select(Projections.bean(ZzSample2Dto.Item.class,
                        ...
                        zzSample2.regSiteId,  // 등록사이트ID
                        regSiteEx.siteNm.as("regSiteNm"),  // 등록사이트명 (조인)
                        regUserEx.userNm.as("regUserNm")   // 등록자명 (조인)
                ))
                .from(zzSample2)
                .leftJoin(regSiteEx).on(regSiteEx.siteId.eq(zzSample2.regSiteId)) // 등록사이트
                .leftJoin(regUserEx).on(regUserEx.userId.eq(zzSample2.regBy)) // 등록자
                ;
    }
    ...
    public Optional<ZzSample2Dto.Item> selectById(String id) {
        ZzSample2Dto.Item dtl = baseSelColumnQuery()
    ...
    public List<ZzSample2Dto.Item> selectList(ZzSample2Dto.Request search) {
        ...
        whereList.add(QdslUtil.strIn(zzSample2.sample1Id, search.getSample1Ids())); // PK 다건 IN
        whereList.add(QdslUtil.strIn(zzSample2.sample2Id, search.getSample2Ids())); // PK 다건 IN
        whereList.add(QdslUtil.strEq(zzSample2.sample2Id, search.getSample2Id())); // 샘플2 ID 검색값
        whereList.add(QdslUtil.strEq(zzSample2.sample1Id, search.getSample1Id())); // 상위 FK 필터
        whereList.add(QdslUtil.strEq(zzSample2.useYn, search.getUseYn())); // 사용 여부(Y/N) 검색값
        ...
        List<ZzSample2Dto.Item> list = query.fetch();
        return list;
    }
    ...
    public BasePage<ZzSample2Dto.Item> selectPageData(ZzSample2Dto.Request search) {
        ...
        BooleanExpression[] wheres = whereList.toArray(BooleanExpression[]::new);
        OrderSpecifier<?>[] orders = orderList.toArray(OrderSpecifier[]::new);
        List<ZzSample2Dto.Item> pageList = query.clone()
    ...
    private BooleanExpression andSearchValue(String searchValue, String searchType) {
        return QdslUtil.searchValueFields(searchValue, searchType, List.of(
            QdslUtil.FieldDef.like("attrNm1", zzSample2.attrNm1), // 속성명1
            QdslUtil.FieldDef.like("attrNm2", zzSample2.attrNm2), // 속성명2
            QdslUtil.FieldDef.like("attrNm3", zzSample2.attrNm3), // 속성명3
            QdslUtil.FieldDef.like("attrNm4", zzSample2.attrNm4), // 속성명4
            QdslUtil.FieldDef.like("cateCds", zzSample2.cateCds), // 카테고리 코드 목록
            QdslUtil.FieldDef.like("cdGrp", zzSample2.cdGrp), // 도메인 구분 키
            QdslUtil.FieldDef.like("cdInfwSeCd", zzSample2.cdInfwSeCd), // 코드 유입 구분 코드
            QdslUtil.FieldDef.like("cdNm", zzSample2.cdNm), // 코드명 / 대표 텍스트
            QdslUtil.FieldDef.like("cdVl", zzSample2.cdVl), // 코드 값
            QdslUtil.FieldDef.like("col01", zzSample2.col01), // 범용 컬럼01
            QdslUtil.FieldDef.like("col02", zzSample2.col02), // 범용 컬럼02
            QdslUtil.FieldDef.like("col03", zzSample2.col03), // 범용 컬럼03
            QdslUtil.FieldDef.like("col04", zzSample2.col04), // 범용 컬럼04
            QdslUtil.FieldDef.like("col05", zzSample2.col05), // 범용 컬럼05
            QdslUtil.FieldDef.like("col06", zzSample2.col06), // 범용 컬럼06
            QdslUtil.FieldDef.like("col07", zzSample2.col07), // 범용 컬럼07
            QdslUtil.FieldDef.like("col08", zzSample2.col08), // 범용 컬럼08
            QdslUtil.FieldDef.like("col09", zzSample2.col09), // 범용 컬럼09
            QdslUtil.FieldDef.like("divCd", zzSample2.divCd), // 구분 코드
            QdslUtil.FieldDef.like("explnCn", zzSample2.explnCn), // 설명 내용
            QdslUtil.FieldDef.like("groupCd", zzSample2.groupCd), // 그룹 코드
            QdslUtil.FieldDef.like("kindCd", zzSample2.kindCd), // 종류 코드
            QdslUtil.FieldDef.like("sample1Id", zzSample2.sample1Id), // 상위 FK 필터
            QdslUtil.FieldDef.like("sample2Id", zzSample2.sample2Id), // 샘플2 ID 검색값
            QdslUtil.FieldDef.like("statusCd", zzSample2.statusCd), // 상태 코드
            QdslUtil.FieldDef.like("typeCd", zzSample2.typeCd), // 유형 코드
            QdslUtil.FieldDef.like("useYn", zzSample2.useYn) // 사용 여부(Y/N) 검색값
        ));
    }
    ...
    private List<OrderSpecifier<?>> buildOrder(String sort) {
        return QdslUtil.buildOrder(sort,
            Map.of("sample2Id", zzSample2.sample2Id,
                   "regDate", zzSample2.regDate),
        new OrderSpecifier<>(Order.DESC, zzSample2.regDate),
        new OrderSpecifier<>(Order.ASC, zzSample2.sample2Id));
    }
