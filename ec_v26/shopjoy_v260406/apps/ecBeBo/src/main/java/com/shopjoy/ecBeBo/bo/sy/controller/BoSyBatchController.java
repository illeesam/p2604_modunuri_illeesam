...
public class BoSyBatchController {
    ...
    /* 등록 */
    @PostMapping
    public ResponseEntity<ApiResponse<SyBatch>> create(@Valid @RequestBody SyBatch body) {
        return ResponseEntity.status(201).body(ApiResponse.created(boSyBatchService.create(body)));
    }

    /* 수정 */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<SyBatch>> update(@PathVariable("id") String id, @Valid @RequestBody SyBatch body) {
        return ResponseEntity.ok(ApiResponse.ok(boSyBatchService.update(id, body)));
    }

    /* upsert */
    @PostMapping("/{id}")
    public ResponseEntity<ApiResponse<SyBatch>> upsert(@PathVariable("id") String id, @Valid @RequestBody SyBatch body) {
        return ResponseEntity.ok(ApiResponse.ok(boSyBatchService.update(id, body)));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> saveListCmd(
            @PathVariable("cmd") String cmd, @Valid @RequestBody List<SyBatch> rows) {
        switch (cmd) {
            case "base" -> boSyBatchService.saveListBase(rows);
    ...
    public ResponseEntity<ApiResponse<java.util.List<java.util.Map<String, Object>>>> pathCounts(@Valid @ModelAttribute SyBatchDto.Request req) {
