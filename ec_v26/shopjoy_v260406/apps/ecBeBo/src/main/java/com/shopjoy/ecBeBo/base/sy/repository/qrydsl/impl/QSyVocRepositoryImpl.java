...
import com.shopjoy.ecBeBo.base.sy.data.entity.QSyVoc;
...
public class QSyVocRepositoryImpl implements QSyVocRepository {
    ...
    private JPAQuery<SyVocDto.Item> baseSelColumnQuery() {
        return queryFactory
                .select(Projections.bean(SyVocDto.Item.class,
                        ...
                        syVoc.updBy,           // 수정자
                        syVoc.updDate,         // 수정일시
                        syVoc.regSiteId,  // 등록사이트ID
                        regSiteEx.siteNm.as("regSiteNm"),  // 등록사이트명 (조인)
                        regUserEx.userNm.as("regUserNm")   // 등록자명 (조인)
                ))
                .from(syVoc)
                .leftJoin(regSiteEx).on(regSiteEx.siteId.eq(syVoc.regSiteId)) // 등록사이트
                .leftJoin(regUserEx).on(regUserEx.userId.eq(syVoc.regBy)) // 등록자
                ;
    }
    ...
    public Optional<SyVocDto.Item> selectById(String vocId) {
        SyVocDto.Item dtl = baseSelColumnQuery()
    ...
    public List<SyVocDto.Item> selectList(SyVocDto.Request search) {
        ...
        List<SyVocDto.Item> list = query.fetch();
        return list;
    }
    ...
    public BasePage<SyVocDto.Item> selectPageData(SyVocDto.Request search) {
        ...
        BooleanExpression[] wheres = whereList.toArray(BooleanExpression[]::new);
        OrderSpecifier<?>[] orders = orderList.toArray(OrderSpecifier[]::new);
        List<SyVocDto.Item> pageList = query.clone()
