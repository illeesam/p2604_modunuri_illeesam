...
public class OdClaimController {
    ...
    /* 클레임(취소/반품/교환) 페이지조회 */
    @GetMapping("/page")
    public ResponseEntity<ApiResponse<BasePage<OdClaimDto.Item>>> page(@Valid @ModelAttribute OdClaimDto.Request req) {
        return ResponseEntity.ok(ApiResponse.ok(service.getPageData(req)));
    }

    /* 클레임(취소/반품/교환) 등록 */
    @PostMapping
    public ResponseEntity<ApiResponse<OdClaim>> create(@Valid @RequestBody OdClaim entity) {
        return ResponseEntity.status(201).body(ApiResponse.created(service.create(entity)));
    }

    /* 클레임(취소/반품/교환) 저장 */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<OdClaim>> save(@PathVariable("id") String id, @Valid @RequestBody OdClaim entity) {
    ...
    /* 클레임(취소/반품/교환) 수정 */
    @PatchMapping("/{id}")
    public ResponseEntity<ApiResponse<OdClaim>> updateSelective(@PathVariable("id") String id, @Valid @RequestBody OdClaim entity) {
        entity.setClaimId(id);
    ...
    public ResponseEntity<ApiResponse<OdClaim>> saveOneCmd(
            @PathVariable("cmd") String cmd, @Valid @RequestBody OdClaim entity) {
