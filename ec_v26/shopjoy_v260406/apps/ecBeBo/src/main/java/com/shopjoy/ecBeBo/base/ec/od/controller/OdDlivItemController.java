...
import com.shopjoy.ecBeBo.base.ec.od.data.entity.OdDlivItem;
...
public class OdDlivItemController {
    ...
    /* 배송 아이템 등록 */
    @PostMapping
    public ResponseEntity<ApiResponse<OdDlivItem>> create(@Valid @RequestBody OdDlivItem entity) {
        return ResponseEntity.status(201).body(ApiResponse.created(service.create(entity)));
    }

    /* 배송 아이템 저장 */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<OdDlivItem>> save(@PathVariable("id") String id, @Valid @RequestBody OdDlivItem entity) {
    ...
    /* 배송 아이템 수정 */
    @PatchMapping("/{id}")
    public ResponseEntity<ApiResponse<OdDlivItem>> updateSelective(@PathVariable("id") String id, @Valid @RequestBody OdDlivItem entity) {
        entity.setDlivItemId(id);
    ...
    public ResponseEntity<ApiResponse<OdDlivItem>> saveOneCmd(
            @PathVariable("cmd") String cmd, @Valid @RequestBody OdDlivItem entity) {
    ...
    public ResponseEntity<ApiResponse<Void>> saveListCmd(
            @PathVariable("cmd") String cmd, @Valid @RequestBody List<OdDlivItem> rows) {
        switch (cmd) {
