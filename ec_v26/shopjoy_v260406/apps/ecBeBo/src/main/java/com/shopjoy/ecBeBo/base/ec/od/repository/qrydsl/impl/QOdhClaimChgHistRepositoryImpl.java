...
import com.shopjoy.ecBeBo.base.ec.od.data.entity.QOdhClaimChgHist;
import com.shopjoy.ecBeBo.base.ec.od.repository.qrydsl.QOdhClaimChgHistRepository;
...
/** OdhClaimChgHist(클레임 변경 이력) QueryDSL Custom 구현체 */
    ...
    private JPAQuery<OdhClaimChgHistDto.Item> baseSelColumnQuery() {
        return queryFactory
                .select(Projections.bean(OdhClaimChgHistDto.Item.class,
                        ...
                        odhClaimChgHist.regBy,      // 등록자
                        odhClaimChgHist.regDate,    // 등록일시
                        odhClaimChgHist.updBy,      // 수정자
                        odhClaimChgHist.updDate,    // 수정일시
                        odhClaimChgHist.regSiteId,  // 등록사이트ID
                        regSiteEx.siteNm.as("regSiteNm"),  // 등록사이트명 (조인)
                        regUserEx.userNm.as("regUserNm")   // 등록자명 (조인)
                ))
                .from(odhClaimChgHist)
                .leftJoin(regSiteEx).on(regSiteEx.siteId.eq(odhClaimChgHist.regSiteId)) // 등록사이트
                .leftJoin(regUserEx).on(regUserEx.userId.eq(odhClaimChgHist.regBy)) // 등록자
                ;
    }
