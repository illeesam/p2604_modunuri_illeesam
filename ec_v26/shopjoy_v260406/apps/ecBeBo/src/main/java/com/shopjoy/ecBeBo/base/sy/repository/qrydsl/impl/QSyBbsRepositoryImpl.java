...
import com.shopjoy.ecBeBo.base.sy.data.entity.QSyBbs;
    ...
    private JPAQuery<SyBbsDto.Item> baseSelColumnQuery() {
        return queryFactory
                .select(Projections.bean(SyBbsDto.Item.class,
                        ...
                        syBbs.updBy,          // 수정자
                        syBbs.updDate,        // 수정일시
                        syBbs.regSiteId,  // 등록사이트ID
                        regSiteEx.siteNm.as("regSiteNm"),  // 등록사이트명 (조인)
                        regUserEx.userNm.as("regUserNm")   // 등록자명 (조인)
                ))
                .from(syBbs)
                .leftJoin(regSiteEx).on(regSiteEx.siteId.eq(syBbs.regSiteId)) // 등록사이트
                .leftJoin(regUserEx).on(regUserEx.userId.eq(syBbs.regBy)) // 등록자
                ;
    }
