...
import com.shopjoy.ecBeBo.base.ec.od.data.entity.QOdhDlivItemChgHist;
import com.shopjoy.ecBeBo.base.ec.od.repository.qrydsl.QOdhDlivItemChgHistRepository;
...
public class QOdhDlivItemChgHistRepositoryImpl implements QOdhDlivItemChgHistRepository {
    ...
    private JPAQuery<OdhDlivItemChgHistDto.Item> baseSelColumnQuery() {
        return queryFactory
                .select(Projections.bean(OdhDlivItemChgHistDto.Item.class,
                        ...
                        odhDlivItemChgHist.regBy,      // 등록자
                        odhDlivItemChgHist.regDate,    // 등록일시
                        odhDlivItemChgHist.updBy,      // 수정자
                        odhDlivItemChgHist.updDate,    // 수정일시
                        odhDlivItemChgHist.regSiteId,  // 등록사이트ID
                        regSiteEx.siteNm.as("regSiteNm"),  // 등록사이트명 (조인)
                        regUserEx.userNm.as("regUserNm")   // 등록자명 (조인)
                ))
                .from(odhDlivItemChgHist)
                .leftJoin(regSiteEx).on(regSiteEx.siteId.eq(odhDlivItemChgHist.regSiteId)) // 등록사이트
                .leftJoin(regUserEx).on(regUserEx.userId.eq(odhDlivItemChgHist.regBy)) // 등록자
                ;
    }
    ...
    public List<OdhDlivItemChgHistDto.Item> selectList(OdhDlivItemChgHistDto.Request search) {
        ...
        whereList.add(QdslUtil.strEq(odhDlivItemChgHist.dlivItemChgHistId, search.getDlivItemChgHistId())); // 이력ID
    ...
    private BooleanExpression andSearchValue(String searchValue, String searchType) {
        return QdslUtil.searchValueFields(searchValue, searchType, List.of(
            QdslUtil.FieldDef.like("afterVal", odhDlivItemChgHist.afterVal), // 변경후값
            QdslUtil.FieldDef.like("beforeVal", odhDlivItemChgHist.beforeVal), // 변경전값
            QdslUtil.FieldDef.like("chgField", odhDlivItemChgHist.chgField), // 변경 필드명
            QdslUtil.FieldDef.like("chgReason", odhDlivItemChgHist.chgReason), // 변경사유
            QdslUtil.FieldDef.like("chgTypeCd", odhDlivItemChgHist.chgTypeCd), // 변경유형코드 (QTY/STATUS/CARRIER/TRACK_NO/RECV_INFO)
            QdslUtil.FieldDef.like("chgUserId", odhDlivItemChgHist.chgUserId), // 처리자 (sy_user.user_id)
            QdslUtil.FieldDef.like("dlivId", odhDlivItemChgHist.dlivId), // 배송ID (od_dliv.)
            QdslUtil.FieldDef.like("dlivItemChgHistId", odhDlivItemChgHist.dlivItemChgHistId), // 이력ID
            QdslUtil.FieldDef.like("dlivItemId", odhDlivItemChgHist.dlivItemId) // 배송품목ID (od_dliv_item.)
        ));
    }
