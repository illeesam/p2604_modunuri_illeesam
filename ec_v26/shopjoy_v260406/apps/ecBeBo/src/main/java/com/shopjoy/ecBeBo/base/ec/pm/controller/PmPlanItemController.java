...
import com.shopjoy.ecBeBo.base.ec.pm.service.PmPlanItemService;
...
public class PmPlanItemController {
    ...
    /* 프로모션 플랜 아이템 등록 */
    @PostMapping
    public ResponseEntity<ApiResponse<PmPlanItem>> create(@Valid @RequestBody PmPlanItem entity) {
        return ResponseEntity.status(201).body(ApiResponse.created(service.create(entity)));
    }

    /* 프로모션 플랜 아이템 저장 */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<PmPlanItem>> save(@PathVariable("id") String id, @Valid @RequestBody PmPlanItem entity) {
    ...
    /* 프로모션 플랜 아이템 수정 */
    @PatchMapping("/{id}")
    public ResponseEntity<ApiResponse<PmPlanItem>> updateSelective(@PathVariable("id") String id, @Valid @RequestBody PmPlanItem entity) {
        entity.setPlanItemId(id);
    ...
    public ResponseEntity<ApiResponse<PmPlanItem>> saveOneCmd(
            @PathVariable("cmd") String cmd, @Valid @RequestBody PmPlanItem entity) {
        PmPlanItem result = switch (cmd) {
    ...
    public ResponseEntity<ApiResponse<Void>> saveListCmd(
            @PathVariable("cmd") String cmd, @Valid @RequestBody List<PmPlanItem> rows) {
        switch (cmd) {
