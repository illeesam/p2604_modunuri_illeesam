...
public class BoDpUiController {
    ...
    /* 페이지조회 */
    @GetMapping("/page")
    public ResponseEntity<ApiResponse<BasePage<DpUiDto.Item>>> page(@Valid @ModelAttribute DpUiDto.Request req) {
    ...
    /* 수정 */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<DpUi>> update(@PathVariable("id") String id, @Valid @RequestBody DpUi body) {
        return ResponseEntity.ok(ApiResponse.ok(boDpUiService.update(id, body)));
    }

    /* upsert */
    @PostMapping("/{id}")
    public ResponseEntity<ApiResponse<DpUi>> upsert(@PathVariable("id") String id, @Valid @RequestBody DpUi body) {
        return ResponseEntity.ok(ApiResponse.ok(boDpUiService.update(id, body)));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> saveListCmd(
            @PathVariable("cmd") String cmd, @Valid @RequestBody List<DpUi> rows) {
        switch (cmd) {
            case "base" -> boDpUiService.saveListBase(rows);
    ...
    public ResponseEntity<ApiResponse<java.util.List<java.util.Map<String, Object>>>> pathCounts(@Valid @ModelAttribute DpUiDto.Request req) {
