...
public class BoSyAlarmController {
    ...
    /* 수정 */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<SyAlarm>> update(@PathVariable("id") String id, @Valid @RequestBody SyAlarm body) {
        return ResponseEntity.ok(ApiResponse.ok(boSyAlarmService.update(id, body)));
    }

    /* upsert */
    @PostMapping("/{id}")
    public ResponseEntity<ApiResponse<SyAlarm>> upsert(@PathVariable("id") String id, @Valid @RequestBody SyAlarm body) {
        return ResponseEntity.ok(ApiResponse.ok(boSyAlarmService.update(id, body)));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> saveListCmd(
            @PathVariable("cmd") String cmd, @Valid @RequestBody List<SyAlarm> rows) {
        switch (cmd) {
