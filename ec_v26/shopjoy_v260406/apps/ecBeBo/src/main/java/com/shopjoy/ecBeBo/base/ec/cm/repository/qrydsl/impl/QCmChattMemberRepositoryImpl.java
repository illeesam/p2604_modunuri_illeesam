...
import com.shopjoy.ecBeBo.base.ec.cm.data.entity.QCmChattMember;
    ...
    private JPAQuery<CmChattMemberDto.Item> baseSelColumnQuery() {
        return queryFactory
                .select(Projections.bean(CmChattMemberDto.Item.class,
                        ...
                        cmChattMember.regSiteId,  // 등록사이트ID
                        regSiteEx.siteNm.as("regSiteNm"),  // 등록사이트명 (조인)
                        regUserEx.userNm.as("regUserNm")   // 등록자명 (조인)
                ))
                .from(cmChattMember)
                .leftJoin(regSiteEx).on(regSiteEx.siteId.eq(cmChattMember.regSiteId)) // 등록사이트
                .leftJoin(regUserEx).on(regUserEx.userId.eq(cmChattMember.regBy)) // 등록자
                ;
    }
