...
| | `npm run deploy:dev-synol-be-ecAdminApi` | `npm run deploy:dev-github-be-api` |
|---|---|---|
...
**평소엔 `deploy:dev-synol-be-ecAdminApi`를 쓰시면 됩니다(더 빠르고 간단함).** 프론트까지 같이 배포하려면 [15번 문서](<15_illeesam_synology_FE_자동배포가이드(npm script).md>)의 `deploy:dev-synol-fe-vue3cdn`를 이어서 실행하거나, 백엔드+프론트를 한 번에 하려면 `npm run deploy:dev-synol-full`을 쓰세요(내부적으로 `deploy:dev-synol-be-ecAdminApi` → `deploy:dev-synol-fe-vue3cdn` 순서로 실행됩니다).

---

## 방식 A — `npm run deploy:dev-synol-be-ecAdminApi` (직접 SSH, 권장)
...
> **백엔드+프론트를 한 번에 배포하고 싶으면**: `npm run deploy:dev-synol-full` (= `deploy:dev-synol-be-ecAdminApi` 다음 `deploy:dev-synol-fe-vue3cdn`를 순서대로 실행 — 자세한 프론트 쪽 내용은 [15번 문서](<15_illeesam_synology_FE_자동배포가이드(npm script).md>) 참조).
