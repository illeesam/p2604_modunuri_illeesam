...
window.DpDispWidgetDtl = {
  ...
  setup(props, { emit }) {
    ...
    const fnCallbackModal = (popCmd, param, result) => {
      console.log(' ■■ DpDispWidgetDtl : fnCallbackModal -> ', popCmd, param, result);
    ...
    const fnLoadCodes = async () => {
      ...
      await codeStore.saLoadCodes(['WIDGET_TYPE_CD'], {compNm: 'DpDispWidgetDtl'});
    ...
    /* cfDispEnvMcsOptions — bo-multi-check-select 용 {value,label} 변환 */
    const cfDispEnvMcsOptions = dispEnvOptions.map(o => ({ value: o.code, label: o.label }));
    ...
    /* fnShareUrl — 이 전시위젯 상세를 가리키는 독립 새창 딥링크 URL 생성 */
    const fnShareUrl = () => {
      const qs = new URLSearchParams();
      qs.set('page', 'dpDispWidgetDtl');
      qs.set('id', form.widgetId || form.libId);
      qs.set('embed', '1');
    ...
    const handleShareKakao = () => {
      try {
        window.coExtSdk.shareKakao({
          title: `전시위젯 ${form.widgetId || form.libId} - ShopJoy BO`,
    ...
    /* pdfAreaRef — 전시위젯 상세 카드 캡처 대상. handleExportPdf — PDF 다운로드(상세보기 모드 전용) */
    const pdfAreaRef = ref(null);
    const pdfExporting = ref(false);
    const handleExportPdf = async () => {
      pdfExporting.value = true;
      try {
        const filename = coUtil.cofBuildExportFilename(`전시위젯상세_${form.widgetId || form.libId}.pdf`);
