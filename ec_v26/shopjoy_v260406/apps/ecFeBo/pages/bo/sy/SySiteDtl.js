...
window.SySiteDtl = {
  ...
  setup(props) {
    ...
    /* fnShareUrl — 이 사이트 상세를 가리키는 독립 새창 딥링크 URL 생성 */
    const fnShareUrl = () => {
      const qs = new URLSearchParams();
      qs.set('page', 'sySiteDtl');
      qs.set('id', form.siteId);
      qs.set('embed', '1');
    ...
    const handleShareKakao = () => {
      try {
        window.coExtSdk.shareKakao({
          title: `사이트 ${form.siteId} - ShopJoy BO`,
          description: form.siteNm || '',
    ...
    /* pdfAreaRef — 사이트 상세 카드 캡처 대상. handleExportPdf — PDF 다운로드(항상 노출) */
    const pdfAreaRef = ref(null);
    const pdfExporting = ref(false);
    const handleExportPdf = async () => {
      pdfExporting.value = true;
      try {
        const filename = coUtil.cofBuildExportFilename(`사이트상세_${form.siteId}.pdf`);
