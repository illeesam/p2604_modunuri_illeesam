/* ShopJoy - Sample05: 게시판 관리 CRUD Grid  (API: GET|POST|PUT|DELETE /api/fo/xs/sample1, cdGrp='S05_BOARD')
 * ZzSample1 필드 매핑:
  ...
  setup() {
    ...
    const handleBtnAction = (cmd, param = {}) => {
      ...
      // 일괄 저장
        ...
        return handleSave();
      // 선택 행 삭제
      } else if (cmd === 'boards-deleteChecked') {
        return deleteRows();
      // 선택 행 취소
      } else if (cmd === 'boards-cancelChecked') {
    ...
    const handleSelectAction = (cmd, param = {}) => {
      ...
      // 행 셀 변경
      if (cmd === 'boards-rowCellChange') {
        return onCellChange(param);
      // 행 취소
      } else if (cmd === 'boards-rowCancel') {
        return onRowCancel(param);
      // 행 삭제
      } else if (cmd === 'boards-rowDelete') {
    ...
    const handleSearchList = async (searchType = 'DEFAULT') => {
      try {
        const res = await foApi.get('/fo/xs/sample1', { params: { cdGrp: CD_GRP }, ...coUtil.cofApiHdr('샘플-게시판관리', '조회') });
    ...
    const handleSave = async () => {
      ...
      try {
        for (const r of dRows) { await foApi.delete(`/fo/xs/sample1/${r.boardId}`, coUtil.cofApiHdr('샘플-게시판관리', '삭제')); }
        for (const r of uRows) { await foApi.put(`/fo/xs/sample1/${r.boardId}`, toPayload(r), coUtil.cofApiHdr('샘플-게시판관리', '저장')); }
        for (const r of iRows) { await foApi.post('/fo/xs/sample1', toPayload(r), coUtil.cofApiHdr('샘플-게시판관리', '저장')); }
...
<div style="padding:clamp(12px,3vw,24px);">
  ...
  <fo-grid-crud
    ...
    @delete-checked="handleBtnAction('boards-deleteChecked')" @cancel-checked="handleBtnAction('boards-cancelChecked')"
    @reorder="handleBtnAction('boards-reorder')" @cell-change="handleSelectAction('boards-rowCellChange', $event)">
    <template #row-actions="{ row }">
      <fo-row-cancel-delete :row="row" @cancel="handleSelectAction('boards-rowCancel', row)" @delete="handleSelectAction('boards-rowDelete', row)" />
    </template>
  </fo-grid-crud>
</div>
</fo-page>
