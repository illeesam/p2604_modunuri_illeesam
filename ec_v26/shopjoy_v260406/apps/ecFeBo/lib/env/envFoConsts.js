/* ShopJoy - 외부 연동 상수 모음 (FO / envFoConsts)
 * ─────────────────────────────────────────────────────────────────────────
 ...
 * lib/env/profiles/envFoConsts.{dev,prod}.js 에 따로 있다. 배포 빌드 시
 * `npm run build:dev`(또는 build:prod)를 주면 scripts/build-minify.js 가 그
 * 프로파일 파일을 dist/lib/env/envFoConsts.js 자리에 덮어써서 만든다 — 즉
...
(function () {
  window.envFoConsts = {
     ...
     * foApiAxios.js 의 apiUrl()/seoUrl() 이 이 값들로 절대(또는 상대) URL을 만든다.
     *   - baseApiHost '' (빈 문자열) = 상대경로. baseApiHost 에 값이 있으면 그 호스트로 절대
     *     URL 조립(포트는 baseApiPort, 비어있으면 생략). 프로토콜은 지금 페이지와 동일하게
     *     맞춘다(window.location.protocol) — 별도 필드 불필요.
     ...
     * foApiAxios.js 의 cdnUrl() 이 이 값들로 URL을 만든다. 지금은 백엔드가 /cdn/** 도 같이
