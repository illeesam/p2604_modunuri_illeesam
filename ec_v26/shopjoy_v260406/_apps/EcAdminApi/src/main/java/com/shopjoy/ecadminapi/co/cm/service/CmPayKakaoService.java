 ...
 *
 * <p>흐름:
 * 1. ready  — 결제 준비 → next_redirect_pc_url(PC) / next_redirect_mobile_url(모바일) 반환
 ...
 *
 * <p>키 출처: sy_prop (app.pay.kakaopay.*) — BO 시스템 > 프로퍼티 관리에서 설정.
 */
@Slf4j
@Service
...
public class CmPayKakaoService {
    ...
    /** sy_prop에서 app.pay.kakaopay.* 키를 현재 active profile 기준으로 조회 */
    ...
    /**
     * 카카오페이 결제 준비. 성공 시 프론트에 next_redirect_pc_url / next_redirect_mobile_url 반환.
     * 프론트는 해당 URL로 리다이렉트(또는 팝업) 하여 카카오페이 결제창을 연다.
     */
    @Transactional
    ...
    /**
     * 카카오페이 결제 승인. approvalUrl 리다이렉트 시 받은 pg_token + ready 응답의 tid 로 승인.
     */
    @Transactional
    ...
    /**
     * 카카오페이 결제 취소 (전체/부분). cancelAmount = 전체 금액이면 전체취소.
     */
    @Transactional
