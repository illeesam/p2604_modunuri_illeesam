 ...
 * filter: "^" 구분 토큰 목록.
 *   - "*"                → 전체 기록
 *   - "BO^FO^EXT"       → 해당 userType 전체 기록
 *   - "admin01^BO"      → 특정 userId 또는 userType 매칭
 *   - 비어있거나 null   → 전체 기록
 */
@Getter @Setter
...
public class AccessLogProperties {
    ...
    /** "^" 구분 필터. userType 코드(BO/FO/EXT/USER/MEMBER) 또는 특정 userId. "*"=전체 */
    ...
    public boolean isMatch(String userType, String userId) {
        ...
        for (String token : filter.split("\\^")) {
            ...
            if (TYPE_TOKENS.contains(t.toUpperCase())) {
                if (t.equalsIgnoreCase(userType)) return true;
            } else {
