 ...
 *
 * <p>흐름:
 * 1. reserve  — 결제 예약 → reserveId 반환 → 프론트가 결제창 URL 로 이동
 *               PC:     https://pay.naver.com/payments/new?reservationId={reserveId}
 *               Mobile: https://m.pay.naver.com/payments/new?reservationId={reserveId}
 * 2. approve  — returnUrl 리다이렉트 후 paymentId 로 최종 승인
 * 3. cancel   — paymentId 로 전체/부분 취소
 *
 * <p>인증: Client ID / Client Secret 을 X-Naver-Client-Id / X-Naver-Client-Secret 헤더로 전달.
 *
 * <p>키 출처: sy_prop (app.pay.naverpay.*) — BO 시스템 > 프로퍼티 관리에서 설정.
 * 테스트 환경: https://dev.apis.naver.com/naverpay-partner/naverpay/...
 * 운영 환경:   https://apis.naver.com/naverpay-partner/naverpay/...
 */
@Slf4j
@Service
...
public class CmPayNaverService {
    ...
    /** sy_prop에서 app.pay.naverpay.* 키를 현재 active profile 기준으로 조회 */
    ...
    /**
     * 네이버페이 결제 예약. 성공 시 reserveId 반환.
     * 프론트는 https://pay.naver.com/payments/new?reservationId={reserveId} 로 이동.
     */
    @Transactional
    ...
    /**
     * 네이버페이 결제 승인. returnUrl 파라미터로 받은 paymentId 로 승인.
     */
    @Transactional
