...
- [공통 API 서비스 (services/)](#공통-api-서비스-services) — coApi / foApiSvc / boApiSvc ⭐
...
| `window.coApiSvc` | `services/coApiSvc.js` | FO·BO 공통 API (code / site / path 등) |
| `window.foApiSvc` | `services/foApiSvc.js` | FO 전용 공통 API (member / product / notice 등) |
| `window.boApiSvc` | `services/boApiSvc.js` | BO 전용 공통 API (member / user / brand / vendor / category 등) |

## 공통 API 서비스 (`services/`)

**목적**: 2개 이상의 화면에서 동일 엔드포인트를 호출하는 경우 `services/` 파일에 등록하여 공유.  
...
| 파일 | 전역 객체 | 로드 대상 | 설명 |
|---|---|---|---|
| `services/coApiSvc.js` | `window.coApi` | FO (`index.html`) + BO (`bo.html`) | FO·BO 양쪽에서 사용하는 공통 엔드포인트 |
...
> 서비스 객체는 `foApiSvc` / `boApiSvc` / `coApi` 로 구분.
...
```js
// ✅ services/ 등록 — 2개 이상 화면에서 동일 엔드포인트
const res = await boApiSvc.member.getPage({ kw: '홍길동' });
const res = await coApiSvc.code.getGrpCodes('ORDER_STATUS');

// ✅ 페이지 직접 선언 — 단 1개 화면 전용
const res = await boApi.get('/bo/sy/batch/run', coUtil.apiHdr('배치', '즉시실행'));
```

### 새 API 추가 방법

1. 해당 `services/` 파일에 도메인 네임스페이스로 추가
2. 동일 패턴: `return global.boApi.get(path, { params, ...coUtil.apiHdr(uiNm, cmdNm) })`
3. `coApi`는 `client()` 헬퍼 사용 (FO·BO 환경 자동 선택)

```js
// services/boApiSvc.js 추가 예시
boApiSvc.coupon = {
  getPage(params) {
    return global.boApi.get('/bo/ec/coupon/page', {
      params,
      ...coUtil.apiHdr('쿠폰', '목록조회'),
    });
  },
};
```
