...
| **axios** | 1.7.9 (공식) | `assets/cdn/pkg/axios/1.7.9/axios.min.js` | 공식 UMD. BO=`window.boApi` (utils/boApiAxios.js) / FO=`window.foApi` (utils/foApiAxios.js) 래퍼 |
...
├─ utils/boApiAxios.js (window.boApi) + utils/boUtil.js (window.boUtil)
...
| `window.foApi` | `utils/foApiAxios.js` | FO axios 래퍼. get/post/put/patch/delete. URL 직접 (예: `foApi.get('api/base/...')`) |
| `window.boApi` | `utils/boApiAxios.js` | BO axios 래퍼. get/post/put/patch/delete. **변경성(post/put/delete/patch) 호출 시 `coUtil.apiHdr(uiNm, cmdNm)` 필수** |
| `window.boUtil` | `utils/boUtil.js` | BO 공통 유틸 (`bofGetSiteNm`, `bofGetDateRange`, `bofGetPathLabel`, `bofBuildRoleTree` 등) |
| `window.foUtil` | `utils/foUtil.js` | FO 공통 유틸 |
| `window.coUtil` | `utils/coUtil.js` | BO·FO 공통 (`cofAnd`, `cofExportCsv`, `cofUseAppCodeReady`, `apiHdr` 등) |
...
10. **소스 코드 포매팅 표준** ⭐ — `pages/**/*.js`·`components/**/*.js` 의 `template:` HTML 내부는 깊이별 2칸 들여쓰기 + 단순 인라인 텍스트 보존(140자 이내) + 멀티라인 속성 재들여쓰기. JS 영역은 2줄 이상 연속 빈줄 → 1줄로 압축, trailing 공백 제거. `<style>/<script>/<pre>/<textarea>` 내부와 `&` 포함 속성은 원본 그대로 보존. `base/boApp.js`·`base/foApp.js` 는 포매팅 예외 (Vue 컴파일러 크래시 방지). 일반 IDE 포매터(Prettier 등) 금지 — 전용 도구만 사용. 상세 → `_doc/정책서/base/base.코드스타일-admin-vue.md` §12
