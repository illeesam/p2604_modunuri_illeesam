...
window.XsSample12 = {
  ...
  setup() {
    ...
    const uiState = reactive({ loading: false, error: null, previewDate: new Date().toISOString().slice(0, 10), previewTime: new Date().toTimeString().slice(0, 5), showAreaDrop: false, showCatModal: false });
    ...
    const handleSelectAction = (cmd, param = {}) => {
      console.log(' ■■ XsSample12.js : handleSelectAction -> ', cmd, param);
      // 영역 토글
      if (cmd === 'areas-toggle') {
    ...
    const fnCallbackModal = (popCmd, param, result) => {
      console.log(' ■■ Sample12 : fnCallbackModal -> ', popCmd, param, result);
