...
window.XsSample09 = {
  ...
  setup() {
    ...
    const CD_GRP = 'S09_FAQ';                      // FAQ 코드 그룹
    ...
    const toRow = d => ({
      faqId: d.sample1Id, question: d.cdNm || '', category: d.col01 || '배송',
      author: d.col02 || '관리자', viewCnt: Number(d.col03) || 0,
      status: d.useYn === 'Y' ? '공개' : '비공개', regDate: d.regDt || '',
    });

    /* toPayload — 화면 행 → ZzSample1 페이로드 */
    const toPayload = r => ({
      cdGrp: CD_GRP, cdNm: r.question, col01: r.category, col02: r.author,
    ...
    const makeRow = d => ({
      ...
      _row_org: { question: d.question, category: d.category, author: d.author, status: d.status },
    });
    ...
    const handleBtnAction = (cmd, param = {}) => {
      console.log(' ■■ Sample09.js : handleBtnAction -> ', cmd, param);
      // 조회 (검색)
      ...
      // 행 추가
        ...
        return addRow();
      // 일괄 저장
      } else if (cmd === 'faqs-save') {
        return handleSave();
      // 선택 행 삭제
      } else if (cmd === 'faqs-deleteChecked') {
        return deleteRows();
      // 선택 행 취소
      } else if (cmd === 'faqs-cancelChecked') {
        return cancelChecked();
      // 정렬 변경 알림
      } else if (cmd === 'faqs-reorder') {
        return onReorder();
      } else {
    ...
    const handleSelectAction = (cmd, param = {}) => {
      console.log(' ■■ Sample09.js : handleSelectAction -> ', cmd, param);
      // 행 셀 변경
      if (cmd === 'faqs-rowCellChange') {
        return onCellChange(param);
      // 행 취소
      } else if (cmd === 'faqs-rowCancel') {
        return onRowCancel(param);
      // 행 삭제
      } else if (cmd === 'faqs-rowDelete') {
    ...
    const cancelChecked = () => {
      const ids = new Set(gridRows.filter(r => r._row_check).map(r => r.faqId));
      if (!ids.size) { showToast('취소할 행을 선택해주세요.', 'info'); return; }
      for (let i = gridRows.length - 1; i >= 0; i--) {
        const row = gridRows[i];
        if (!ids.has(row.faqId)) { continue; }
    ...
    const handleSave = async () => {
      ...
      for (const r of [...iRows, ...uRows]) {
        if (!r.question) { showToast('질문은 필수 항목입니다.', 'error'); return; }
      }
      const parts = [];
...
<div style="padding:clamp(12px,3vw,24px);">
  ...
  <fo-grid-crud
    ...
    @delete-checked="handleBtnAction('faqs-deleteChecked')" @cancel-checked="handleBtnAction('faqs-cancelChecked')"
    @reorder="handleBtnAction('faqs-reorder')" @cell-change="handleSelectAction('faqs-rowCellChange', $event)">
    <template #row-actions="{ row }">
      <fo-row-cancel-delete :row="row" @cancel="handleSelectAction('faqs-rowCancel', row)" @delete="handleSelectAction('faqs-rowDelete', row)" />
    </template>
  </fo-grid-crud>
</div>
<!-- ===== □. 목록 영역 =================================================== -->
