...
window.Prod01List = {
  ...
  setup(props) {
    ...
    const handleBtnAction = (cmd, param = {}) => {
      console.log(' ■■ Prod01List.js : handleBtnAction -> ', cmd, param);
      // 페이지 이동: 홈
      if (cmd === 'page-goHome') {
    ...
    const handleSelectAction = (cmd, param = {}) => {
      console.log(' ■■ Prod01List.js : handleSelectAction -> ', cmd, param);
      // 카테고리 토글
    ...
    const handleLoadProds = async () => {
      ...
      try {
        ...
        allProds.splice(0, allProds.length, ...(d.pageList || []).map(p => assignImage({
          ...p,
