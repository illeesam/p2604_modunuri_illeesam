    ...
    <div>
      ...
      <div v-if="cfPost.files && cfPost.files.length" style="margin-bottom:36px;">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:10px;">
          <a v-for="f in cfPost.files" :key="f.blogImgId" :href="f.imgUrl" target="_blank" rel="noopener"
            ...
            <span>📎</span>
            <span>{{ f.imgAltText || f.imgUrl }}</span>
          </a>
        </div>
      </div>
      <!-- -- 댓글 --------------------------------------------------------- -->
      <div style="margin-bottom:40px;">
        <h3 style="font-size:1rem;font-weight:700;color:var(--text-primary);margin-bottom:20px;padding-bottom:10px;border-bottom:2px solid var(--blue);">
          댓글
          <span style="color:var(--blue);">({{ cfAllComments.length }})</span>
        </h3>
        ...
        <div style="margin-top:24px;">
          ...
          <div style="display:flex;gap:10px;">
            ...
            <button class="btn-blue" @click="addComment" style="padding:11px 20px;font-size:0.85rem;white-space:nowrap;border-radius:4px;">
              등록
            </button>
          </div>
        </div>
