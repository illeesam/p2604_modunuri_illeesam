/* ShopJoy - Sample02: 상품 관리 CRUD Grid */
window.Sample02 = {
  name: 'Sample02',
  setup() {
    ...
    const makeRow = d => ({ ...d, _row_status: 'N', _row_check: false, _orig: { productNm: d.productNm, category: d.category, price: d.price, stock: d.stock, status: d.status } });
    ...
    onMounted(async () => {
      try { const r = await window.axiosApi.get('xs/sample02.json'); (r.data || []).forEach(d => allData.push(d)); loadGrid(); }
    ...
    const cancelChecked = () => { const ids = new Set(gridRows.filter(r => r._row_check).map(r => r.productId)); if (!ids.size) { showToast('취소할 행을 선택해주세요.', 'info'); return; } for (let i = gridRows.length - 1; i >= 0; i--) { const row = gridRows[i]; if (!ids.has(row.productId)) continue; if (row._row_status === 'N') continue; if (row._row_status === 'I') gridRows.splice(i, 1); else { if (row._orig) EDIT_FIELDS.forEach(f => { row[f] = row._orig[f]; }); row._row_status = 'N'; } } };
...
<div style="padding:16px;">
  ...
  <div style="font-size:16px;font-weight:700;margin-bottom:12px;">02. 상품 관리 <span style="font-size:12px;font-weight:400;color:#888;margin-left:8px;">CRUD Grid 예제</span></div>

  <div style="background:#fff;border:1px solid #e0e0e0;border-radius:8px;padding:12px 16px;margin-bottom:8px;">
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
      <input v-model="searchKw" placeholder="상품명 검색" @keyup.enter="onSearch" style="font-size:12px;padding:5px 10px;border:1px solid #ddd;border-radius:6px;width:180px;outline:none;" />
  ...
  <div style="background:#fff;border:1px solid #e0e0e0;border-radius:8px;overflow:hidden;">
    <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 12px;border-bottom:1px solid #f0f0f0;">
      <span style="font-size:12px;font-weight:700;">상품 목록 <span style="color:#e8587a;margin-left:4px;">{{ total }}건</span></span>
    ...
    <div style="overflow-x:auto;">
      <table style="width:100%;border-collapse:collapse;font-size:12px;min-width:720px;">
        ...
        <tbody>
          ...
          <tr v-for="(row,idx) in pagedRows" :key="row.productId"
            ...
            <td style="text-align:right;">
              <input v-model.number="row.stock" type="number" :disabled="row._row_status==='D'" @input="onCellChange(row)"
                style="width:60px;border:1px solid transparent;background:transparent;padding:3px 5px;font-size:12px;text-align:right;border-radius:3px;outline:none;"
                @focus="e=>e.target.style.border='1px solid #93c5fd'" @blur="e=>e.target.style.border='1px solid transparent'" />
            </td>
            <td style="text-align:center;">
