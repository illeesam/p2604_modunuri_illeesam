/* ShopJoy - Sample04: 주문 관리 CRUD Grid */
window.Sample04 = {
  name: 'Sample04',
  setup() {
    ...
    const applied = reactive({ kw: '', payMethod: '', status: '' });

    const allData    = reactive([]);
    const gridRows   = reactive([]);
    let   _tempId    = -1;
    const focusedIdx = ref(null);
    const EDIT_FIELDS = ['memberNm', 'productNm', 'amount', 'payMethod', 'status'];

    const makeRow = d => ({ ...d, _row_status: 'N', _row_check: false, _orig: { memberNm: d.memberNm, productNm: d.productNm, amount: d.amount, payMethod: d.payMethod, status: d.status } });
    ...
    const loadGrid = () => {
      ...
      allData.filter(d => {
        ...
        if (kw && !['memberNm', 'productNm'].some(f => String(d[f] || '').toLowerCase().includes(kw))) return false;
        if (applied.payMethod && d.payMethod !== applied.payMethod) return false;
    ...
    onMounted(async () => {
      try { const r = await window.axiosApi.get('xs/sample04.json'); (r.data || []).forEach(d => allData.push(d)); loadGrid(); }
      catch { showToast('데이터 로딩 실패', 'error'); }
    });

    const onSearch = () => { Object.assign(applied, { kw: searchKw.value, payMethod: searchPayMethod.value, status: searchStatus.value }); loadGrid(); };
    const onReset  = () => { searchKw.value = ''; searchPayMethod.value = ''; searchStatus.value = ''; Object.assign(applied, { kw: '', payMethod: '', status: '' }); loadGrid(); };
    ...
    const addRow = () => {
      ...
      gridRows.splice(at, 0, { orderId: _tempId--, memberNm: '', productNm: '', amount: 0, payMethod: '카드', status: '주문완료', orderDate: '', _row_status: 'I', _row_check: false, _orig: null });
    ...
    const cancelChecked = () => { const ids = new Set(gridRows.filter(r => r._row_check).map(r => r.orderId)); if (!ids.size) { showToast('취소할 행을 선택해주세요.', 'info'); return; } for (let i = gridRows.length - 1; i >= 0; i--) { const row = gridRows[i]; if (!ids.has(row.orderId)) continue; if (row._row_status === 'N') continue; if (row._row_status === 'I') gridRows.splice(i, 1); else { if (row._orig) EDIT_FIELDS.forEach(f => { row[f] = row._orig[f]; }); row._row_status = 'N'; } } };
    ...
    const doSave = async () => {
      ...
      for (const r of [...iRows, ...uRows]) { if (!r.memberNm || !r.productNm) { showToast('주문자명, 상품명은 필수 항목입니다.', 'error'); return; } }
      ...
      dRows.forEach(r => { const i = allData.findIndex(d => d.orderId === r.orderId); if (i !== -1) allData.splice(i, 1); });
      uRows.forEach(r => { const i = allData.findIndex(d => d.orderId === r.orderId); if (i !== -1) EDIT_FIELDS.forEach(f => { allData[i][f] = r[f]; }); });
      let nid = Math.max(...allData.map(d => d.orderId), 0);
      iRows.forEach(r => { allData.push({ orderId: ++nid, memberNm: r.memberNm, productNm: r.productNm, amount: r.amount, payMethod: r.payMethod, status: r.status, orderDate: new Date().toISOString().slice(0, 10) }); });
    ...
    return {
      toast, searchKw, searchPayMethod, searchStatus, onSearch, onReset,
...
<div style="padding:16px;">
  ...
  <div style="font-size:16px;font-weight:700;margin-bottom:12px;">04. 주문 관리 <span style="font-size:12px;font-weight:400;color:#888;margin-left:8px;">CRUD Grid 예제</span></div>

  <div style="background:#fff;border:1px solid #e0e0e0;border-radius:8px;padding:12px 16px;margin-bottom:8px;">
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
      <input v-model="searchKw" placeholder="주문자명 / 상품명 검색" @keyup.enter="onSearch" style="font-size:12px;padding:5px 10px;border:1px solid #ddd;border-radius:6px;width:200px;outline:none;" />
      <select v-model="searchPayMethod" style="font-size:12px;padding:5px 8px;border:1px solid #ddd;border-radius:6px;">
        <option value="">결제수단 전체</option><option>카드</option><option>무통장</option><option>포인트</option>
      </select>
      <select v-model="searchStatus" style="font-size:12px;padding:5px 8px;border:1px solid #ddd;border-radius:6px;">
        <option value="">상태 전체</option><option>주문완료</option><option>배송중</option><option>배송완료</option><option>취소</option><option>반품</option>
      </select>
  ...
  <div style="background:#fff;border:1px solid #e0e0e0;border-radius:8px;overflow:hidden;">
    <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 12px;border-bottom:1px solid #f0f0f0;">
      <span style="font-size:12px;font-weight:700;">주문 목록 <span style="color:#e8587a;margin-left:4px;">{{ total }}건</span></span>
      ...
      <table style="width:100%;border-collapse:collapse;font-size:12px;min-width:780px;">
        <thead>
          <tr style="background:#f8f9fa;border-bottom:2px solid #e0e0e0;">
            ...
            <th style="padding:7px;text-align:left;font-weight:600;color:#555;">주문자명</th>
            <th style="padding:7px;text-align:left;font-weight:600;color:#555;">상품명</th>
            <th style="width:90px;padding:7px;text-align:right;font-weight:600;color:#555;">금액</th>
            <th style="width:80px;padding:7px;text-align:center;font-weight:600;color:#555;">결제수단</th>
            <th style="width:88px;padding:7px;text-align:center;font-weight:600;color:#555;">주문상태</th>
            <th style="width:96px;padding:7px;text-align:center;font-weight:600;color:#555;">주문일</th>
        ...
        <tbody>
          ...
          <tr v-for="(row,idx) in pagedRows" :key="row.orderId"
            ...
            <td style="text-align:center;color:#999;font-size:11px;">{{ row.orderId > 0 ? row.orderId : 'NEW' }}</td>
            ...
            <td><input v-model="row.productNm" :disabled="row._row_status==='D'" @input="onCellChange(row)"
              style="width:100%;border:1px solid transparent;background:transparent;padding:3px 5px;font-size:12px;border-radius:3px;outline:none;"
              @focus="e=>e.target.style.border='1px solid #93c5fd'" @blur="e=>e.target.style.border='1px solid transparent'" /></td>
            <td style="text-align:right;">
              <input v-model.number="row.amount" type="number" :disabled="row._row_status==='D'" @input="onCellChange(row)"
            ...
            <td style="text-align:center;">
              <select v-model="row.payMethod" :disabled="row._row_status==='D'" @change="onCellChange(row)" style="font-size:11px;padding:2px 4px;border:1px solid #ddd;border-radius:4px;background:#fff;">
                <option>카드</option><option>무통장</option><option>포인트</option>
              </select>
            </td>
            <td style="text-align:center;">
              <select v-model="row.status" :disabled="row._row_status==='D'" @change="onCellChange(row)" style="font-size:11px;padding:2px 4px;border:1px solid #ddd;border-radius:4px;background:#fff;">
                <option>주문완료</option><option>배송중</option><option>배송완료</option><option>취소</option><option>반품</option>
              </select>
            </td>
            <td style="text-align:center;color:#999;font-size:11px;">{{ row.orderDate }}</td>
