/* ShopJoy - Sample07: API 요청 관리 CRUD Grid */
window.Sample07 = {
  name: 'Sample07',
  setup() {
    ...
    const applied = reactive({ kw: '', method: '', status: '' });

    const allData    = reactive([]);
    const gridRows   = reactive([]);
    let   _tempId    = -1;
    const focusedIdx = ref(null);
    const EDIT_FIELDS = ['reqNm', 'method', 'url', 'group', 'status'];

    const makeRow = d => ({ ...d, _row_status: 'N', _row_check: false, _orig: { reqNm: d.reqNm, method: d.method, url: d.url, group: d.group, status: d.status } });
    ...
    const methodBadge = m => METHOD_BADGE[m] || 'background:#f0f0f0;color:#666;';

    const loadGrid = () => {
      gridRows.splice(0); focusedIdx.value = null; pager.page = 1;
      allData.filter(d => {
        const kw = applied.kw.toLowerCase();
        if (kw && !['reqNm', 'url'].some(f => String(d[f] || '').toLowerCase().includes(kw))) return false;
        if (applied.method && d.method !== applied.method) return false;
    ...
    onMounted(async () => {
      try { const r = await window.axiosApi.get('xs/sample07.json'); (r.data || []).forEach(d => allData.push(d)); loadGrid(); }
      catch { showToast('데이터 로딩 실패', 'error'); }
    });

    const onSearch = () => { Object.assign(applied, { kw: searchKw.value, method: searchMethod.value, status: searchStatus.value }); loadGrid(); };
    const onReset  = () => { searchKw.value = ''; searchMethod.value = ''; searchStatus.value = ''; Object.assign(applied, { kw: '', method: '', status: '' }); loadGrid(); };
    ...
    const addRow = () => {
      ...
      gridRows.splice(at, 0, { reqId: _tempId--, reqNm: '', method: 'GET', url: '', group: '', status: '활성', regDate: '', _row_status: 'I', _row_check: false, _orig: null });
    ...
    const cancelChecked = () => { const ids = new Set(gridRows.filter(r => r._row_check).map(r => r.reqId)); if (!ids.size) { showToast('취소할 행을 선택해주세요.', 'info'); return; } for (let i = gridRows.length - 1; i >= 0; i--) { const row = gridRows[i]; if (!ids.has(row.reqId)) continue; if (row._row_status === 'N') continue; if (row._row_status === 'I') gridRows.splice(i, 1); else { if (row._orig) EDIT_FIELDS.forEach(f => { row[f] = row._orig[f]; }); row._row_status = 'N'; } } };
    ...
    const doSave = async () => {
      ...
      for (const r of [...iRows, ...uRows]) { if (!r.reqNm || !r.url) { showToast('요청명, URL은 필수 항목입니다.', 'error'); return; } }
      ...
      dRows.forEach(r => { const i = allData.findIndex(d => d.reqId === r.reqId); if (i !== -1) allData.splice(i, 1); });
      uRows.forEach(r => { const i = allData.findIndex(d => d.reqId === r.reqId); if (i !== -1) EDIT_FIELDS.forEach(f => { allData[i][f] = r[f]; }); });
      let nid = Math.max(...allData.map(d => d.reqId), 0);
      iRows.forEach(r => { allData.push({ reqId: ++nid, reqNm: r.reqNm, method: r.method, url: r.url, group: r.group, status: r.status, regDate: new Date().toISOString().slice(0, 10) }); });
    ...
    return {
      toast, searchKw, searchMethod, searchStatus, onSearch, onReset,
      ...
      checkAll, toggleCheckAll, statusBadge, rowBg, methodBadge,
    };
  },
  template: /* html */`
<div style="padding:16px;">
  ...
  <div style="font-size:16px;font-weight:700;margin-bottom:12px;">07. API 요청 관리 (Postman) <span style="font-size:12px;font-weight:400;color:#888;margin-left:8px;">CRUD Grid 예제</span></div>

  <div style="background:#fff;border:1px solid #e0e0e0;border-radius:8px;padding:12px 16px;margin-bottom:8px;">
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
      <input v-model="searchKw" placeholder="요청명 / URL 검색" @keyup.enter="onSearch" style="font-size:12px;padding:5px 10px;border:1px solid #ddd;border-radius:6px;width:200px;outline:none;" />
      <select v-model="searchMethod" style="font-size:12px;padding:5px 8px;border:1px solid #ddd;border-radius:6px;">
        <option value="">메서드 전체</option><option>GET</option><option>POST</option><option>PUT</option><option>PATCH</option><option>DELETE</option>
      </select>
  ...
  <div style="background:#fff;border:1px solid #e0e0e0;border-radius:8px;overflow:hidden;">
    <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 12px;border-bottom:1px solid #f0f0f0;">
      <span style="font-size:12px;font-weight:700;">API 요청 목록 <span style="color:#e8587a;margin-left:4px;">{{ total }}건</span></span>
      ...
      <table style="width:100%;border-collapse:collapse;font-size:12px;min-width:840px;">
        <thead>
          <tr style="background:#f8f9fa;border-bottom:2px solid #e0e0e0;">
            ...
            <th style="padding:7px;text-align:left;font-weight:600;color:#555;">요청명</th>
            <th style="width:72px;padding:7px;text-align:center;font-weight:600;color:#555;">메서드</th>
            <th style="padding:7px;text-align:left;font-weight:600;color:#555;">URL</th>
            <th style="width:100px;padding:7px;text-align:left;font-weight:600;color:#555;">그룹</th>
            <th style="width:68px;padding:7px;text-align:center;font-weight:600;color:#555;">활성여부</th>
        ...
        <tbody>
          ...
          <tr v-for="(row,idx) in pagedRows" :key="row.reqId"
            ...
            <td style="text-align:center;color:#999;font-size:11px;">{{ row.reqId > 0 ? row.reqId : 'NEW' }}</td>
            ...
            <td><input v-model="row.reqNm" :disabled="row._row_status==='D'" @input="onCellChange(row)"
            ...
            <td style="text-align:center;">
              <select v-model="row.method" :disabled="row._row_status==='D'" @change="onCellChange(row)"
                style="font-size:11px;padding:2px 4px;border:1px solid #ddd;border-radius:4px;background:#fff;font-weight:700;"
                :style="methodBadge(row.method)">
                <option>GET</option><option>POST</option><option>PUT</option><option>PATCH</option><option>DELETE</option>
              </select>
            </td>
            <td><input v-model="row.url" :disabled="row._row_status==='D'" @input="onCellChange(row)"
              style="width:100%;border:1px solid transparent;background:transparent;padding:3px 5px;font-size:12px;border-radius:3px;outline:none;font-family:monospace;"
              @focus="e=>e.target.style.border='1px solid #93c5fd'" @blur="e=>e.target.style.border='1px solid transparent'" /></td>
            <td><input v-model="row.group" :disabled="row._row_status==='D'" @input="onCellChange(row)"
              style="width:100%;border:1px solid transparent;background:transparent;padding:3px 5px;font-size:12px;border-radius:3px;outline:none;"
              @focus="e=>e.target.style.border='1px solid #93c5fd'" @blur="e=>e.target.style.border='1px solid transparent'" /></td>
            <td style="text-align:center;">
