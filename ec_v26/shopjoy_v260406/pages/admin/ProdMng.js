...
window.ProdMng = {
  ...
  setup(props) {
    ...
    return { searchKw, searchCate, searchStatus, pager, PAGE_SIZES, filtered, total, totalPages, pageList, pageNums, categories, statusBadge, onSearch, setPage, onSizeChange, doDelete };
  },
  template: /* html */`
<div>
  ...
  <div class="card">
    <div class="toolbar">
      ...
      <button class="btn btn-primary btn-sm" @click="navigate('prodDtl', {id:null})">+ 신규</button>
    </div>
    <table class="admin-table">
      ...
      <tbody>
        ...
        <tr v-for="p in pageList" :key="p.productId">
          <td>{{ p.productId }}</td>
          <td><span class="title-link" @click="navigate('prodDtl',{id:p.productId})">{{ p.productName }}</span></td>
          <td>{{ p.category }}</td>
          ...
          <td><div class="actions">
            <button class="btn btn-blue btn-sm" @click="navigate('prodDtl',{id:p.productId})">수정</button>
