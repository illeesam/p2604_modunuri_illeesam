...
window.CacheMng = {
  ...
  setup(props) {
    ...
    return { searchKw, searchType, pager, PAGE_SIZES, filtered, total, totalPages, pageList, pageNums, typeBadge, onSearch, setPage, onSizeChange, doDelete };
  },
  template: /* html */`
<div>
  ...
  <div class="card">
    <div class="toolbar">
      ...
      <button class="btn btn-primary btn-sm" @click="navigate('cacheDtl', {id:null})">+ 신규</button>
    </div>
    <table class="admin-table">
      <thead><tr>
        <th>ID</th><th>회원</th><th>일시</th><th>유형</th><th>금액</th><th>잔액</th><th>내용</th><th style="text-align:right">관리</th>
      </tr></thead>
      <tbody>
        <tr v-if="pageList.length===0"><td colspan="8" style="text-align:center;color:#999;padding:30px;">데이터가 없습니다.</td></tr>
        <tr v-for="c in pageList" :key="c.cacheId">
          <td>{{ c.cacheId }}</td>
          ...
          <td><span class="title-link" @click="navigate('cacheDtl',{id:c.cacheId})">{{ c.desc }}</span></td>
          <td><div class="actions">
            <button class="btn btn-blue btn-sm" @click="navigate('cacheDtl',{id:c.cacheId})">수정</button>
