  ...
  <div class="card">
    ...
    <div v-show="topTab==='info'">
      ...
      <div class="form-actions">
        ...
        <button class="btn btn-secondary" @click="navigate('prodMng')">취소</button>
      </div>
    </div>
      ...
      <table class="admin-table" v-if="relatedProducts.length" style="margin-top:12px;">
        ...
        <tbody>
          <tr v-for="p in relatedProducts" :key="p.productId">
            ...
            <td>{{ p.stock }}개</td>
            <td>{{ p.status }}</td>
            <td><button class="btn btn-blue btn-sm" @click="navigate('prodDtl',{id:p.productId})">수정</button></td>
          </tr>
        </tbody>
      </table>
