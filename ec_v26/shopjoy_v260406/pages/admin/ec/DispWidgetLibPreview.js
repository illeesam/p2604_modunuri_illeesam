...
/* ── 위젯라이브러리 경로 트리 (인라인, 구 DispX05WidgetLib) ── */
const _WidgetLibTree = {
...
window.EcDispWidgetLibPreview = {
  ...
  setup(props) {
    ...
    const WIDGET_ICONS = {
      ...
      'info_card':'ℹ️','popup':'💬','file':'📎','file_list':'📁',
    ...
    const filteredLibs = computed(() => {
      ...
      return (props.adminData.widgetLibs || []).filter(lib => {
...
<div>
  ...
  <!-- 조회 조건 -->
  <div class="card" style="padding:10px 18px;margin-bottom:0;border-radius:8px 8px 0 0;border-bottom:none;">
  ...
  <!-- 2단 레이아웃 -->
  <div style="display:flex;gap:0;height:calc(100vh - 204px);min-height:500px;border:1px solid #e8e8e8;border-radius:0 0 8px 8px;overflow:hidden;background:#fff;">

    <!-- 왼쪽: 트리 -->
    <div style="width:378px;flex-shrink:0;display:flex;flex-direction:column;border-right:1px solid #f0f0f0;">
  ...
  `,
  components: {
    DispX05WidgetLib: _WidgetLibTree,
