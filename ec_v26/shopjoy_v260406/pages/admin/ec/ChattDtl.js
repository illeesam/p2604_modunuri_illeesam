    ...
    <div v-show="tab==='chat'">
      <template v-if="chat">
        ...
        <div class="form-actions">
          <button class="btn btn-secondary" @click="navigate('ecChattMng')">목록으로</button>
        </div>
      </template>
      ...
      <table class="admin-table" v-if="memberChats.length">
        ...
        <tbody>
          <tr v-for="c in memberChats" :key="c.chatId">
            ...
            <td>{{ c.date }}</td>
            <td><button class="btn btn-blue btn-sm" @click="navigate('ecChattDtl',{id:c.chatId})">상세</button></td>
          </tr>
        </tbody>
      </table>
          ...
          <table class="admin-table" v-if="userChats.length">
            ...
            <tbody>
              <tr v-for="c in userChats" :key="c.chatId">
                ...
                <td style="max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ c.lastMsg || '-' }}</td>
                <td>{{ c.date }}</td>
                <td><button class="btn btn-blue btn-sm" @click="navigate('ecChattDtl',{id:c.chatId})">보기</button></td>
              </tr>
            </tbody>
          </table>
