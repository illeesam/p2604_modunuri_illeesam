...
window.CategoryMng = {
  ...
  setup(props) {
    ...
    const filtered = computed(() => props.adminData.categories.filter(c => {
      ...
      if (searchDepth.value && String(c.depth) !== searchDepth.value) return false;
    ...
    return { searchDateRange, DATE_RANGE_OPTIONS, siteName, searchKw, searchStatus, searchDepth, pager, PAGE_SIZES, filtered, total, totalPages, pageList, pageNums, onSearch, setPage, onSizeChange, getParentName, depthLabel, depthBadge, statusBadge, doDelete, selectedId, detailEditId, loadDetail, openNew, closeDetail, inlineNavigate };
  },
  template: /* html */`
<div>
  ...
  <div class="card">
    <div class="search-bar">
      <input v-model="searchKw" placeholder="카테고리명 검색" @keyup.enter="onSearch" />
      <select v-model="searchDepth" @change="onSearch">
