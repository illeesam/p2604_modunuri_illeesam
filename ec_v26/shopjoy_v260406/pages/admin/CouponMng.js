...
window.CouponMng = {
  ...
  setup(props) {
    ...
    return { searchKw, searchStatus, pager, PAGE_SIZES, filtered, total, totalPages, pageList, pageNums, discountLabel, statusBadge, onSearch, setPage, onSizeChange, doDelete };
  },
  template: /* html */`
<div>
  ...
  <div class="card">
    <div class="toolbar">
      ...
      <button class="btn btn-primary btn-sm" @click="navigate('couponDtl', {id:null})">+ 신규</button>
    </div>
    <table class="admin-table">
      <thead><tr>
        <th>ID</th><th>쿠폰명</th><th>코드</th><th>할인</th><th>최소주문</th><th>발급대상</th><th>발급/사용</th><th>만료일</th><th>상태</th><th style="text-align:right">관리</th>
      </tr></thead>
      <tbody>
        <tr v-if="pageList.length===0"><td colspan="10" style="text-align:center;color:#999;padding:30px;">데이터가 없습니다.</td></tr>
        <tr v-for="c in pageList" :key="c.couponId">
          <td>{{ c.couponId }}</td>
          <td><span class="title-link" @click="navigate('couponDtl',{id:c.couponId})">{{ c.name }}</span></td>
          ...
          <td><div class="actions">
            <button class="btn btn-blue btn-sm" @click="navigate('couponDtl',{id:c.couponId})">수정</button>
