...
window.SyCodeDtl = {
  ...
  setup(props) {
    ...
    const handleSave = async () => {
      ...
      try {
        const res = await (cfIsNew.value ? window.boApi.post(`/bo/sy/code/${form.codeId}`, { ...form }, coUtil.apiHdr('코드관리', '등록')) : window.boApi.put(`/bo/sy/code/${form.codeId}`, { ...form }, coUtil.apiHdr('코드관리', '저장')));
