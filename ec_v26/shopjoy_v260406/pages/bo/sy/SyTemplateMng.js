...
window.SyTemplateMng = {
  ...
  setup(props) {
    ...
    const cfTree = computed(() => window.boUtil.buildPathTree('sy_template'));
...
const pager = reactive({ pageType: 'PAGE', pageNo: 1, pageSize: 10, pageTotalCount: 0, pageTotalPage: 1, pageSizes: [5, 10, 20, 30, 50, 100, 200, 500], pageCond: {} });
    ...
    const handleDelete = async (t) => {
      ...
      try {
        const res = await window.boApi.delete(`/bo/sy/template/${t.templateId}`, coUtil.apiHdr('템플릿관리', '삭제'));
    ...
    const exportExcel = () => window.boUtil.exportCsv(templates, [{label:'ID',key:'templateId'},{label:'템플릿명',key:'templateNm'},{label:'유형',key:'templateTypeCd'},{label:'사용여부',key:'useYn'},{label:'등록일',key:'regDate'}], '템플릿목록.csv');
