...
const uiState = reactive({ descOpen: false, isPageCodeLoad: false, activeTab: 'log', dateRange: '이번달', dateStart: '', dateEnd: '', searchKw: '', searchResult: '', searchIp: '', searchTokenAction: '' });
    ...
    const cfLogList = computed(() => {
      ...
      LOG_DATES.forEach((dt, i) => {
        const u = boUserList[i % Math.max(1, boUserList.length)];
        if (!u) return;
    ...
    <div v-if="uiState.activeTab==='hist'">
      <table class="bo-table">
        ...
        <tbody>
          ...
          <tr v-if="!cfFiltered.slice((pager.pageNo-1)*pager.pageSize, pager.pageNo*pager.pageSize).length"><td colspan="8" style="text-align:center;color:#999;padding:24px">데이터가 없습니다.</td></tr>
        </tbody>
      </table>
    </div>
