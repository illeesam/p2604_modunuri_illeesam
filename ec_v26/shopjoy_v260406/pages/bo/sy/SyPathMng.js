  ...
  setup(props) {
    ...
    const handleSearchTree = async () => {
      try {
        const res = await window.boApi.get('/bo/sy/path/page', {
          ...
          headers: { 'X-UI-Nm': '표시경로', 'X-Cmd-Nm': '트리조회' }
        });
    ...
    const handleGridSearch = async () => {
      try {
        ...
        const res = await window.boApi.get('/bo/sy/path/page', { params, headers: { 'X-UI-Nm': '표시경로', 'X-Cmd-Nm': '조회' } });
    ...
    const deleteRow = async (row) => {
      ...
      if (!ok) return;
      try {
        const res = await window.boApi.delete(`/bo/sy/path/${row.pathId}`, { headers: { 'X-UI-Nm': '표시경로', 'X-Cmd-Nm': '삭제' } });
    ...
    const handleSave = async () => {
      ...
      for (const row of changed) {
        ...
        try {
          const res = isNew
            ? await window.boApi.post('/bo/sy/path', payload, { headers: { 'X-UI-Nm': '표시경로', 'X-Cmd-Nm': '등록' } })
            : await window.boApi.put(`/bo/sy/path/${row.pathId}`, payload, { headers: { 'X-UI-Nm': '표시경로', 'X-Cmd-Nm': '수정' } });
          if (props.setApiRes) props.setApiRes({ ok: true, status: res.status, data: res.data });
        } catch (err) {
          const msg = err.response?.data?.message || err.message || '오류가 발생했습니다.';
          if (props.setApiRes) props.setApiRes({ ok: false, status: err.response?.status, data: err.response?.data, message: err.message });
