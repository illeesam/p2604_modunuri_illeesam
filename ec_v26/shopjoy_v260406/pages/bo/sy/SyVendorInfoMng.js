...
window.SyVendorInfoMng = {
  ...
  setup(props) {
    ...
    /* handleSelectAction — 그리드 행/노드/탭 선택 액션 dispatch (cmd: '{영역명}-기능명'). 5줄 이하 짧은 로직은 인라인 */
    ...
    /* pickVendorRow — 업체 행 선택 → 선택 업체 변경 + 현재 탭 데이터 로드 */
    ...
    /* loadTabData — 선택 업체 기준 탭별 데이터 조회 */
    const loadTabData = async (tabId) => {
      const vendorId = uiState.selectedVendorId;
      if (vendorId == null) { return; }
      try {
        if (tabId === 'brand') {
          tabLoading.brand = true;
          const res = await boApiSvc.syBrand.getPage({ vendorId, pageNo: 1, pageSize: 1000 }, '업체정보', '브랜드조회');
          brands.splice(0, brands.length, ...(res.data?.data?.pageList || []));
        } else if (tabId === 'price') {
          tabLoading.price = true;
          const res = await boApiSvc.pmDiscnt.getPage({ vendorId, pageNo: 1, pageSize: 1000 }, '업체정보', '가격정책조회');
          discnts.splice(0, discnts.length, ...(res.data?.data?.pageList || []));
        } else if (tabId === 'dliv') {
          tabLoading.dliv = true;
          const res = await boApiSvc.pdDlivTmplt.getPage({ vendorId, pageNo: 1, pageSize: 1000 }, '업체정보', '배송템플릿조회');
          dlivTmplts.splice(0, dlivTmplts.length, ...(res.data?.data?.pageList || []));
        } else if (tabId === 'extra') {
          tabLoading.extra = true;
          extras.splice(0, extras.length);   // 부가서비스: 별도 API 연동 전까지 빈 목록
        }
      } catch (err) {
        console.error('[loadTabData]', tabId, err);
      } finally {
        tabLoading.brand = false; tabLoading.price = false; tabLoading.dliv = false; tabLoading.extra = false;
      }
    };
    ...
    /* 3단 탭 정의 (computed 금지 → reactive + getter 카운트) */
    const tabs = reactive([
      { id: 'brand', label: '브랜드',     icon: '🏷', get count() { return brands.length; } },
      { id: 'price', label: '가격정책',   icon: '💰', get count() { return discnts.length; } },
      { id: 'dliv',  label: '배송템플릿', icon: '🚚', get count() { return dlivTmplts.length; } },
...
<div>
  ...
  <bo-grid
    ...
    :loading="uiState.loading" :row-style="fnRowStyle" row-clickable row-actions
  ...
  <div class="card" style="margin-top:16px;">
    ...
    <!-- ===== ■.■. 업체 미선택 안내 ============================================ -->
    <div v-if="uiState.selectedVendorId == null" style="text-align:center;color:#aaa;padding:40px 16px;font-size:13px;">
      위 업체목록에서 업체를 선택하면 브랜드 / 가격정책 / 배송템플릿 / 부가서비스 정보가 표시됩니다.
    </div>
    <!-- ===== ■.■. 탭 컨텐츠 ================================================= -->
    <div v-else>
      <!-- ===== ■.■.■. 브랜드 탭 ============================================= -->
      <bo-grid v-if="uiState.tab==='brand'" bare
        :columns="columns.brandGrid" :rows="brands" row-key="brandId"
        :loading="tabLoading.brand" empty-text="등록된 브랜드가 없습니다." />
      <!-- ===== ■.■.■. 가격정책 탭 =========================================== -->
      <bo-grid v-else-if="uiState.tab==='price'" bare
        :columns="columns.priceGrid" :rows="discnts" row-key="discntId"
        :loading="tabLoading.price" empty-text="등록된 가격정책이 없습니다." />
      <!-- ===== ■.■.■. 배송템플릿 탭 ========================================= -->
      <bo-grid v-else-if="uiState.tab==='dliv'" bare
        :columns="columns.dlivGrid" :rows="dlivTmplts" row-key="dlivTmpltId"
        :loading="tabLoading.dliv" empty-text="등록된 배송템플릿이 없습니다." />
      <!-- ===== ■.■.■. 부가서비스 탭 ========================================= -->
      <div v-else-if="uiState.tab==='extra'" style="text-align:center;color:#aaa;padding:40px 16px;font-size:13px;">
        부가서비스 정보는 준비 중입니다.
      </div>
    </div>
    <!-- ===== □.□. 탭 컨텐츠 ================================================= -->
