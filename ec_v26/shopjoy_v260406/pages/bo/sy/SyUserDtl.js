...
window.SyUserDtl = {
  ...
  setup(props) {
    ...
    const handleSearchList = async (searchType = 'DEFAULT') => {
      ...
      try {
        const res = await window.boApi.get('/bo/sy/user/page', {
          ...
          headers: { 'X-UI-Nm': '사용자상세', 'X-Cmd-Nm': '조회' }
        });
    ...
    const handleSave = async () => {
      ...
      try {
        const res = await (cfIsNew.value ? window.boApi.post(`/bo/sy/user`, { ...form }, { headers: { 'X-UI-Nm': '사용자관리', 'X-Cmd-Nm': '등록' } }) : window.boApi.put(`/bo/sy/user/${form.boUserId}`, { ...form }, { headers: { 'X-UI-Nm': '사용자관리', 'X-Cmd-Nm': '저장' } }));
