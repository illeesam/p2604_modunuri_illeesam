...
window.SyUserDtl = {
  ...
  setup(props) {
    ...
    const baseForm = reactive({                        // 사용자 폼 데이터
    ...
    const handleBtnAction = (cmd, param = {}) => {
      ...
      // 부서 선택 비우기
        ...
        baseForm.deptId = null; baseForm.deptNm = '';
        return;
      } else {
    ...
    const handleSelectAction = (cmd, param = {}) => {
      ...
      if (cmd === 'deptModal-select') {
        baseForm.deptId = param.deptId;
        baseForm.deptNm = param.deptNm;
        deptModal.show = false;
        return;
      } else {
    ...
    const handleLoadDetail = async () => {
      ...
      try {
        ...
        if (d) { Object.assign(baseForm, { ...d, password: '' }); }
        uiState.error = null;
      } catch (err) {
    ...
    const openKakaoPostcode = () => {
      const run = () => {
        new window.daum.Postcode({
          oncomplete(data) {
            baseForm.zipcode = data.zonecode;
            baseForm.address = data.roadAddress || data.jibunAddress;
    ...
    const handleSave = async () => {
      ...
      if (cfIsNew.value && !baseForm.password) { showToast('신규 등록 시 비밀번호는 필수입니다.', 'error'); return; }
      const ok = await showConfirm(cfIsNew.value ? '등록' : '저장', cfIsNew.value ? '등록하시겠습니까?' : '저장하시겠습니까?');
      if (!ok) { return; }
      try {
        const { password, ...rest } = baseForm;
        const body = { ...rest };
        if (password) { body.loginPwdHash = password; }
        const res = await (cfIsNew.value ? boApiSvc.syUser.create(body, '사용자관리', '등록') : boApiSvc.syUser.update(baseForm.userId, body, '사용자관리', '저장'));
    ...
    /* ##### [06] return (템플릿 노출) ############################################## */
    return {
      uiState, codes, baseForm, errors, addrDetailRef, deptModal,             // 상태 / 데이터
      ...
      <template #dept>
        ...
        <div v-else style="display:flex;gap:8px;align-items:center;">
          <div class="form-control" style="flex:1;cursor:pointer;background:#fafafa;display:flex;align-items:center;min-height:36px;"
            ...
            <span v-if="baseForm.deptNm" style="color:#1a1a2e;">
              {{ baseForm.deptNm }}
            </span>
          ...
          <button v-if="baseForm.deptId" type="button" class="btn btn-secondary btn-sm" @click="handleBtnAction('deptModal-clear')">
            ✕
          </button>
        </div>
      </template>
