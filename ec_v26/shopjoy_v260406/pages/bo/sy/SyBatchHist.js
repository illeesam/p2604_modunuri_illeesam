    ...
    const handleSearchData = async (searchType = 'DEFAULT') => {
      ...
      try {
        const [resBatch, resLogs] = await Promise.all([
          window.boApi.get('/bo/sy/batch/page', { params: { pageNo: 1, pageSize: 10000 }, ...coUtil.apiHdr('배치이력', '목록조회') }),
          window.boApi.get('/bo/sy/batch/log/page', { params: { pageNo: 1, pageSize: 10000 }, ...coUtil.apiHdr('배치이력', '목록조회') }),
        ]);
