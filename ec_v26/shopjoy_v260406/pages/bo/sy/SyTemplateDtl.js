...
window.SyTemplateDtl = {
  ...
  setup(props) {
    ...
    const baseForm = reactive({                                   // 템플릿 폼 데이터
    ...
    const cfUseHtmlEditor = computed(() => {
      if (['메일템플릿', '시스템알림'].includes(baseForm.templateTypeCd)) return true;
      const c = baseForm.templateContent || '';
      return /<\s*\w+[^>]*>/.test(c); // HTML 태그 패턴 자동 감지
    });
    const cfNeedSubject   = computed(() => ['메일템플릿', 'MMS템플릿', '시스템알림'].includes(baseForm.templateTypeCd));
    const cfIsLongContent = computed(() => ['MMS템플릿'].includes(baseForm.templateTypeCd));
    ...
    const handleLoadDetail = async () => {
      ...
      try {
        ...
        if (data) { Object.assign(baseForm, { sampleParams: '{}', ...data }); }
        uiState.error = null;
      } catch (err) {
    ...
    const handleSave = async () => {
      ...
      if (baseForm.sampleParams) {
        try { JSON.parse(baseForm.sampleParams); }
      ...
      try {
        const res = await (cfIsNew.value
          ? boApiSvc.syTemplate.create({ ...baseForm }, '템플릿관리', '등록')
          : boApiSvc.syTemplate.update(baseForm.templateId, { ...baseForm }, '템플릿관리', '저장'));
    ...
    /* ##### [06] return (템플릿 노출) ############################################## */
    return {
      uiState, codes, baseForm, errors,                                     // 상태 / 데이터
...
<div>
  ...
  <div class="card">
    ...
    <bo-form-area :columns="baseFormColumns" :form="baseForm" :errors="errors"
      ...
      <template #content>
        <template v-if="cfUseHtmlEditor">
          <div v-if="cfDtlMode" class="form-control" style="height:260px;line-height:1.6;overflow:auto;" v-html="baseForm.templateContent || '<span style=color:#bbb>-</span>'">
          </div>
          <base-html-editor v-else v-model="baseForm.templateContent" height="320px" />
        </template>
        <textarea v-else class="form-control" v-model="baseForm.templateContent"
    ...
    :tmpl="baseForm" :sample-params="baseForm.sampleParams"
    @close="handleBtnAction('previewModal-close')" />
    <!-- ===== □. 미리보기 모달 ================================================= -->
    <!-- ===== ■. 발송하기 모달 ================================================= -->
    <template-send-modal v-if="uiState.sendOpen"
    :tmpl="baseForm" :show-toast="showToast" :show-confirm="showConfirm"
