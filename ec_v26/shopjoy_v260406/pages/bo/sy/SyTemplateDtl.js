...
window.SyTemplateDtl = {
  ...
  setup(props) {
    ...
    /* fnShareUrl — 이 템플릿 상세를 가리키는 독립 새창 딥링크 URL 생성 */
    const fnShareUrl = () => {
      const qs = new URLSearchParams();
      qs.set('page', 'syTemplateDtl');
      qs.set('id', form.templateId);
      qs.set('embed', '1');
    ...
    const handleShareKakao = () => {
      try {
        window.coExtSdk.shareKakao({
          title: `템플릿 ${form.templateId} - ShopJoy BO`,
          description: form.templateNm || '',
    ...
    /* pdfAreaRef — 템플릿 상세 카드 캡처 대상. handleExportPdf — PDF 다운로드(항상 노출) */
    const pdfAreaRef = ref(null);
    const pdfExporting = ref(false);
    const handleExportPdf = async () => {
      pdfExporting.value = true;
      try {
        const filename = coUtil.cofBuildExportFilename(`템플릿상세_${form.templateId}.pdf`);
