...
window.DashboardBoEc03 = {
  ...
  setup() {
    ...
    /* X-View 히트맵 목업 데이터 생성 (실시간 스캐터 시뮬) */
    const buildXviewData = () => {
      ...
      for (let i = 0; i < COUNT; i++) {
        ...
        const rt = Math.random() < 0.75
          ? Math.random() * 500            // 정상 0~500ms
          : Math.random() < 0.7
            ? 500 + Math.random() * 2500   // 느림 500~3000ms
            : 3000 + Math.random() * 5000; // 매우 느림 3000~8000ms
      ...
      return {
        tooltip: {
          ...
          formatter: p => {
            ...
            const level = ['정상','느림','오류'][p.data[2]] || '';
            return hm + '<br/>응답시간: <b>' + p.data[1].toFixed(0) + 'ms</b><br/>상태: ' + level;
          },
        },
        toolbox: {
        ...
        yAxis: {
          ...
          splitLine: { lineStyle:{ color:'#f0f0f0' } },
          /* 임계선: 500ms 경고 / 3000ms 오류 */
        },
        visualMap: {
          show: true, type:'piecewise',
          categories: [0,1,2],
          dimension: 2,
          pieces: [
        ...
        series: [
          ...
          {
            type: 'line',
            data: [[from, 500],[now, 500]],
            lineStyle: { type:'dashed', color:'#f59e0b', width:1.5 },
            symbol: 'none', tooltip:{ show:false }, z: 5,
          },
          /* 임계선 — 3000ms */
          {
            type: 'line',
            data: [[from, 3000],[now, 3000]],
    ...
    onMounted(() => {
      ...
      xviewTimer = setInterval(() => {
        ...
        const err = rt > 5000 && Math.random() < 0.3;
        const next = [...xviewData.value.filter(p => p.t > now - 10*60*1000), { t:now, rt:Math.round(rt), err }];
        xviewData.value = next;
      }, 2000);
    });
  ...
  <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
    <div class="tab-nav" style="margin-bottom:0;flex:1;flex-wrap:wrap;">
      <button v-for="t in TABS" :key="t.key" class="tab-btn"
        :class="{active: uiState.activeTab===t.key ? uiState.tabMode==='tab' : false}"
