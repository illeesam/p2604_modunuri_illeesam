  ...
  setup(props) {
    ...
    const handleLoadData = async () => {
      ...
      try {
        ...
        const baseCalls = [
          window.boApi.get('/bo/sy/user/page',          { params: { pageNo: 1, pageSize: 1000 }, ...coUtil.apiHdr('상품관리', '상세조회') }),
          window.boApi.get('/bo/ec/pd/category/page',   { params: { pageNo: 1, pageSize: 1000 }, ...coUtil.apiHdr('상품관리', '상세조회') }),
        ];
    ...
    const handleSave = async () => {
      ...
      try {
        ...
        const res = await (cfIsNew.value
          ? window.boApi.post(`/bo/ec/pd/prod`, payload, { ...coUtil.apiHdr('상품관리', '등록') })
          : window.boApi.put(`/bo/ec/pd/prod/${form.prodId}`, payload, { ...coUtil.apiHdr('상품관리', '저장') }));
