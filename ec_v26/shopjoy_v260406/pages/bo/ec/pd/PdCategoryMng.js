  ...
  setup(props) {
    ...
    const handleGridSearch = async () => {
      try {
        ...
        const res = await window.boApi.get('/bo/ec/pd/category/page', { params, ...coUtil.apiHdr('카테고리관리', '목록조회') });
        const list = res.data?.data?.pageList || res.data?.data?.list || [];
        gridRows.splice(0);
...
const EDIT_FIELDS = ['categoryNm', 'parentCategoryId', 'sortOrd', 'categoryDesc', 'categoryStatusCd'];
    ...
    const deleteRow = async (idx) => {
      ...
      row._row_status = 'D';
      try {
        const res = await window.boApi.delete(`/bo/ec/pd/category/${row.categoryId}`, { ...coUtil.apiHdr('카테고리관리', '삭제') });
    ...
    const deleteRows = async () => {
      ...
      for (let i = idxs.length - 1; i >= 0; i--) {
        ...
        try {
          await window.boApi.delete(`/bo/ec/pd/category/${row.categoryId}`, { headers: { 'X-UI-Nm': '카테고리관리', 'X-Cmd-Nm': '삭제' } });
          gridRows.splice(idx, 1);
    ...
    const handleSave = async () => {
      ...
      for (const row of changed) {
        ...
        try {
          const res = isNew
            ? await window.boApi.post('/bo/ec/pd/category', payload, { ...coUtil.apiHdr('카테고리관리', '저장') })
            : await window.boApi.put(`/bo/ec/pd/category/${row.categoryId}`, payload, { ...coUtil.apiHdr('카테고리관리', '저장') });
          if (props.setApiRes) props.setApiRes({ ok: true, status: res.status, data: res.data });
          row._row_status = null;
        } catch (err) {
