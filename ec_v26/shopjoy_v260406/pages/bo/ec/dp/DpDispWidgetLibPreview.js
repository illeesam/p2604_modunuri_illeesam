  ...
  setup(props) {
    ...
    const handleBtnAction = (cmd, param = {}) => {
      ...
      // 실제컨텐츠 토글
        ...
        uiState.showRealContent = !uiState.showRealContent;
        return;
      // 현재 그리드 / 대시보드 초기화
    ...
    const handleSelectAction = (cmd, param = {}) => {
      console.log(' ■■ DpDispWidgetLibPreview.js : handleSelectAction -> ', cmd, param);
      // 트리 노드 토글 (param: key)
      if (cmd === 'pathTree-toggle') {
        return toggleNode(param);
      // 트리 노드 전체 자식 토글 (param: { e, node })
      } else if (cmd === 'pathTree-toggleChildren') {
        return toggleAllChildren(param.e, param.node);
...
window.DpDispWidgetLibPreview = {
  ...
  setup(props) {
    ...
    return {
      ...
      cfPlacedCount, cfDispWidgetTypes,                                              // computed
      VIEWPORT, GRID_TABS, GRID_COLS, today,                                          // 상수
      wIcon, wTypeLabel,                                                              // 헬퍼
      isOpen, allChildrenOpen,                                                        // 헬퍼 (template 사용)
    ...
    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
      ...
      <div style="display:flex;align-items:center;gap:5px;">
        ...
        <select v-model="searchParam.filterStatus" class="form-control" style="width:76px;margin:0;font-size:12px;">
          ...
          <option v-for="o in codes.widget_status_opts" :key="o.value" :value="o.value">
            {{ o.label }}
          </option>
        </select>
      </div>
      ...
      <div style="display:flex;align-items:center;gap:5px;">
        ...
        <select v-model="searchParam.filterAuthReq" class="form-control" style="width:70px;margin:0;font-size:12px;">
          ...
          <option v-for="o in codes.need_yn_opts" :key="o.codeValue" :value="o.codeValue">
            {{ o.codeLabel }}
          </option>
        </select>
      </div>
      ...
      <div style="display:flex;align-items:center;gap:5px;">
        ...
        <select v-model="searchParam.filterAuthGrade" class="form-control" style="width:72px;margin:0;font-size:12px;">
          ...
          <option v-for="g in codes.auth_grade_opts" :key="Math.random()" :value="g">
            {{ g }}↑
          </option>
        </select>
      </div>
      ...
      <div style="display:flex;align-items:center;gap:5px;">
        ...
        <select v-model="searchParam.filterType" class="form-control" style="width:114px;margin:0;font-size:12px;">
          <option v-for="t in cfDispWidgetTypes" :key="t?.value" :value="t.codeValue">
            {{ t.codeLabel }}
          </option>
        </select>
      </div>
      ...
      <button @click="handleBtnAction('searchParam-list')" class="btn btn-primary btn-sm" style="height:30px;padding:0 14px;">
        검색
      </button>
        ...
        <div v-if="isOpen('__root__')" style="padding-left:8px;">
          <div v-for="node in cfTree" :key="node?.label">
            <div @click="handleSelectAction('pathTree-toggle', node.label)"
              ...
              <span v-if="isOpen(node.label)" @click="handleSelectAction('pathTree-toggleChildren', { e: $event, node })"
          ...
          <div :style="{
            ...
            <div :style="{
              ...
              <template v-for="(slot, idx) in cfCurrentSlots" :key="Math.random()">
                <div v-if="!uiState.showRealContent || slot" @dragover="onDragOver($event, idx)" @dragleave="onDragLeave" @drop="onDrop($event, idx)" style="border-radius:8px;transition:all .15s;position:relative;" :style="[ uiState.dragOverIdx===idx ? 'border:2px dashed #1d4ed8;background:#eff6ff;min-height:110px;' : slot ? (uiState.showRealContent ? 'border:none;background:transparent;min-height:0;' : 'border:1px solid #e5e7eb;background:#fff;box-shadow:0 1px 4px rgba(0,0,0,.07);min-height:110px;') : 'border:2px dashed #d1d5db;background:#f9fafb;min-height:60px;', slot && (slot.colSpan||1) > 1 ? { gridColumn: 'span ' + slot.colSpan } : {}, slot && (slot.rowSpan||1) > 1 ? { gridRow: 'span ' + slot.rowSpan } : {}, ]">
                <!-- ===== ■.■.■.■.■.■.■.■.■. 비어있음 ==================================== -->
            ...
            <template v-else-if="slot">
              ...
              <div v-if="uiState.spanPopupIdx===idx" @click.stop
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
                  ...
                  <button @click="handleBtnAction('spanPopup-close')" style="border:none;background:none;cursor:pointer;font-size:13px;color:#9ca3af;padding:0;line-height:1;">
                    ✕
                  </button>
                </div>
                <!-- ===== ■.■.■.■.■.■.■.■.■.■.■. 열(colspan) ========================== -->
                <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px;">
                  ...
                  <button @click="handleSelectAction('slot-spanSet', { idx, axis:'col', delta:+1 })" :disabled="(slot.colSpan||1)>=(GRID_COLS[uiState.previewGrid]||1)"
                          style="width:24px;height:24px;border:1px solid #e5e7eb;border-radius:4px;background:#f9fafb;cursor:pointer;font-size:13px;display:flex;align-items:center;justify-content:center;padding:0;"
...
<div v-else style="flex:1;overflow:auto;padding:16px;">
  <div
    ...
    <!-- ===== ■.■.■.■.■. 빈 상태 ============================================ -->
    <div v-if="!dashItems.length && !uiState.dashDragOver" style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;color:#d1d5db;pointer-events:none;">
  ...
  <div v-if="uiState.dashDragOver && !dashItems.length" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:#1d4ed8;font-size:14px;font-weight:700;pointer-events:none;">
  ▼ 여기에 배치
</div>
<!-- ===== ■.■.■.■.■. 배치된 아이템 ========================================= -->
