...
const _WidgetPreview = {
  ...
  setup(props) {
    ...
    const dashDragOver = Vue.toRef(uiState, 'dashDragOver');
    ...
    const showRealContent = Vue.toRef(uiState, 'showRealContent');
...
window.DpDispWidgetLibPreview = {
  ...
  setup(props) {
    ...
    const handleSearchList = async (searchType = 'DEFAULT') => {
      try {
        const res = await window.boApi.get('/bo/ec/dp/widget-lib/page', { params: { pageNo: 1, pageSize: 10000 }, headers: { 'X-UI-Nm': '위젯라이브러리관리', 'X-Cmd-Nm': '조회' } });
