...
window.DpDispWidgetLibDtl = {
  ...
  setup(props, { emit }) {
    ...
    const handleSave = async () => {
      ...
      try {
        const res = await (isNewLib ? window.boApi.post('/bo/ec/dp/widget-lib', { ...form }, { ...coUtil.apiHdr('전시위젯라이브러리', '등록') }) : window.boApi.put(`/bo/ec/dp/widget-lib/${form.libId}`, { ...form }, { ...coUtil.apiHdr('전시위젯라이브러리', '저장') }));
    ...
    const handleDelete = async () => {
      ...
      try {
        const res = await window.boApi.delete(`/bo/ec/dp/widget-lib/${form.libId}`, { ...coUtil.apiHdr('전시위젯라이브러리', '삭제') });
        if (props.setApiRes) props.setApiRes({ ok: true, status: res.status, data: res.data });
