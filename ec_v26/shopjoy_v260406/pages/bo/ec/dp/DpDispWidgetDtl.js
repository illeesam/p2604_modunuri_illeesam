...
window.DpDispWidgetDtl = {
  ...
  setup(props, { emit }) {
    ...
    const handleSave = async () => {
      ...
      try {
        const res = await (isNewWidget ? window.boApi.post('/bo/ec/dp/widget-lib', { ...form }, { ...apiHdr('전시위젯관리', '등록') }) : window.boApi.put(`/bo/ec/dp/widget-lib/${form.libId}`, { ...form }, { ...apiHdr('전시위젯관리', '저장') }));
    ...
    const handleDelete = async () => {
      ...
      try {
        const res = await window.boApi.delete(`/bo/ec/dp/widget-lib/${form.libId}`, { ...apiHdr('전시위젯관리', '삭제') });
        if (props.setApiRes) props.setApiRes({ ok: true, status: res.status, data: res.data });
