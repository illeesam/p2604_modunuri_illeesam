...
window.DpDispWidgetDtl = {
  ...
  setup(props, { emit }) {
    ...
    const handleSearchList = async (searchType = 'DEFAULT') => {
      try {
        const res = await window.boApi.get('/bo/ec/dp/widget-lib/page', { params: { pageNo: 1, pageSize: 10000 }, headers: { 'X-UI-Nm': '전시위젯상세', 'X-Cmd-Nm': '조회' } });
    ...
    const handleSave = async () => {
      ...
      try {
        const res = await (isNewWidget ? window.boApi.post('/bo/ec/dp/widget-lib', { ...form }, { headers: { 'X-UI-Nm': '전시위젯관리', 'X-Cmd-Nm': '등록' } }) : window.boApi.put(`/bo/ec/dp/widget-lib/${form.libId}`, { ...form }, { headers: { 'X-UI-Nm': '전시위젯관리', 'X-Cmd-Nm': '저장' } }));
    ...
    const handleDelete = async () => {
      ...
      try {
        const res = await window.boApi.delete(`/bo/ec/dp/widget-lib/${form.libId}`, { headers: { 'X-UI-Nm': '전시위젯관리', 'X-Cmd-Nm': '삭제' } });
        if (props.setApiRes) props.setApiRes({ ok: true, status: res.status, data: res.data });
