...
window.DpDispRelationMng = {
  ...
  setup(props) {
    ...
    const handleSearchData = async (searchType = 'DEFAULT') => {
      try {
        const res = await window.boApi.get('/bo/ec/dp/ui/page', { params: { pageNo: 1, pageSize: 10000 }, ...apiHdr('전시연관관리', '조회') });
  ...
  const searchParamOrg = reactive({
    ...
    try {
      ...
      const res = await window.boApi.get('/bo/ec/resource/page', { params, ...apiHdr('전시연관관리', '조회') });
