...
window.DpDispUiDtl = {
  ...
  setup(props) {
    ...
    const fnLoadCodes = async () => {
      ...
      await s.saLoadCodes(['DEVICE_TYPE_CD'], {compNm: 'DpDispUiDtl'});
    ...
    /* fnShareUrl — 이 전시UI 상세를 가리키는 독립 새창 딥링크 URL 생성 */
    const fnShareUrl = () => {
      const qs = new URLSearchParams();
      qs.set('page', 'dpDispUiDtl');
      qs.set('id', baseForm.uiId);
      qs.set('embed', '1');
    ...
    const handleShareKakao = () => {
      try {
        window.coExtSdk.shareKakao({
          title: `전시UI ${baseForm.uiId} - ShopJoy BO`,
          description: baseForm.uiDesc || baseForm.uiNm || '',
    ...
    /* pdfAreaRef — 전시UI 상세 카드 캡처 대상. handleExportPdf — PDF 다운로드(상세보기 모드 전용) */
    const pdfAreaRef = ref(null);
    const pdfExporting = ref(false);
    const handleExportPdf = async () => {
      pdfExporting.value = true;
      try {
        const filename = coUtil.cofBuildExportFilename(`전시UI상세_${baseForm.uiId}.pdf`);
    ...
    return {
      ...
      modals,   // 모달 표시 상태 모음
      columns,
      uiState, codes, baseForm, errors, areas, handleBtnAction, fnCallbackModal,
      cfIsNew, cfReadonly,
