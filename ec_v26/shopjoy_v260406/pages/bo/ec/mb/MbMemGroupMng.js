...
const isAppReady = computed(() => {
  ...
  const searchParamOrg = reactive({
    ...
    const handleDeleteRow    = async (idx) => {
      ...
      try {
        const res = await window.boApi.delete(`/bo/ec/mb/member-group/${row.groupId}`, { headers: { 'X-UI-Nm': '회원그룹관리', 'X-Cmd-Nm': '삭제' } });
    ...
    const handleSaveAll = async () => {
      ...
      for (const row of changed) {
        ...
        try {
          const res = await (isNewRow ? window.boApi.post(`/bo/ec/mb/member-group/${row.groupId}`, { ...row }, { headers: { 'X-UI-Nm': '회원그룹관리', 'X-Cmd-Nm': '등록' } }) : window.boApi.put(`/bo/ec/mb/member-group/${row.groupId}`, { ...row }, { headers: { 'X-UI-Nm': '회원그룹관리', 'X-Cmd-Nm': '저장' } }));
          if (props.setApiRes) props.setApiRes({ ok: true, status: res.status, data: res.data });
