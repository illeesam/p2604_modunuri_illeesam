...
window.PmSaveDtl = {
  ...
  setup(props) {
    ...
    const handleSearchDetail = async () => {
      ...
      try {
        const res = await window.boApi.get(`/bo/ec/pm/save/${props.editId}`, { headers: { 'X-UI-Nm': '적립금상세', 'X-Cmd-Nm': '상세조회' } });
    ...
    const handleSave = async () => {
      ...
      if (!ok) return;
      try {
        const res = await (cfIsNew.value ? window.boApi.post(`/bo/ec/pm/save`, { ...form }, { headers: { 'X-UI-Nm': '적립금관리', 'X-Cmd-Nm': '등록' } }) : window.boApi.put(`/bo/ec/pm/save/${form.saveId}`, { ...form }, { headers: { 'X-UI-Nm': '적립금관리', 'X-Cmd-Nm': '저장' } }));
