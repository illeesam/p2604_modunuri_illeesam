...
const uiStateDetail = reactive({ selectedId: null, openMode: 'view', reloadTrigger: 0 });
  const searchParam = reactive(_initSearchParam());
    ...
    const gridColumns = [
      { key: 'giftNm',       label: '사은품명', sortKey: 'nm' },
      { key: 'giftTypeCd',   label: '조건유형' },
      { key: 'condVal',      label: '조건값' },
      { key: 'giftStock',    label: '재고' },
      { key: 'startDate',    label: '시작일', sortKey: 'reg' },
      { key: 'endDate',      label: '종료일' },
      { key: 'giftStatusCd', label: '상태' },
...
<div>
  ...
  <div class="card">
    ...
    <bo-grid v-if="tabMode==='list'" :bare="true"
      ...
      <template #cell-giftNm="{ row: g }"><td><span class="title-link" @click="handleLoadDetail(g.giftId)" :style="selectedId===g.giftId?'color:#e8587a;font-weight:700;':''">{{ g.giftNm }}<span v-if="selectedId===g.giftId" style="font-size:10px;margin-left:3px;">▼</span></span></td></template>
      <template #cell-giftTypeCd="{ row: g }"><td><span class="badge" :class="fnTypeBadge(g.giftTypeCd)">{{ g.giftTypeCd }}</span></td></template>
      <template #cell-condVal="{ row: g }"><td>{{ g.giftTypeCd === '금액조건' ? (g.minOrderAmt||0).toLocaleString() + '원↑' : g.giftTypeCd === '수량조건' ? (g.minOrderQty||0) + '개↑' : '-' }}</td></template>
      <template #cell-giftStock="{ row: g }"><td>{{ (g.giftStock||0).toLocaleString() }}개</td></template>
      <template #cell-giftStatusCd="{ row: g }"><td><span class="badge" :class="fnStatusBadge(g.giftStatusCd)">{{ g.giftStatusCd }}</span></td></template>
