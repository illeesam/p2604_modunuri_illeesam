...
window.PmPlanDtl = {
  ...
  setup(props) {
    ...
    const handleSearchDetail = async () => {
      ...
      try {
        const calls = [window.boApi.get('/bo/ec/pd/prod/page', { params: { pageNo: 1, pageSize: 10000 }, ...apiHdr('요금제관리', '조회') })];
        if (!cfIsNew.value) calls.unshift(window.boApi.get(`/bo/ec/pm/plan/${props.editId}`, { ...apiHdr('요금제관리', '상세조회') }));
    ...
    const handleSave = async () => {
      ...
      if (!ok) return;
      try {
        const res = await (cfIsNew.value ? window.boApi.post(`/bo/ec/pm/plan`, form, { ...apiHdr('요금제관리', '등록') }) : window.boApi.put(`/bo/ec/pm/plan/${props.editId}`, form, { ...apiHdr('요금제관리', '저장') }));
