...
window.PmCacheDtl = {
  ...
  setup(props) {
    ...
    const handleSave = async () => {
      ...
      if (!ok) return;
      try {
        const res = await (cfIsNew.value ? boApi.post(`/bo/ec/pm/cache/${form.cacheId}`, { ...form }, { ...coUtil.apiHdr('캐시관리', '등록') }) : boApi.put(`/bo/ec/pm/cache/${form.cacheId}`, { ...form }, { ...coUtil.apiHdr('캐시관리', '저장') }));
