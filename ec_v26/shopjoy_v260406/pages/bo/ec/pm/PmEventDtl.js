...
window.PmEventDtl = {
  ...
  setup(props) {
    ...
    const handleSearchDetail = async () => {
      ...
      try {
        const calls = [window.boApi.get('/bo/ec/pd/prod/page', { params: { pageNo: 1, pageSize: 10000 }, ...apiHdr('이벤트관리', '조회') })];
        if (!cfIsNew.value) calls.unshift(window.boApi.get(`/bo/ec/pm/event/${props.editId}`, { ...apiHdr('이벤트관리', '상세조회') }));
    ...
    const handleSave = async () => {
      ...
      if (!ok) return;
      try {
        const res = await (cfIsNew.value ? window.boApi.post(`/bo/ec/pm/event`, { ...form }, { ...apiHdr('이벤트관리', '등록') }) : window.boApi.put(`/bo/ec/pm/event/${form.eventId}`, { ...form }, { ...apiHdr('이벤트관리', '저장') }));
