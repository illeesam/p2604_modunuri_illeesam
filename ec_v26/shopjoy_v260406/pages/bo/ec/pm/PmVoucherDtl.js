...
window.PmVoucherDtl = {
  ...
  setup(props) {
    ...
    const handleSearchDetail = async () => {
      ...
      try {
        const res = await window.boApi.get(`/bo/ec/pm/voucher/${props.editId}`, { ...coUtil.apiHdr('바우처관리', '상세조회') });
    ...
    const sendSns = async () => {
      ...
      snsModal.show = false;
      try {
        const res = await window.boApi.post(`/bo/ec/pm/voucher/${form.voucherId}/send-sns`, { channel: snsModal.channel, message: uiState.snsMsg }, { ...coUtil.apiHdr('바우처관리', '전송') });
    ...
    const handleSave = async () => {
      ...
      if (!ok) return;
      try {
        const res = await (cfIsNew.value ? window.boApi.post(`/bo/ec/pm/voucher`, { ...form }, { ...coUtil.apiHdr('바우처관리', '등록') }) : window.boApi.put(`/bo/ec/pm/voucher/${form.voucherId}`, { ...form }, { ...coUtil.apiHdr('바우처관리', '저장') }));
        if (props.setApiRes) props.setApiRes({ ok: true, status: res.status, data: res.data });
