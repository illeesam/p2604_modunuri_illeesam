...
window.PmVoucherDtl = {
  ...
  setup(props) {
    ...
    const uiState = reactive({ loading: false, showVendorModal: false, error: null, tab: window._pmVoucherDtlState.tab || 'info', tabMode2: window._pmVoucherDtlState.tabMode || 'tab', previewTab: 'barcode', barcodeContainer: null, qrcodeContainer: null, snsMsg: ''});
...
watch(() => uiState.tab, v => { window._pmVoucherDtlState.tab = v; });
    ...
    /* fnShareUrl — 이 상품권 상세를 가리키는 독립 새창 딥링크 URL 생성 */
    const fnShareUrl = () => {
      const qs = new URLSearchParams();
      qs.set('page', 'pmVoucherDtl');
      qs.set('id', form.voucherId);
      qs.set('embed', '1');
    ...
    const handleShareKakao = () => {
      try {
        window.coExtSdk.shareKakao({
          title: `상품권 ${form.voucherId} - ShopJoy BO`,
          description: form.remark || form.voucherNm || '',
    ...
    const handleExportPdf = async () => {
      ...
      try {
        const filename = coUtil.cofBuildExportFilename(`상품권상세_${form.voucherId}.pdf`);
    ...
    <div class="dtl-pane" v-if="showTab('detail')" style="margin:0;">
      ...
      <div style="padding:20px;">
        ...
        <div style="display:flex;gap:12px;margin-bottom:20px;">
          ...
          <button @click="handleBtnAction('snsModal-open', 'email')" class="btn btn-secondary">📧 이메일</button>
        </div>
      </div>
    </div>
    <!-- ===== □. 상세정보 탭 ================================================== -->
