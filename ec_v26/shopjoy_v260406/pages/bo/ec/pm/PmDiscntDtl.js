...
window.PmDiscntDtl = {
  ...
  setup(props) {
    ...
    const handleSearchDetail = async () => {
      ...
      try {
        const res = await window.boApi.get(`/bo/ec/pm/discnt/${props.editId}`, { ...apiHdr('할인관리', '상세조회') });
    ...
    const handleSave = async () => {
      ...
      if (!ok) return;
      try {
        const res = await (cfIsNew.value ? window.boApi.post(`/bo/ec/pm/discnt/${form.discntId}`, { ...form }, { ...apiHdr('할인관리', '등록') }) : window.boApi.put(`/bo/ec/pm/discnt/${form.discntId}`, { ...form }, { ...apiHdr('할인관리', '저장') }));
