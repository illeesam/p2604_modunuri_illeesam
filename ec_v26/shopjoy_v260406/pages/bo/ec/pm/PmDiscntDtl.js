...
window.PmDiscntDtl = {
  ...
  setup(props) {
    ...
    const handleSearchDetail = async () => {
      ...
      try {
        const res = await window.boApi.get(`/bo/ec/pm/discnt/${props.editId}`, { headers: { 'X-UI-Nm': '할인상세', 'X-Cmd-Nm': '상세조회' } });
    ...
    const handleSave = async () => {
      ...
      if (!ok) return;
      try {
        const res = await (cfIsNew.value ? window.boApi.post(`/bo/ec/pm/discnt/${form.discntId}`, { ...form }, { headers: { 'X-UI-Nm': '할인관리', 'X-Cmd-Nm': '등록' } }) : window.boApi.put(`/bo/ec/pm/discnt/${form.discntId}`, { ...form }, { headers: { 'X-UI-Nm': '할인관리', 'X-Cmd-Nm': '저장' } }));
