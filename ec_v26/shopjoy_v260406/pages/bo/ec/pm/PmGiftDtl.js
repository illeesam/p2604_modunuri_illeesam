...
window.PmGiftDtl = {
  ...
  setup(props) {
    ...
    const handleSearchDetail = async () => {
      ...
      try {
        const res = await window.boApi.get(`/bo/ec/pm/gift/${props.editId}`, { ...coUtil.apiHdr('선물관리', '상세조회') });
    ...
    const handleSave = async () => {
      ...
      if (!ok) return;
      try {
        const res = await (cfIsNew.value ? window.boApi.post(`/bo/ec/pm/gift`, { ...form }, { ...coUtil.apiHdr('선물관리', '등록') }) : window.boApi.put(`/bo/ec/pm/gift/${form.giftId}`, { ...form }, { ...coUtil.apiHdr('선물관리', '저장') }));
