  ...
  <div class="tab-bar-row">
    <div class="tab-nav">
      <button class="tab-btn" :class="{active:botTab==='items'}"   :disabled="tabMode2!=='tab'" @click="botTab='items'">
        ↩ 클레임 항목
        <span class="tab-count">{{ claimItems.length }}</span>
      </button>
      <button class="tab-btn" :class="{active:botTab==='process'}" :disabled="tabMode2!=='tab'" @click="botTab='process'">⚙ 처리 정보</button>
      <button class="tab-btn" :class="{active:botTab==='order'}"   :disabled="tabMode2!=='tab'" @click="botTab='order'">
        🛒 연관 주문
        <span class="tab-count">{{ relatedOrder ? 1 : 0 }}</span>
      </button>
    </div>
  </div>
          ...
          <div style="display:flex;justify-content:space-between;align-items:flex-start;">
            <div>
              ...
              <div style="font-size:13px;color:#555;line-height:2;">
                ...
                <span class="ref-link" style="margin:0 6px;" @click="showRefModal('member', relatedOrder.userId)">
                  {{ relatedOrder.userNm }}
                </span>
                <span style="color:#888;">주문일</span>
                <b style="margin-left:4px;">{{ relatedOrder.orderDate }}</b>
                <br/>
                <span style="color:#888;">상품</span>
                <b style="margin-left:4px;">{{ relatedOrder.prodNm }}</b>
                <br/>
                <span style="color:#888;">금액</span>
                <b style="margin-left:4px;color:#e8587a;">{{ (relatedOrder.totalPrice||0).toLocaleString() }}원</b>
                &nbsp;·&nbsp;
                <span style="color:#888;">결제</span>
                <b style="margin-left:4px;">{{ relatedOrder.payMethodCd }}</b>
                <br/>
                <span style="color:#888;">상태</span>
                <span class="badge badge-blue" style="margin-left:4px;">{{ relatedOrder.statusCd }}</span>
              </div>
            </div>
        ...
        <template v-if="relatedDliv">
          <div style="padding:12px 14px;background:#f0f7ff;border-radius:8px;border:1px solid #bae0ff;font-size:13px;">
            ...
            <div style="line-height:2;color:#444;">
              ...
              <b style="margin-left:4px;">{{ relatedDliv.receiver }}</b>
              &nbsp;·&nbsp;
              <span style="color:#888;">배송지</span>
              <b style="margin-left:4px;">{{ relatedDliv.address }}</b>
              <br/>
              <span style="color:#888;">택배사</span>
              <b style="margin-left:4px;">{{ relatedDliv.courierCd }}</b>
              &nbsp;·&nbsp;
              <span style="color:#888;">운송장</span>
              <b style="margin-left:4px;">{{ relatedDliv.trackingNo || '-' }}</b>
              &nbsp;·&nbsp;
              <span class="badge badge-green">{{ relatedDliv.statusCd }}</span>
            </div>
            <button class="btn btn-secondary btn-sm" style="margin-top:8px;" @click="navigate('odDlivDtl',{id:relatedDliv.dlivId})">
              배송 수정
            </button>
          </div>
        </template>
