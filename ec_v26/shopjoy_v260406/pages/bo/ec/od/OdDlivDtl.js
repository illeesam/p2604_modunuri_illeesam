...
window.OdDlivDtl = {
  ...
  setup(props) {
    ...
    const fnLoadCodes = async () => {
      ...
      await codeStore.saLoadCodes(['DLIV_STATUS'], {compNm: 'OdDlivDtl'});
    ...
    /* fnShareUrl — 이 배송 상세를 가리키는 독립 새창 딥링크 URL 생성 */
    const fnShareUrl = () => {
      const qs = new URLSearchParams();
      qs.set('page', 'odDlivDtl');
      qs.set('id', form.dlivId);
      qs.set('embed', '1');
    ...
    const handleShareKakao = () => {
      try {
        window.coExtSdk.shareKakao({
          title: `배송 ${form.dlivId} - ShopJoy BO`,
          description: form.recvNm || '',
    ...
    /* pdfAreaRef — 배송 상세 카드 캡처 대상. handleExportPdf — PDF 다운로드(상세보기 모드 전용) */
    const pdfAreaRef = ref(null);
    const pdfExporting = ref(false);
    const handleExportPdf = async () => {
      pdfExporting.value = true;
      try {
        const filename = coUtil.cofBuildExportFilename(`배송상세_${form.dlivId}.pdf`);
