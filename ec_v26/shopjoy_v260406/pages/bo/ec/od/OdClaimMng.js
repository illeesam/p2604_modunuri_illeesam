...
window.OdClaimMng = {
  ...
  setup(props) {
    ...
    /* 클레임(취소/반품/교환) closeMemberPick */
    const closeMemberPick = () => { memberPick.open = false; };

    /* 클레임(취소/반품/교환) handlePickSearch */
    ...
    /* BoGrid 컬럼 정의 (정렬 sortKey 'reg' 는 SORT_MAP 키와 일치) */
      ...
      { key: 'type', type: 'select', options: () => codes.claim_types, nullLabel: '유형 전체' },
      { key: 'status', type: 'select', options: () => codes.claim_statuses, nullLabel: '상태 전체' },
      ...
      <div style="background:linear-gradient(135deg,#fff0f4,#ffe4ec,#ffd5e1);padding:18px 24px 14px;border-bottom:1px solid #fce7f3;flex-shrink:0;">
        <div style="display:flex;align-items:center;justify-content:space-between;">
          <div>
            ...
            <div style="font-size:12px;color:#9ca3af;margin-top:2px;">클레임 조회 기준 회원을 선택해주세요</div>
          </div>
