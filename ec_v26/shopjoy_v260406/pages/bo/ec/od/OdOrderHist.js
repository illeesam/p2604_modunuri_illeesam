    ...
    const handleSearchData = async (searchType = 'DEFAULT') => {
      ...
      try {
        const [resO, resC, resD] = await Promise.all([
          window.boApi.get('/bo/ec/od/order/page', { params: { pageNo: 1, pageSize: 10000 }, ...coUtil.apiHdr('주문관리', '이력조회') }),
          window.boApi.get('/bo/ec/od/claim/page', { params: { pageNo: 1, pageSize: 10000 }, ...coUtil.apiHdr('클레임관리', '이력조회') }),
          window.boApi.get('/bo/ec/od/dliv/page', { params: { pageNo: 1, pageSize: 10000 }, ...coUtil.apiHdr('배송관리', '이력조회') }),
        ]);
