    ...
    const handleSearchDetail = async () => {
      ...
      try {
        const [orderRes, vendorsRes, deliveriesRes, claimsRes] = await Promise.all([
          window.boApi.get(`/bo/ec/od/order/${props.editId}`, { headers: { 'X-UI-Nm': '주문상세', 'X-Cmd-Nm': '상세조회' } }),
          window.boApi.get('/bo/sy/vendor/page', { params: { pageNo: 1, pageSize: 10000 }, headers: { 'X-UI-Nm': '주문상세', 'X-Cmd-Nm': '조회' } }),
          window.boApi.get('/bo/ec/od/dliv/page', { params: { pageNo: 1, pageSize: 10000 }, headers: { 'X-UI-Nm': '주문상세', 'X-Cmd-Nm': '조회' } }),
          window.boApi.get('/bo/ec/od/claim/page', { params: { pageNo: 1, pageSize: 10000 }, headers: { 'X-UI-Nm': '주문상세', 'X-Cmd-Nm': '조회' } }),
        ]);
    ...
    const handleSave = async () => {
      ...
      try {
        const res = await (isNewOrder
          ? window.boApi.post('/bo/ec/od/order', { ...form, totalPrice: Number(form.totalPrice) }, { headers: { 'X-UI-Nm': '주문관리', 'X-Cmd-Nm': '등록' } })
          : window.boApi.put(`/bo/ec/od/order/${form.orderId}`, { ...form, totalPrice: Number(form.totalPrice) }, { headers: { 'X-UI-Nm': '주문관리', 'X-Cmd-Nm': '저장' } }));
