...
window.CmNoticeDtl = {
  ...
  setup(props) {
    ...
    const handleSearchDetail = async () => {
      ...
      try {
        const res = await window.boApi.get(`/bo/ec/cm/notice/${props.editId}`, { ...apiHdr('공지사항관리', '상세조회') });
    ...
    const handleSave = async () => {
      ...
      try {
        const res = await (isNewNotice
          ? window.boApi.post('/bo/ec/cm/notice', { ...form }, { ...apiHdr('공지사항관리', '등록') })
          : window.boApi.put(`/bo/ec/cm/notice/${props.editId}`, { ...form }, { ...apiHdr('공지사항관리', '저장') }));
