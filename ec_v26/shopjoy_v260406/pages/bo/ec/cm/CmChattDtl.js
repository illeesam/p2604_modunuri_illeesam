...
window.CmChattDtl = {
  ...
  setup(props) {
    ...
    const handleSearchDetail = async () => {
      ...
      try {
        const res = await window.boApi.get(`/bo/ec/cm/chatt/${props.editId}`, { ...coUtil.apiHdr('채팅관리', '상세조회') });
    ...
    const handleSave = async () => {
      ...
      if (!ok) return;
      try {
        const res = await window.boApi.post(`/bo/ec/cm/chatt`, { ...form }, { ...coUtil.apiHdr('채팅관리', '등록') });
