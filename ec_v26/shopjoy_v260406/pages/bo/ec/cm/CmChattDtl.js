...
window.CmChattDtl = {
  ...
  setup(props) {
    ...
    const fnLoadCodes = async () => {
      ...
      await codeStore.saLoadCodes(['CHATT_STATUS'], {compNm: 'CmChattDtl'});
    ...
    const fnShareUrl = () => {
      const id = uiState.chat?.chattRoomId || props.dtlId;
      const qs = new URLSearchParams();
      qs.set('page', 'cmChattDtl');
      qs.set('id', id);
      qs.set('embed', '1');
      return `${window.location.origin}${window.location.pathname}?${qs.toString()}`;
    };
    /* handleShareKakao — 카카오톡 공유(피드 카드, 기존 채팅방 조회 시에만) */
    const handleShareKakao = () => {
      try {
        const id = uiState.chat?.chattRoomId || props.dtlId;
        window.coExtSdk.shareKakao({
          title: `채팅 ${id} - ShopJoy BO`,
          description: uiState.chat?.subject || uiState.chat?.memberNm || '',
    ...
    /* pdfAreaRef — 채팅 상세 카드 캡처 대상. handleExportPdf — PDF 다운로드(항상 노출) */
    const pdfAreaRef = ref(null);
    const pdfExporting = ref(false);
    const handleExportPdf = async () => {
      pdfExporting.value = true;
      try {
        const id = uiState.chat?.chattRoomId || props.dtlId || 'new';
        const filename = coUtil.cofBuildExportFilename(`채팅상세_${id}.pdf`);
...
<bo-container bare>
  ...
  <bo-container :title="!active ? '채팅 상세' : (cfIsNew ? '채팅 등록' : '채팅 상세')"
    ...
    <template #toolbar-actions>
      <button v-if="active ? !cfIsNew : false" class="btn btn_link" title="링크 공유(URL만)" @click="handleCopyLink">🔗</button>
      <button v-if="active ? !cfIsNew : false" class="btn btn_kakao" title="카카오톡 공유" @click="handleShareKakao">💬</button>
