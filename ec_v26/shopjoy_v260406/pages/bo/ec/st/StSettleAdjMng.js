...
const uiState = reactive({ descOpen: false, error: null, isPageCodeLoad: false, dateRange: '이번달', dateStart: '', dateEnd: '', selectedId: null, isNew: false});
    ...
    const handleSearchData = async (searchType = 'DEFAULT') => {
      try {
        const [resV, resA] = await Promise.all([
          ...
          boApi.get('/base/ec/st/settle-adj/page', {
            params: {
              pageNo: pager.pageNo, pageSize: pager.pageSize,
              ...Object.fromEntries(Object.entries(searchParam).filter(([, v]) => v !== '' && v !== null && v !== undefined))
            },
            ...coUtil.apiHdr('정산조정관리', '목록조회')
          })
        ]);
  ...
  const searchParamOrg = reactive({
    ...
    const handleSave = async () => {
      ...
      console.error('[catch-info]', err); err.inner.forEach(e => { errors[e.path] = e.message; }); props.showToast('입력 내용을 확인해주세요.', 'error'); return; }
      ...
      closeForm();
      try {
        const res = await (uiState.isNew ? boApi.post('/bo/ec/st/adj', { ...form }, coUtil.apiHdr('정산조정관리', '등록')) : boApi.put(`/bo/ec/st/adj/${form.adjId}`, { ...form }, coUtil.apiHdr('정산조정관리', '저장')));
    ...
    const handleDelete = async (r) => {
      ...
      try {
        const res = await boApi.delete(`/bo/ec/st/adj/${r.adjId}`, coUtil.apiHdr('정산조정관리', '삭제'));
        if (props.setApiRes) props.setApiRes({ ok: true, status: res.status, data: res.data });
    ...
    const doApprove = async (r) => {
      ...
      r.aprvStatus = '승인';
      try {
        const res = await boApi.put(`/bo/ec/st/adj/${r.adjId}/approve`, { aprvStatus: '승인' }, coUtil.apiHdr('정산조정관리', '상태변경'));
        if (props.setApiRes) props.setApiRes({ ok: true, status: res.status, data: res.data });
    ...
    return { uiState, codes, handleDateRangeChange, pager, adjList, cfPageNums, cfVendors, form, errors, openNew, openEdit, closeForm, handleSave, handleDelete, doApprove, fnAprvBadge, fnTypeBadge, fmtW, onSearch, onReset, searchParam, setPage, onSizeChange };
  },
  template: /* html */`
<div>
