...
window.StSettleCloseMng = {
  ...
  setup(props) {
    ...
    const handleSearchData = async (searchType = 'DEFAULT') => {
      try {
        const [resO, resC, resV] = await Promise.all([
          window.boApi.get('/bo/ec/od/order/page', { params: { pageNo: 1, pageSize: 10000 }, ...apiHdr('정산마감관리', '목록조회') }),
          window.boApi.get('/bo/ec/od/claim/page', { params: { pageNo: 1, pageSize: 10000 }, ...apiHdr('정산마감관리', '목록조회') }),
          window.boApi.get('/bo/sy/vendor/page', { params: { pageNo: 1, pageSize: 10000 }, ...apiHdr('정산마감관리', '목록조회') }),
        ]);
    ...
    const doClose = async () => {
      ...
      try {
        const res = await window.boApi.post('/bo/ec/st/close', { closeMon: thisMonth, sales: cfThisMonthSales.value, refund: cfThisMonthRefund.value, net: cfThisMonthNet.value, comm: cfThisMonthComm.value, promo: cfThisMonthPromo.value, settle: cfThisMonthSettle.value }, { ...apiHdr('정산마감관리', '저장') });
    ...
    const doReopen = async (r) => {
      ...
      r.status = '마감취소';
      try {
        const res = await window.boApi.put(`/bo/ec/st/close/${r.closeId}/reopen`, {}, { ...apiHdr('정산마감관리', '상태변경') });
        if (props.setApiRes) props.setApiRes({ ok: true, status: res.status, data: res.data });
