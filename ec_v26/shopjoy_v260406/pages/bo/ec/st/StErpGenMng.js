...
window.StErpGenMng = {
  ...
  setup(props) {
    ...
    const handleSearchData = async (searchType = 'DEFAULT') => {
      try {
        const [resO, resV] = await Promise.all([
          window.boApi.get('/bo/ec/od/order/page', { params: { pageNo: 1, pageSize: 10000 }, ...apiHdr('정산ERP생성', '목록조회') }),
          window.boApi.get('/bo/sy/vendor/page', { params: { pageNo: 1, pageSize: 10000 }, ...apiHdr('정산ERP생성', '목록조회') }),
        ]);
    ...
    const doGenerate = async () => {
      ...
      try {
        const res = await window.boApi.post('/bo/ec/st/erp/gen', { targetMon: targetMon.value, slipType: slipType.value, rows: cfPreviewRows.value }, { ...apiHdr('정산ERP생성', '저장') });
