  ...
  window.ZdSimulPromoMng = {
    ...
    setup(props) {
      ...
      const simul = useSimulSetup({
        domain: '프로모션',
        label: '시뮬프로모',
        defaultCfg: { mode: 'create', countMin: 1, countMax: 2, intervalVal: 12, intervalUnit: 'sec', durationMin: 3 },
        runFn: async ({ mode, namePrefix, randInt, pick }) => {
          if (mode === 'create') {
            ...
            if (type === 'coupon') {
              ...
              const res = await boApi.post('/bo/ec/pm/coupon/save/base', body, coUtil.apiHdr('프로모시뮬', '쿠폰생성'));
              ...
              const res = await boApi.post('/bo/ec/pm/discnt/save/base', body, coUtil.apiHdr('프로모시뮬', '할인생성'));
              ...
              const res = await boApi.post('/bo/ec/pm/save/save/base', body, coUtil.apiHdr('프로모시뮬', '적립생성'));
            ...
            const path  = type === 'coupon' ? '/bo/ec/pm/coupon/save/' + target.couponId : '/bo/ec/pm/discnt/save/' + target.discntId;
            await boApi.put(path, body, coUtil.apiHdr('프로모시뮬', '수정'));
      ...
      const couponCfgColumns = [
        ...
        { key: 'couponDiscRateMin',  label: '할인율 최소', type: 'number', hint: '%', visible: (f) => f.couponDiscType === 'RATE' },
        { key: 'couponDiscRateMax',  label: '할인율 최대', type: 'number', hint: '%', visible: (f) => f.couponDiscType === 'RATE' },
        ...
        { key: 'couponIssueCountMin', label: '발행수 최소', type: 'number', hint: '매' },
        { key: 'couponIssueCountMax', label: '발행수 최대', type: 'number', hint: '매' },
      ...
      const discntCfgColumns = [
        ...
        { key: 'discntRateMin',  label: '할인율 최소', type: 'number', hint: '%', visible: (f) => f.discntType === 'RATE' },
        { key: 'discntRateMax',  label: '할인율 최대', type: 'number', hint: '%', visible: (f) => f.discntType === 'RATE' },
      ...
      const saveCfgColumns = [
        { key: 'saveRateMin',      label: '적립률 최소',  type: 'number', hint: '%' },
        { key: 'saveRateMax',      label: '적립률 최대',  type: 'number', hint: '%' },
