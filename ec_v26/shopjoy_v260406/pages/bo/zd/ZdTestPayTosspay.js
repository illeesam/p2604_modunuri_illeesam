...
window.ZdTestPayTosspay = {
  ...
  setup(props) {
    ...
    const cfgFormColumns = [
      { key: 'clientKey', label: 'Client Key (클라이언트)', type: 'text', hint: 'test_ck_… or live_ck_…', mono: true, colSpan: 2 },
      { key: 'secretKey', label: 'Secret Key (서버)', type: 'text', hint: 'test_sk_… or live_sk_…', mono: true },
    ];

    const baseFormColumns = [
      { key: 'amount', label: '금액', type: 'number' },
      { key: 'orderId', label: '주문 ID', type: 'slot', name: 'orderId' },
      { key: 'orderName', label: '상품명', type: 'text' },
      { key: 'customerName', label: '구매자명', type: 'text' },
      { key: 'customerEmail', label: '구매자 이메일', type: 'text' },
    ];
    ...
    return { cfg, form, result, uiState, handleBtnAction, cfgFormColumns, baseFormColumns, confirmGridColumns };
  },
...
<div>
  ...
  <div class="card" style="margin-bottom:12px">
    <div class="toolbar">
      ...
      <span style="font-size:11px;color:#888;margin-left:8px">결제창 전용 키 (test_ck_ / live_ck_ 접두어)</span>
    </div>
