...
`scripts/scripts_deploy_illeesam_synol/`(배포 전용), `scripts/scripts_testUrl/`(URL 점검 전용)처럼 목적이
뚜렷한 도구는 각자 최상위 `scripts/scripts_*` 폴더를 갖지만, 그 정도로 크지 않은 **자잘한 개발용
유틸리티**는 여기 `scripts_funcs/` 밑에 **기능별 서브폴더**로 모은다 — 최상위에 폴더가 계속 늘어나는
걸 막기 위함(요청사항: "scripts/scripts_funcs 안에 서브폴더 만들고 기능 적재하면 좋겠다").
...
3. `scripts/scripts_deploy_illeesam_synol/package.json` 등 다른 폴더의 관례대로, 여러 개의 관련
