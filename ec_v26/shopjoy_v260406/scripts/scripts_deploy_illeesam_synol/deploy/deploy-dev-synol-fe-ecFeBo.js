 ...
 * 백엔드는 별도 deploy-dev-synol-be-ecBeBo.js(= cd deploy && npm run "apps DEPLOY ecBeBo-22300 (springboot)").
 * (2026-09-06: 예전엔 deploy:dev-synol-full 이 백엔드+프론트를 묶었으나, npm run "apps DEPLOY zmulti-ecBeBo-ecBeCdn"
 ...
 *
 * 사용법: scripts/scripts_deploy_illeesam_synol/deploy/ 에서 npm run "apps DEPLOY ecFeBo-22000 (nginx-vue3cdn)"
 *          (또는 루트에서 npm run "apps DEPLOY ecFeBo-22000 (nginx-vue3cdn)" --workspace=deploy)
...
(async () => {
  try {
    ...
    // API 헬스체크는 여기서 뺀다 — 백엔드 자체 헬스체크는 deploy/ 의 npm run "apps DEPLOY ecBeBo-22300 (springboot)" 가 담당.
